...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.PbListType.*;

public class bmr_PbListType {

	private static final String table_name = "PB_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		PbListType body = PbListType.Factory.newInstance();
		PbList[] list = new PbList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:pb_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='PbList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = PbList.Factory.newInstance();

					if (!set_string(list[0], "setAaSeqOneLetterCode", "setNilAaSeqOneLetterCode", false, rset.getString("AA_seq_one_letter_code"), logw))
						continue;
					...
					if (!set_string(list[0], "setOutputFileName", "setNilOutputFileName", false, rset.getString("Output_file_name"), logw))
						continue;
					if (!set_string(list[0], "setPbSeqCode", "setNilPbSeqCode", false, rset.getString("PB_seq_code"), logw))
						continue;
					if (!set_string(list[0], "setPdbId", "setNilPdbId", false, rset.getString("PDB_ID"), logw))
						continue;
					if (!set_string(list[0], "setPdbxExptlMethod", "setNilPdbxExptlMethod", false, rset.getString("PDBX_exptl_method"), logw))
						continue;
					if (!set_string(list[0], "setPdbxNmrRefineMethod", "setNilPdbxNmrRefineMethod", false, rset.getString("PDBX_NMR_refine_method"), logw))
						continue;
					if (!set_enum_pdbx_refine_ls_d_res_high(list[0], "setPdbxRefineLsDResHigh", "setNilPdbxRefineLsDResHigh", false, rset.getString("PDBX_refine_ls_d_res_high"), logw, errw))
						continue;
					if (!set_enum_pdbx_refine_ls_d_res_low(list[0], "setPdbxRefineLsDResLow", "setNilPdbxRefineLsDResLow", false, rset.getString("PDBX_refine_ls_d_res_low"), logw, errw))
						continue;
					if (!set_enum_pdbx_refine_ls_r_factor_r_free(list[0], "setPdbxRefineLsRFactorRFree", "setNilPdbxRefineLsRFactorRFree", false, rset.getString("PDBX_refine_ls_R_factor_R_free"), logw, errw))
						continue;
					if (!set_enum_pdbx_refine_ls_r_factor_r_work(list[0], "setPdbxRefineLsRFactorRWork", "setNilPdbxRefineLsRFactorRWork", false, rset.getString("PDBX_refine_ls_R_factor_R_work"), logw, errw))
						continue;
					if (!set_date(list[0], "setQueriedDate", "setNilQueriedDate", false, rset.getString("Queried_date") != null && !rset.getString("Queried_date").equals(".") && !rset.getString("Queried_date").equals("?") ? rset.getDate("Queried_date") : null, logw))
						continue;
					if (!set_string(list[0], "setQueryId", "setNilQueryId", false, rset.getString("Query_ID"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_PbListType.class.getName());
	...
	private static boolean set_string(PbList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='PbList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PbList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(PbList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_date(PbList list, String method_name, String nil_method_name, boolean required, java.sql.Date date, FileWriter logw) {
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PbList', value='" + date + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_enum_pdbx_refine_ls_d_res_high(PbList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			...
			try {
				if (nil)
					...
					String[] num_val = val_name.split("[Mm][Hh][Zz]$");
					BigDecimal _enum = BigDecimal.valueOf(Double.valueOf(num_val[0]));
					if (_enum != null)
	...
	private static boolean set_enum_pdbx_refine_ls_d_res_low(PbList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
	...
	private static boolean set_enum_pdbx_refine_ls_r_factor_r_free(PbList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
	...
	private static boolean set_enum_pdbx_refine_ls_r_factor_r_work(PbList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
