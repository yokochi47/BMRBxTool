...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SpinSystemLinkType.*;

public class bmr_SpinSystemLinkType {

	private static final String table_name = "Spin_system_link";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SpinSystemLinkType body = SpinSystemLinkType.Factory.newInstance();
		SpinSystemLink[] list = new SpinSystemLink[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Resonance_linker_list_ID\")::decimal,(0 || \"From_spin_system_ID\")::decimal,(0 || \"To_spin_system_ID\")::decimal");
				...
				buffw.write("  <BMRBx:spin_system_linkCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SpinSystemLink', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SpinSystemLink.Factory.newInstance();

					if (!set_integer(list[0], "setOffset", "setNilOffset", false, rset.getString("Offset"), logw))
						continue;
					if (!set_string(list[0], "setSelected", "setNilSelected", false, rset.getString("Selected"), logw))
						continue;
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setFromSpinSystemId", "", true, rset.getString("From_spin_system_ID"), logw))
						continue;
					if (!set_integer(list[0], "setResonanceLinkerListId", "", true, rset.getString("Resonance_linker_list_ID"), logw))
						continue;
					if (!set_integer(list[0], "setToSpinSystemId", "", true, rset.getString("To_spin_system_ID"), logw))
						continue;

					body.setSpinSystemLinkArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SpinSystemLinkType.class.getName());
	...
	private static boolean set_string(SpinSystemLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SpinSystemLink', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SpinSystemLink', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SpinSystemLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(SpinSystemLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ SpinSystemLink.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					SpinSystemLink.Type.Enum _enum = SpinSystemLink.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:SpinSystemLink method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= SpinSystemLink.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + SpinSystemLink.Type.Enum.forInt(i));
						try {
							errw.write("class_name:SpinSystemLink method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= SpinSystemLink.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + SpinSystemLink.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
