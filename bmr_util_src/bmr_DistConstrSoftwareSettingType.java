...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DistConstrSoftwareSettingType.*;

public class bmr_DistConstrSoftwareSettingType {

	private static final String table_name = "Dist_constr_software_setting";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DistConstrSoftwareSettingType body = DistConstrSoftwareSettingType.Factory.newInstance();
		DistConstrSoftwareSetting[] list = new DistConstrSoftwareSetting[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Distance_constraint_list_ID\")::decimal,(0 || \"Software_ID\")::decimal");
				...
				buffw.write("  <BMRBx:dist_constr_software_settingCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DistConstrSoftwareSetting', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DistConstrSoftwareSetting.Factory.newInstance();
					...
					body.setDistConstrSoftwareSettingArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DistConstrSoftwareSettingType.class.getName());
	...
	private static boolean set_string(DistConstrSoftwareSetting list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DistConstrSoftwareSetting', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DistConstrSoftwareSetting', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DistConstrSoftwareSetting list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(DistConstrSoftwareSetting list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ DistConstrSoftwareSetting.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					DistConstrSoftwareSetting.Type.Enum _enum = DistConstrSoftwareSetting.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:DistConstrSoftwareSetting method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= DistConstrSoftwareSetting.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + DistConstrSoftwareSetting.Type.Enum.forInt(i));
						try {
							errw.write("class_name:DistConstrSoftwareSetting method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= DistConstrSoftwareSetting.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + DistConstrSoftwareSetting.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
