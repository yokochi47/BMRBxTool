...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SampleComponentAtomIsotopeType.*;

public class bmr_SampleComponentAtomIsotopeType {

	private static final String table_name = "Sample_component_atom_isotope";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SampleComponentAtomIsotopeType body = SampleComponentAtomIsotopeType.Factory.newInstance();
		SampleComponentAtomIsotope[] list = new SampleComponentAtomIsotope[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Sample_component_ID\")::decimal,(0 || \"Comp_index_ID\")::decimal,(0 || \"Sample_ID\")::decimal");
				...
				buffw.write("  <BMRBx:sample_component_atom_isotopeCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SampleComponentAtomIsotope', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SampleComponentAtomIsotope.Factory.newInstance();
					...
					if (!set_string(list[0], "setCompId", "", true, rset.getString("Comp_ID"), logw))
						continue;
					if (!set_string(list[0], "setCompIsotopeLabelCode", "", true, rset.getString("Comp_isotope_label_code"), logw))
						continue;
					if (!set_string(list[0], "setEntityLabel", "setNilEntityLabel", false, rset.getString("Entity_label"), logw))
						continue;
					if (!set_decimal(list[0], "setLabelPct", "", true, rset.getString("Label_pct"), logw))
						continue;
					if (!set_string(list[0], "setMolCommonName", "setNilMolCommonName", false, rset.getString("Mol_common_name"), logw))
						continue;
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSampleComponentId", "", true, rset.getString("Sample_component_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSampleId", "", true, rset.getString("Sample_ID"), logw))
						continue;

					body.setSampleComponentAtomIsotopeArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SampleComponentAtomIsotopeType.class.getName());
	...
	private static boolean set_string(SampleComponentAtomIsotope list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SampleComponentAtomIsotope', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SampleComponentAtomIsotope', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SampleComponentAtomIsotope list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(SampleComponentAtomIsotope list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
