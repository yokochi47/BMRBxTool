...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DHFractFactorSoftwareType.*;

public class bmr_DHFractFactorSoftwareType {

	private static final String table_name = "D_H_fract_factor_software";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DHFractFactorSoftwareType body = DHFractFactorSoftwareType.Factory.newInstance();
		DHFractFactorSoftware[] list = new DHFractFactorSoftware[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"D_H_fractionation_factor_list_ID\")::decimal,(0 || \"Software_ID\")::decimal");
				...
				buffw.write("  <BMRBx:d_h_fract_factor_softwareCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DHFractFactorSoftware', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DHFractFactorSoftware.Factory.newInstance();
					...
					body.setDHFractFactorSoftwareArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DHFractFactorSoftwareType.class.getName());
	...
	private static boolean set_string(DHFractFactorSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DHFractFactorSoftware', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DHFractFactorSoftware', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DHFractFactorSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_software_id(DHFractFactorSoftware list, String method_name, String nil_method_name, boolean required, String val_name, int id, FileWriter logw) {
