...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.PdbxChemCompFeatureType.*;

public class bmr_PdbxChemCompFeatureType {

	private static final String table_name = "PDBX_chem_comp_feature";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		PdbxChemCompFeatureType body = PdbxChemCompFeatureType.Factory.newInstance();
		PdbxChemCompFeature[] list = new PdbxChemCompFeature[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:pdbx_chem_comp_featureCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='PdbxChemCompFeature', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = PdbxChemCompFeature.Factory.newInstance();

					if (!set_string(list[0], "setSupport", "setNilSupport", false, rset.getString("Support"), logw))
						continue;
					...
					body.setPdbxChemCompFeatureArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_PdbxChemCompFeatureType.class.getName());
	...
	private static boolean set_string(PdbxChemCompFeature list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='PdbxChemCompFeature', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PdbxChemCompFeature', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
