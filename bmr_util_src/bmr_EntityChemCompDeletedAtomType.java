...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EntityChemCompDeletedAtomType.*;

public class bmr_EntityChemCompDeletedAtomType {

	private static final String table_name = "Entity_chem_comp_deleted_atom";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EntityChemCompDeletedAtomType body = EntityChemCompDeletedAtomType.Factory.newInstance();
		EntityChemCompDeletedAtom[] list = new EntityChemCompDeletedAtom[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Comp_index_ID\")::decimal,(0 || \"Entity_ID\")::decimal");
				...
				buffw.write("  <BMRBx:entity_chem_comp_deleted_atomCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EntityChemCompDeletedAtom', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EntityChemCompDeletedAtom.Factory.newInstance();
					...
					body.setEntityChemCompDeletedAtomArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_EntityChemCompDeletedAtomType.class.getName());
	...
	private static boolean set_string(EntityChemCompDeletedAtom list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EntityChemCompDeletedAtom', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EntityChemCompDeletedAtom', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EntityChemCompDeletedAtom list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
