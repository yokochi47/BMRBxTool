...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemCompSystematicNameType.*;

public class bmr_ChemCompSystematicNameType {

	private static final String table_name = "Chem_comp_systematic_name";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemCompSystematicNameType body = ChemCompSystematicNameType.Factory.newInstance();
		ChemCompSystematicName[] list = new ChemCompSystematicName[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:chem_comp_systematic_nameCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemCompSystematicName', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemCompSystematicName.Factory.newInstance();
					...
					body.setChemCompSystematicNameArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemCompSystematicNameType.class.getName());
	...
	private static boolean set_string(ChemCompSystematicName list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemCompSystematicName', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemCompSystematicName', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_enum_naming_system(ChemCompSystematicName list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_ChemCompSystematicName.getNamingSystem(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemCompSystematicName.NamingSystem.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemCompSystematicName.NamingSystem.Enum _enum = ChemCompSystematicName.NamingSystem.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemCompSystematicName method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemCompSystematicName.NamingSystem.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemCompSystematicName.NamingSystem.Enum.forInt(i));
						try {
							errw.write("class_name:ChemCompSystematicName method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemCompSystematicName.NamingSystem.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemCompSystematicName.NamingSystem.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
