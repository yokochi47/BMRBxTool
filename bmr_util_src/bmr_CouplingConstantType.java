...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.CouplingConstantType.*;

public class bmr_CouplingConstantType {

	private static final String table_name = "Coupling_constant";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		CouplingConstantType body = CouplingConstantType.Factory.newInstance();
		CouplingConstant[] list = new CouplingConstant[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Coupling_constant_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:coupling_constantCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='CouplingConstant', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = CouplingConstant.Factory.newInstance();
					...
					if (!set_integer(list[0], "setResonanceId2", "setNilResonanceId2", false, rset.getString("Resonance_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setSeqId1", "", false, rset.getString("Seq_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setSeqId2", "", false, rset.getString("Seq_ID_2"), logw))
						continue;
					...
					body.setCouplingConstantArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_CouplingConstantType.class.getName());
	...
	private static boolean set_string(CouplingConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='CouplingConstant', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='CouplingConstant', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(CouplingConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(CouplingConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
