...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MzPrecursorIonType.*;

public class bmr_MzPrecursorIonType {

	private static final String table_name = "MZ_precursor_ion";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MzPrecursorIonType body = MzPrecursorIonType.Factory.newInstance();
		MzPrecursorIon[] list = new MzPrecursorIon[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"MZ_ratio_data_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:mz_precursor_ionCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MzPrecursorIon', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MzPrecursorIon.Factory.newInstance();
					...
					if (!set_decimal(list[0], "setFullWidthHalfMaximum", "setNilFullWidthHalfMaximum", false, rset.getString("Full_width_half_maximum"), logw))
						continue;
					if (!set_decimal(list[0], "setIntensity", "setNilIntensity", false, rset.getString("Intensity"), logw))
						continue;
					...
					if (!set_decimal(list[0], "setMzRatioErr", "setNilMzRatioErr", false, rset.getString("MZ_ratio_err"), logw))
						continue;
					if (!set_decimal(list[0], "setRelativeIntensity", "setNilRelativeIntensity", false, rset.getString("Relative_intensity"), logw))
						continue;
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setMzRatioDataListId", "", true, rset.getString("MZ_ratio_data_list_ID"), logw))
						continue;

					body.setMzPrecursorIonArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MzPrecursorIonType.class.getName());
	...
	private static boolean set_string(MzPrecursorIon list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MzPrecursorIon', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MzPrecursorIon', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MzPrecursorIon list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(MzPrecursorIon list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
