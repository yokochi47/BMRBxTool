...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MsChromatogramSoftwareType.*;

public class bmr_MsChromatogramSoftwareType {

	private static final String table_name = "MS_chromatogram_software";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MsChromatogramSoftwareType body = MsChromatogramSoftwareType.Factory.newInstance();
		MsChromatogramSoftware[] list = new MsChromatogramSoftware[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"MS_chromatogram_list_ID\")::decimal,(0 || \"Software_ID\")::decimal");
				...
				buffw.write("  <BMRBx:ms_chromatogram_softwareCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MsChromatogramSoftware', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MsChromatogramSoftware.Factory.newInstance();
					...
					body.setMsChromatogramSoftwareArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MsChromatogramSoftwareType.class.getName());
	...
	private static boolean set_string(MsChromatogramSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MsChromatogramSoftware', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MsChromatogramSoftware', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MsChromatogramSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_software_id(MsChromatogramSoftware list, String method_name, String nil_method_name, boolean required, String val_name, int id, FileWriter logw) {
