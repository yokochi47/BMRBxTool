...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalChemShiftType.*;

public class bmr_TheoreticalChemShiftType {

	private static final String table_name = "Theoretical_chem_shift";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalChemShiftType body = TheoreticalChemShiftType.Factory.newInstance();
		TheoreticalChemShift[] list = new TheoreticalChemShift[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Theoretical_chem_shift_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:theoretical_chem_shiftCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalChemShift', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalChemShift.Factory.newInstance();
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setFermiContactSpinDensity", "setNilFermiContactSpinDensity", false, rset.getString("Fermi_contact_spin_density"), logw))
						continue;
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setTheoreticalChemShiftListId", "", true, rset.getString("Theoretical_chem_shift_list_ID"), logw))
						continue;

					body.setTheoreticalChemShiftArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalChemShiftType.class.getName());
	...
	private static boolean set_string(TheoreticalChemShift list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalChemShift', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalChemShift', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalChemShift list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(TheoreticalChemShift list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
