...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.InteratomicDistType.*;

public class bmr_InteratomicDistType {

	private static final String table_name = "Interatomic_dist";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		InteratomicDistType body = InteratomicDistType.Factory.newInstance();
		InteratomicDist[] list = new InteratomicDist[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Interatomic_distance_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:interatomic_distCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='InteratomicDist', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = InteratomicDist.Factory.newInstance();
					...
					if (!set_integer(list[0], "setSeqId2", "", true, rset.getString("Seq_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setSetId", "", true, rset.getString("Set_ID"), logw))
						continue;
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setInteratomicDistanceListId", "", true, rset.getString("Interatomic_distance_list_ID"), logw))
						continue;

					body.setInteratomicDistArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_InteratomicDistType.class.getName());
	...
	private static boolean set_string(InteratomicDist list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='InteratomicDist', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='InteratomicDist', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(InteratomicDist list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(InteratomicDist list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
