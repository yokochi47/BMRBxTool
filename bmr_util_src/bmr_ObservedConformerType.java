...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ObservedConformerType.*;

public class bmr_ObservedConformerType {

	private static final String table_name = "Observed_conformer";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ObservedConformerType body = ObservedConformerType.Factory.newInstance();
		ObservedConformer[] list = new ObservedConformer[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Structure_interaction_list_ID\")::decimal,(0 || \"Conformer_family_coord_set_ID\")::decimal");
				...
				buffw.write("  <BMRBx:observed_conformerCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ObservedConformer', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ObservedConformer.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setStructureInteractionId", "", true, rset.getString("Structure_interaction_ID"), logw))
						continue;
					if (!set_integer(list[0], "setStructureInteractionListId", "", true, rset.getString("Structure_interaction_list_ID"), logw))
						continue;

					body.setObservedConformerArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ObservedConformerType.class.getName());
	...
	private static boolean set_string(ObservedConformer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ObservedConformer', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ObservedConformer', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ObservedConformer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
