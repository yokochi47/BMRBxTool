...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ForceConstantListType.*;

public class bmr_ForceConstantListType {

	private static final String table_name = "Force_constant_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ForceConstantListType body = ForceConstantListType.Factory.newInstance();
		ForceConstantList[] list = new ForceConstantList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:force_constant_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ForceConstantList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ForceConstantList.Factory.newInstance();

					if (!set_enum_default_software_values_used(list[0], "setDefaultSoftwareValuesUsed", "setNilDefaultSoftwareValuesUsed", false, rset.getString("Default_software_values_used"), logw, errw))
						continue;
					...
					body.setForceConstantListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ForceConstantListType.class.getName());
	...
	private static boolean set_string(ForceConstantList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ForceConstantList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ForceConstantList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ForceConstantList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_default_software_values_used(ForceConstantList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ForceConstantList.DefaultSoftwareValuesUsed.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ForceConstantList.DefaultSoftwareValuesUsed.Enum _enum = ForceConstantList.DefaultSoftwareValuesUsed.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ForceConstantList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ForceConstantList.DefaultSoftwareValuesUsed.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ForceConstantList.DefaultSoftwareValuesUsed.Enum.forInt(i));
						try {
							errw.write("class_name:ForceConstantList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ForceConstantList.DefaultSoftwareValuesUsed.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ForceConstantList.DefaultSoftwareValuesUsed.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
