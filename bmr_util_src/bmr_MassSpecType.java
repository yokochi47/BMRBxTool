...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MassSpecType.*;

public class bmr_MassSpecType {

	private static final String table_name = "Mass_spec";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MassSpecType body = MassSpecType.Factory.newInstance();
		MassSpec[] list = new MassSpec[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:mass_specCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MassSpec', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MassSpec.Factory.newInstance();
			...
			Logger lgr = Logger.getLogger(bmr_MassSpecType.class.getName());
	...
	private static boolean set_string(MassSpec list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MassSpec', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MassSpec', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MassSpec list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_manufacturer(MassSpec list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ MassSpec.Manufacturer.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					MassSpec.Manufacturer.Enum _enum = MassSpec.Manufacturer.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:MassSpec method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= MassSpec.Manufacturer.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + MassSpec.Manufacturer.Enum.forInt(i));
						try {
							errw.write("class_name:MassSpec method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= MassSpec.Manufacturer.Enum.table.lastInt(); i++)
								errw.write(" enum:" + MassSpec.Manufacturer.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_type(MassSpec list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ MassSpec.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					MassSpec.Type.Enum _enum = MassSpec.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:MassSpec method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= MassSpec.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + MassSpec.Type.Enum.forInt(i));
						try {
							errw.write("class_name:MassSpec method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= MassSpec.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + MassSpec.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
