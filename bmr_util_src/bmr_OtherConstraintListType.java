...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.OtherConstraintListType.*;

public class bmr_OtherConstraintListType {

	private static final String table_name = "Other_constraint_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		OtherConstraintListType body = OtherConstraintListType.Factory.newInstance();
		OtherConstraintList[] list = new OtherConstraintList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:other_constraint_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='OtherConstraintList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = OtherConstraintList.Factory.newInstance();
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_string(list[0], "setFileFormat", "", true, rset.getString("File_format"), logw))
						continue;
					...
					if (!set_string(list[0], "setSfFramecode", "", true, rset.getString("Sf_framecode"), logw))
						continue;
					if (!set_string(list[0], "setSubtype", "", true, rset.getString("Subtype"), logw))
						continue;
					if (!set_string(list[0], "setTextData", "", true, rset.getString("Text_data"), logw))
						continue;
					if (!set_string(list[0], "setTextDataFormat", "setNilTextDataFormat", false, rset.getString("Text_data_format"), logw))
						continue;
					if (!set_string(list[0], "setType", "", true, rset.getString("Type"), logw))
						continue;
					...
					body.setOtherConstraintListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_OtherConstraintListType.class.getName());
	...
	private static boolean set_string(OtherConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='OtherConstraintList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='OtherConstraintList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(OtherConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
