...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.LacsCharType.*;

public class bmr_LacsCharType {

	private static final String table_name = "LACS_char";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		LacsCharType body = LacsCharType.Factory.newInstance();
		LacsChar[] list = new LacsChar[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"LACS_plot_ID\")::decimal");
				...
				buffw.write("  <BMRBx:lacs_charCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='LacsChar', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = LacsChar.Factory.newInstance();
					...
					if (!set_integer(list[0], "setCompIndexId", "", true, rset.getString("Comp_index_ID"), logw))
						continue;
					if (!set_string(list[0], "setDesignator", "setNilDesignator", false, rset.getString("Designator"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSfId", "", true, rset.getString("Sf_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setXCoordVal", "setNilXCoordVal", false, rset.getString("X_coord_val"), logw))
						continue;
					if (!set_decimal(list[0], "setYCoordVal", "setNilYCoordVal", false, rset.getString("Y_coord_val"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setLacsPlotId", "", true, rset.getString("LACS_plot_ID"), logw))
						continue;

					body.setLacsCharArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_LacsCharType.class.getName());
	...
	private static boolean set_string(LacsChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='LacsChar', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='LacsChar', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(LacsChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(LacsChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
