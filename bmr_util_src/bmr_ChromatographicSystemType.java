...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChromatographicSystemType.*;

public class bmr_ChromatographicSystemType {

	private static final String table_name = "Chromatographic_system";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChromatographicSystemType body = ChromatographicSystemType.Factory.newInstance();
		ChromatographicSystem[] list = new ChromatographicSystem[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:chromatographic_systemCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChromatographicSystem', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChromatographicSystem.Factory.newInstance();

					if (!set_string(list[0], "setAutosamplerManufacturer", "setNilAutosamplerManufacturer", false, rset.getString("Autosampler_manufacturer"), logw))
						continue;
					if (!set_string(list[0], "setAutosamplerModel", "setNilAutosamplerModel", false, rset.getString("Autosampler_model"), logw))
						continue;
					if (!set_string(list[0], "setAutosamplerSerialNumber", "setNilAutosamplerSerialNumber", false, rset.getString("Autosampler_serial_number"), logw))
						continue;
					if (!set_string(list[0], "setColCompartmentManufacturer", "setNilColCompartmentManufacturer", false, rset.getString("Col_compartment_manufacturer"), logw))
						continue;
					if (!set_string(list[0], "setColCompartmentModel", "setNilColCompartmentModel", false, rset.getString("Col_compartment_model"), logw))
						continue;
					if (!set_string(list[0], "setColCompartmentSerialNumber", "setNilColCompartmentSerialNumber", false, rset.getString("Col_compartment_serial_number"), logw))
						continue;
					...
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_string(list[0], "setPumpManufacturer", "", true, rset.getString("Pump_manufacturer"), logw))
						continue;
					if (!set_string(list[0], "setPumpModel", "", true, rset.getString("Pump_model"), logw))
						continue;
					if (!set_string(list[0], "setPumpSerialNumber", "setNilPumpSerialNumber", false, rset.getString("Pump_serial_number"), logw))
						continue;
					...
					body.setChromatographicSystemArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChromatographicSystemType.class.getName());
	...
	private static boolean set_string(ChromatographicSystem list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChromatographicSystem', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChromatographicSystem', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChromatographicSystem list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
