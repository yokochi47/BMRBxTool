...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.RecouplingPulseSequenceType.*;

public class bmr_RecouplingPulseSequenceType {

	private static final String table_name = "Recoupling_pulse_sequence";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		RecouplingPulseSequenceType body = RecouplingPulseSequenceType.Factory.newInstance();
		RecouplingPulseSequence[] list = new RecouplingPulseSequence[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:recoupling_pulse_sequenceCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='RecouplingPulseSequence', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = RecouplingPulseSequence.Factory.newInstance();

					if (!set_string(list[0], "setNucleus", "", true, rset.getString("Nucleus"), logw))
						continue;
					...
					if (!set_integer(list[0], "setNmrSpecExptId", "", true, rset.getString("NMR_spec_expt_ID"), logw))
						continue;
					if (!set_string(list[0], "setTimePeriod", "", true, rset.getString("Time_period"), logw))
						continue;

					body.setRecouplingPulseSequenceArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_RecouplingPulseSequenceType.class.getName());
	...
	private static boolean set_string(RecouplingPulseSequence list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='RecouplingPulseSequence', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='RecouplingPulseSequence', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(RecouplingPulseSequence list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
