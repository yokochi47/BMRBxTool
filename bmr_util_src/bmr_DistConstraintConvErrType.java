...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DistConstraintConvErrType.*;

public class bmr_DistConstraintConvErrType {

	private static final String table_name = "Dist_constraint_conv_err";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DistConstraintConvErrType body = DistConstraintConvErrType.Factory.newInstance();
		DistConstraintConvErr[] list = new DistConstraintConvErr[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:dist_constraint_conv_errCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DistConstraintConvErr', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DistConstraintConvErr.Factory.newInstance();

					if (!set_string(list[0], "setConvErrorNote", "setNilConvErrorNote", false, rset.getString("Conv_error_note"), logw))
						continue;
					if (!set_string(list[0], "setConvErrorType", "", true, rset.getString("Conv_error_type"), logw))
						continue;
					if (!set_integer(list[0], "setDistConstraintParseFileId", "", true, rset.getString("Dist_constraint_parse_file_ID"), logw))
						continue;
					if (!set_integer(list[0], "setParseFileConstraintId", "", true, rset.getString("Parse_file_constraint_ID"), logw))
						continue;
					...
					body.setDistConstraintConvErrArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DistConstraintConvErrType.class.getName());
	...
	private static boolean set_string(DistConstraintConvErr list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DistConstraintConvErr', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DistConstraintConvErr', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DistConstraintConvErr list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
