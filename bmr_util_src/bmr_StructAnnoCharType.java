...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.StructAnnoCharType.*;

public class bmr_StructAnnoCharType {

	private static final String table_name = "Struct_anno_char";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		StructAnnoCharType body = StructAnnoCharType.Factory.newInstance();
		StructAnnoChar[] list = new StructAnnoChar[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:struct_anno_charCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='StructAnnoChar', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = StructAnnoChar.Factory.newInstance();
					...
					if (!set_integer(list[0], "setCompIndexId", "", true, rset.getString("Comp_index_ID"), logw))
						continue;
					if (!set_string(list[0], "setEdgeDesignation", "setNilEdgeDesignation", false, rset.getString("Edge_designation"), logw))
						continue;
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setHydrophobicity", "setNilHydrophobicity", false, rset.getString("Hydrophobicity"), logw))
						continue;
					if (!set_string(list[0], "setPdbStrandId", "setNilPdbStrandId", false, rset.getString("PDB_strand_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setPhiAngle", "setNilPhiAngle", false, rset.getString("Phi_angle"), logw))
						continue;
					if (!set_decimal(list[0], "setPsiAngle", "setNilPsiAngle", false, rset.getString("Psi_angle"), logw))
						continue;
					if (!set_string(list[0], "setSecondaryStructureCode", "setNilSecondaryStructureCode", false, rset.getString("Secondary_structure_code"), logw))
						continue;
					if (!set_decimal(list[0], "setSolventAccessibleSurfaceArea", "setNilSolventAccessibleSurfaceArea", false, rset.getString("Solvent_accessible_surface_area"), logw))
						continue;
					...
					body.setStructAnnoCharArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_StructAnnoCharType.class.getName());
	...
	private static boolean set_string(StructAnnoChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='StructAnnoChar', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='StructAnnoChar', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(StructAnnoChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(StructAnnoChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
