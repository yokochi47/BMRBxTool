...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ConformerFamilyRefinementType.*;

public class bmr_ConformerFamilyRefinementType {

	private static final String table_name = "Conformer_family_refinement";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ConformerFamilyRefinementType body = ConformerFamilyRefinementType.Factory.newInstance();
		ConformerFamilyRefinement[] list = new ConformerFamilyRefinement[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:conformer_family_refinementCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ConformerFamilyRefinement', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ConformerFamilyRefinement.Factory.newInstance();

					if (!set_string(list[0], "setRefineDetails", "setNilRefineDetails", false, rset.getString("Refine_details"), logw))
						continue;
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_enum_refine_method(list[0], "setRefineMethod", "", true, rset.getString("Refine_method"), logw, errw))
						continue;

					body.setConformerFamilyRefinementArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ConformerFamilyRefinementType.class.getName());
	...
	private static boolean set_string(ConformerFamilyRefinement list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ConformerFamilyRefinement', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ConformerFamilyRefinement', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ConformerFamilyRefinement list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_refine_method(ConformerFamilyRefinement list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_ConformerFamilyRefinement.getRefineMethod(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ConformerFamilyRefinement.RefineMethod.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ConformerFamilyRefinement.RefineMethod.Enum _enum = ConformerFamilyRefinement.RefineMethod.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ConformerFamilyRefinement method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ConformerFamilyRefinement.RefineMethod.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ConformerFamilyRefinement.RefineMethod.Enum.forInt(i));
						try {
							errw.write("class_name:ConformerFamilyRefinement method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ConformerFamilyRefinement.RefineMethod.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ConformerFamilyRefinement.RefineMethod.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
