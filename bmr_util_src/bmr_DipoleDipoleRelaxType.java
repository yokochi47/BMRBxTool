...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DipoleDipoleRelaxType.*;

public class bmr_DipoleDipoleRelaxType {

	private static final String table_name = "Dipole_dipole_relax";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DipoleDipoleRelaxType body = DipoleDipoleRelaxType.Factory.newInstance();
		DipoleDipoleRelax[] list = new DipoleDipoleRelax[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Dipole_dipole_relax_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:dipole_dipole_relaxCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DipoleDipoleRelax', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DipoleDipoleRelax.Factory.newInstance();
					...
					body.setDipoleDipoleRelaxArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DipoleDipoleRelaxType.class.getName());
	...
	private static boolean set_string(DipoleDipoleRelax list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DipoleDipoleRelax', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DipoleDipoleRelax', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DipoleDipoleRelax list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(DipoleDipoleRelax list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
