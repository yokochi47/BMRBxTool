...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DeducedSecdStructExptlType.*;

public class bmr_DeducedSecdStructExptlType {

	private static final String table_name = "Deduced_secd_struct_exptl";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DeducedSecdStructExptlType body = DeducedSecdStructExptlType.Factory.newInstance();
		DeducedSecdStructExptl[] list = new DeducedSecdStructExptl[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Deduced_secd_struct_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:deduced_secd_struct_exptlCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DeducedSecdStructExptl', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DeducedSecdStructExptl.Factory.newInstance();

					if (!set_string(list[0], "setAuthSeqIdEnd", "setNilAuthSeqIdEnd", false, rset.getString("Auth_seq_ID_end"), logw))
						continue;
					if (!set_string(list[0], "setAuthSeqIdStart", "setNilAuthSeqIdStart", false, rset.getString("Auth_seq_ID_start"), logw))
						continue;
					if (!set_string(list[0], "setCode", "", true, rset.getString("Code"), logw))
						continue;
					if (!set_integer(list[0], "setCompIndexIdEnd", "", true, rset.getString("Comp_index_ID_end"), logw))
						continue;
					if (!set_integer(list[0], "setCompIndexIdStart", "", true, rset.getString("Comp_index_ID_start"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSelectionMethodId", "", true, rset.getString("Selection_method_ID"), logw))
						continue;
					if (!set_string(list[0], "setSelectionMethodLabel", "setNilSelectionMethodLabel", false, rset.getString("Selection_method_label"), logw))
						continue;
					if (!set_integer(list[0], "setSeqIdEnd", "", true, rset.getString("Seq_ID_end"), logw))
						continue;
					if (!set_integer(list[0], "setSeqIdStart", "", true, rset.getString("Seq_ID_start"), logw))
						continue;
					if (!set_decimal(list[0], "setStaticFieldOrientationAngle", "setNilStaticFieldOrientationAngle", false, rset.getString("Static_field_orientation_angle"), logw))
						continue;
					if (!set_integer(list[0], "setDeducedSecdStructListId", "", true, rset.getString("Deduced_secd_struct_list_ID"), logw))
						continue;
					...
					body.setDeducedSecdStructExptlArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DeducedSecdStructExptlType.class.getName());
	...
	private static boolean set_string(DeducedSecdStructExptl list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DeducedSecdStructExptl', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DeducedSecdStructExptl', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DeducedSecdStructExptl list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(DeducedSecdStructExptl list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
