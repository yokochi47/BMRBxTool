...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.PhParamListType.*;

public class bmr_PhParamListType {

	private static final String table_name = "PH_param_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		PhParamListType body = PhParamListType.Factory.newInstance();
		PhParamList[] list = new PhParamList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:ph_param_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='PhParamList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = PhParamList.Factory.newInstance();
					...
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_enum_observed_nmr_param(list[0], "setObservedNmrParam", "", true, rset.getString("Observed_NMR_param"), logw, errw))
						continue;
					if (!set_integer(list[0], "setPhTitrationListId", "", true, rset.getString("PH_titration_list_ID"), logw))
						continue;
					if (!set_string(list[0], "setPhTitrationListLabel", "", true, rset.getString("PH_titration_list_label"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_PhParamListType.class.getName());
	...
	private static boolean set_string(PhParamList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='PhParamList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PhParamList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(PhParamList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_observed_nmr_param(PhParamList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ PhParamList.ObservedNmrParam.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					PhParamList.ObservedNmrParam.Enum _enum = PhParamList.ObservedNmrParam.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:PhParamList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= PhParamList.ObservedNmrParam.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + PhParamList.ObservedNmrParam.Enum.forInt(i));
						try {
							errw.write("class_name:PhParamList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= PhParamList.ObservedNmrParam.Enum.table.lastInt(); i++)
								errw.write(" enum:" + PhParamList.ObservedNmrParam.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
