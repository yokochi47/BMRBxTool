...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SpectralTransitionType.*;

public class bmr_SpectralTransitionType {

	private static final String table_name = "Spectral_transition";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SpectralTransitionType body = SpectralTransitionType.Factory.newInstance();
		SpectralTransition[] list = new SpectralTransition[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:spectral_transitionCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SpectralTransition', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SpectralTransition.Factory.newInstance();
					...
					body.setSpectralTransitionArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SpectralTransitionType.class.getName());
	...
	private static boolean set_string(SpectralTransition list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SpectralTransition', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SpectralTransition', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SpectralTransition list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(SpectralTransition list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
