...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MsChromatogramListType.*;

public class bmr_MsChromatogramListType {

	private static final String table_name = "MS_chromatogram_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MsChromatogramListType body = MsChromatogramListType.Factory.newInstance();
		MsChromatogramList[] list = new MsChromatogramList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:ms_chromatogram_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MsChromatogramList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MsChromatogramList.Factory.newInstance();
					...
					if (!set_string(list[0], "setTextData", "setNilTextData", false, rset.getString("Text_data"), logw))
						continue;
					if (!set_enum_text_data_format(list[0], "setTextDataFormat", "setNilTextDataFormat", false, rset.getString("Text_data_format"), logw, errw))
						continue;
					...
					body.setMsChromatogramListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MsChromatogramListType.class.getName());
	...
	private static boolean set_string(MsChromatogramList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MsChromatogramList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MsChromatogramList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MsChromatogramList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_text_data_format(MsChromatogramList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ MsChromatogramList.TextDataFormat.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					MsChromatogramList.TextDataFormat.Enum _enum = MsChromatogramList.TextDataFormat.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:MsChromatogramList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= MsChromatogramList.TextDataFormat.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + MsChromatogramList.TextDataFormat.Enum.forInt(i));
						try {
							errw.write("class_name:MsChromatogramList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= MsChromatogramList.TextDataFormat.Enum.table.lastInt(); i++)
								errw.write(" enum:" + MsChromatogramList.TextDataFormat.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
