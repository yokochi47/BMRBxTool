...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HomonuclNoeExperimentType.*;

public class bmr_HomonuclNoeExperimentType {

	private static final String table_name = "Homonucl_NOE_experiment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HomonuclNoeExperimentType body = HomonuclNoeExperimentType.Factory.newInstance();
		HomonuclNoeExperiment[] list = new HomonuclNoeExperiment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Homonucl_NOE_list_ID\")::decimal,(0 || \"Experiment_ID\")::decimal");
				...
				buffw.write("  <BMRBx:homonucl_noe_experimentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HomonuclNoeExperiment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HomonuclNoeExperiment.Factory.newInstance();
					...
					if (!set_string(list[0], "setSampleLabel", "setNilSampleLabel", false, rset.getString("Sample_label"), logw))
						continue;
					if (!set_string(list[0], "setSampleState", "setNilSampleState", false, rset.getString("Sample_state"), logw))
						continue;
					...
					if (!set_integer_experiment_id(list[0], "setExperimentId", "", true, rset.getString("Experiment_ID"), conn_bmrb, entry_id, rset.getString("Sample_ID"), logw))
						continue;
					if (!set_integer(list[0], "setHomonuclNoeListId", "", true, rset.getString("Homonucl_NOE_list_ID"), logw))
						continue;

					body.setHomonuclNoeExperimentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HomonuclNoeExperimentType.class.getName());
	...
	private static boolean set_string(HomonuclNoeExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HomonuclNoeExperiment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HomonuclNoeExperiment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HomonuclNoeExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_experiment_id(HomonuclNoeExperiment list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String sample_id, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")))
			val_name = bmr_Util_BMRB.getExperimentID(val_name, conn_bmrb, entry_id, sample_id, bmr_Util_BMRB.experiment_type.HomonuclNOE);
	...
	private static boolean set_integer_sample_id(HomonuclNoeExperiment list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String experiment_id, FileWriter logw) {
