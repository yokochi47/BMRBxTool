...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MzRatioSpectrumParamType.*;

public class bmr_MzRatioSpectrumParamType {

	private static final String table_name = "MZ_ratio_spectrum_param";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MzRatioSpectrumParamType body = MzRatioSpectrumParamType.Factory.newInstance();
		MzRatioSpectrumParam[] list = new MzRatioSpectrumParam[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:mz_ratio_spectrum_paramCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MzRatioSpectrumParam', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MzRatioSpectrumParam.Factory.newInstance();

					if (!set_string(list[0], "setName", "", true, rset.getString("Name"), logw))
						continue;
					if (!set_string(list[0], "setUnits", "setNilUnits", false, rset.getString("Units"), logw))
						continue;
					...
					body.setMzRatioSpectrumParamArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MzRatioSpectrumParamType.class.getName());
	...
	private static boolean set_string(MzRatioSpectrumParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MzRatioSpectrumParam', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MzRatioSpectrumParam', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MzRatioSpectrumParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
