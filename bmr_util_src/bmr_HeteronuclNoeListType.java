...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HeteronuclNoeListType.*;

public class bmr_HeteronuclNoeListType {

	private static final String table_name = "Heteronucl_NOE_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HeteronuclNoeListType body = HeteronuclNoeListType.Factory.newInstance();
		HeteronuclNoeList[] list = new HeteronuclNoeList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:heteronucl_noe_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HeteronuclNoeList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HeteronuclNoeList.Factory.newInstance();
					...
					body.setHeteronuclNoeListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HeteronuclNoeListType.class.getName());
	...
	private static boolean set_string(HeteronuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HeteronuclNoeList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HeteronuclNoeList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HeteronuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(HeteronuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_heteronuclear_noe_val_type(HeteronuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_HeteronuclNOEList.getHeteronuclearNOEValType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ HeteronuclNoeList.HeteronuclearNoeValType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					HeteronuclNoeList.HeteronuclearNoeValType.Enum _enum = HeteronuclNoeList.HeteronuclearNoeValType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:HeteronuclNoeList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= HeteronuclNoeList.HeteronuclearNoeValType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + HeteronuclNoeList.HeteronuclearNoeValType.Enum.forInt(i));
						try {
							errw.write("class_name:HeteronuclNoeList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= HeteronuclNoeList.HeteronuclearNoeValType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + HeteronuclNoeList.HeteronuclearNoeValType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
