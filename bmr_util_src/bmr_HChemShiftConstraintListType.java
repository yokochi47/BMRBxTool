...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HChemShiftConstraintListType.*;

public class bmr_HChemShiftConstraintListType {

	private static final String table_name = "H_chem_shift_constraint_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HChemShiftConstraintListType body = HChemShiftConstraintListType.Factory.newInstance();
		HChemShiftConstraintList[] list = new HChemShiftConstraintList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:h_chem_shift_constraint_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HChemShiftConstraintList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HChemShiftConstraintList.Factory.newInstance();
					...
					if (!set_string(list[0], "setTextDataFormat", "setNilTextDataFormat", false, rset.getString("Text_data_format"), logw))
						continue;
					if (!set_string(list[0], "setUnits", "", true, rset.getString("Units"), logw))
						continue;
					...
					body.setHChemShiftConstraintListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HChemShiftConstraintListType.class.getName());
	...
	private static boolean set_string(HChemShiftConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HChemShiftConstraintList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HChemShiftConstraintList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HChemShiftConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
