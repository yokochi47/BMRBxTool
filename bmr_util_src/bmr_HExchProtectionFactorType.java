...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HExchProtectionFactorType.*;

public class bmr_HExchProtectionFactorType {

	private static final String table_name = "H_exch_protection_factor";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HExchProtectionFactorType body = HExchProtectionFactorType.Factory.newInstance();
		HExchProtectionFactor[] list = new HExchProtectionFactor[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"H_exch_protection_factor_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:h_exch_protection_factorCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HExchProtectionFactor', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HExchProtectionFactor.Factory.newInstance();
					...
					if (!set_string(list[0], "setAuthSeqId", "setNilAuthSeqId", false, rset.getString("Auth_seq_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setCalculatedIntrinsicRate", "setNilCalculatedIntrinsicRate", false, rset.getString("Calculated_intrinsic_rate"), logw))
						continue;
					...
					body.setHExchProtectionFactorArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HExchProtectionFactorType.class.getName());
	...
	private static boolean set_string(HExchProtectionFactor list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HExchProtectionFactor', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HExchProtectionFactor', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HExchProtectionFactor list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(HExchProtectionFactor list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
