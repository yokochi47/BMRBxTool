...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AuxiliaryFilesType.*;

public class bmr_AuxiliaryFilesType {

	private static final String table_name = "Auxiliary_files";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AuxiliaryFilesType body = AuxiliaryFilesType.Factory.newInstance();
		AuxiliaryFiles[] list = new AuxiliaryFiles[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:auxiliary_filesCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AuxiliaryFiles', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AuxiliaryFiles.Factory.newInstance();
					...
					if (!set_string(list[0], "setDoi", "setNilDoi", false, rset.getString("DOI"), logw))
						continue;
					if (!set_string(list[0], "setFormat", "", true, rset.getString("Format"), logw))
						continue;
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_string(list[0], "setPath", "", true, rset.getString("Path"), logw))
						continue;
					if (!set_string(list[0], "setUri", "setNilUri", false, rset.getString("URI"), logw))
						continue;
					...
					body.setAuxiliaryFilesArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AuxiliaryFilesType.class.getName());
	...
	private static boolean set_string(AuxiliaryFiles list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AuxiliaryFiles', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AuxiliaryFiles', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AuxiliaryFiles list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
