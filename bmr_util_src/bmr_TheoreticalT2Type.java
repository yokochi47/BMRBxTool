...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalT2Type.*;

public class bmr_TheoreticalT2Type {

	private static final String table_name = "Theoretical_T2";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalT2Type body = TheoreticalT2Type.Factory.newInstance();
		TheoreticalT2[] list = new TheoreticalT2[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Theoretical_heteronucl_T2_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:theoretical_t2Category>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalT2', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalT2.Factory.newInstance();
					...
					if (!set_integer(list[0], "setResonanceId", "", true, rset.getString("Resonance_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setRexErr", "setNilRexErr", false, rset.getString("Rex_err"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSeqId", "", true, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setT2Val", "", true, rset.getString("T2_val"), logw))
						continue;
					if (!set_decimal(list[0], "setT2ValErr", "setNilT2ValErr", false, rset.getString("T2_val_err"), logw))
						continue;
					...
					body.setTheoreticalT2Array(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalT2Type.class.getName());
	...
	private static boolean set_string(TheoreticalT2 list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalT2', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalT2', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalT2 list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(TheoreticalT2 list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
