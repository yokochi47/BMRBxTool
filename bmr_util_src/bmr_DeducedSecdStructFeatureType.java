...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DeducedSecdStructFeatureType.*;

public class bmr_DeducedSecdStructFeatureType {

	private static final String table_name = "Deduced_secd_struct_feature";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DeducedSecdStructFeatureType body = DeducedSecdStructFeatureType.Factory.newInstance();
		DeducedSecdStructFeature[] list = new DeducedSecdStructFeature[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:deduced_secd_struct_featureCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DeducedSecdStructFeature', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DeducedSecdStructFeature.Factory.newInstance();
					...
					if (!set_string(list[0], "setAuthSeqId", "setNilAuthSeqId", false, rset.getString("Auth_seq_ID"), logw))
						continue;
					if (!set_string(list[0], "setChemCompStructElementType", "", true, rset.getString("Chem_comp_struct_element_type"), logw))
						continue;
					if (!set_decimal(list[0], "setChemCompStructVal", "", true, rset.getString("Chem_comp_struct_val"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSeqId", "", true, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSpinSystemId", "", true, rset.getString("Spin_system_ID"), logw))
						continue;
					...
					body.setDeducedSecdStructFeatureArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DeducedSecdStructFeatureType.class.getName());
	...
	private static boolean set_string(DeducedSecdStructFeature list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DeducedSecdStructFeature', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DeducedSecdStructFeature', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DeducedSecdStructFeature list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(DeducedSecdStructFeature list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
