...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyInteractionType.*;

public class bmr_AssemblyInteractionType {

	private static final String table_name = "Assembly_interaction";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssemblyInteractionType body = AssemblyInteractionType.Factory.newInstance();
		AssemblyInteraction[] list = new AssemblyInteraction[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:assembly_interactionCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssemblyInteraction', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AssemblyInteraction.Factory.newInstance();
					...
					if (!set_integer(list[0], "setEntityAssemblyId2", "", false, rset.getString("Entity_assembly_ID_2"), logw))
						continue;
					if (!set_enum_mol_interaction_type(list[0], "setMolInteractionType", "", true, rset.getString("Mol_interaction_type"), logw, errw))
						continue;
					...
					body.setAssemblyInteractionArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssemblyInteractionType.class.getName());
	...
	private static boolean set_string(AssemblyInteraction list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssemblyInteraction', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssemblyInteraction', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AssemblyInteraction list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_mol_interaction_type(AssemblyInteraction list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_AssemblyInteraction.getMolInteractionType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ AssemblyInteraction.MolInteractionType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					AssemblyInteraction.MolInteractionType.Enum _enum = AssemblyInteraction.MolInteractionType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:AssemblyInteraction method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= AssemblyInteraction.MolInteractionType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + AssemblyInteraction.MolInteractionType.Enum.forInt(i));
						try {
							errw.write("class_name:AssemblyInteraction method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= AssemblyInteraction.MolInteractionType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + AssemblyInteraction.MolInteractionType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
