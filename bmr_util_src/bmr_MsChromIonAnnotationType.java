...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MsChromIonAnnotationType.*;

public class bmr_MsChromIonAnnotationType {

	private static final String table_name = "MS_chrom_ion_annotation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MsChromIonAnnotationType body = MsChromIonAnnotationType.Factory.newInstance();
		MsChromIonAnnotation[] list = new MsChromIonAnnotation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:ms_chrom_ion_annotationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MsChromIonAnnotation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MsChromIonAnnotation.Factory.newInstance();
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_integer(list[0], "setMsChromatogramIonId", "", true, rset.getString("MS_chromatogram_ion_ID"), logw))
						continue;
					...
					body.setMsChromIonAnnotationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MsChromIonAnnotationType.class.getName());
	...
	private static boolean set_string(MsChromIonAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MsChromIonAnnotation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MsChromIonAnnotation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MsChromIonAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
