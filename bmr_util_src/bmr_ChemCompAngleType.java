...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemCompAngleType.*;

public class bmr_ChemCompAngleType {

	private static final String table_name = "Chem_comp_angle";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemCompAngleType body = ChemCompAngleType.Factory.newInstance();
		ChemCompAngle[] list = new ChemCompAngle[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:chem_comp_angleCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemCompAngle', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemCompAngle.Factory.newInstance();
					...
					body.setChemCompAngleArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemCompAngleType.class.getName());
	...
	private static boolean set_string(ChemCompAngle list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemCompAngle', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemCompAngle', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChemCompAngle list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
