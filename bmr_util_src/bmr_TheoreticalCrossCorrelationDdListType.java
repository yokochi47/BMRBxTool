...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalCrossCorrelationDdListType.*;

public class bmr_TheoreticalCrossCorrelationDdListType {

	private static final String table_name = "Theoretical_cross_correlation_DD_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalCrossCorrelationDdListType body = TheoreticalCrossCorrelationDdListType.Factory.newInstance();
		TheoreticalCrossCorrelationDdList[] list = new TheoreticalCrossCorrelationDdList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:theoretical_cross_correlation_dd_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalCrossCorrelationDdList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalCrossCorrelationDdList.Factory.newInstance();
					...
					if (!set_string(list[0], "setTextDataFormat", "setNilTextDataFormat", false, rset.getString("Text_data_format"), logw))
						continue;
					if (!set_enum_val_units(list[0], "setValUnits", "", true, rset.getString("Val_units"), logw, errw))
						continue;
					...
					body.setTheoreticalCrossCorrelationDdListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalCrossCorrelationDdListType.class.getName());
	...
	private static boolean set_string(TheoreticalCrossCorrelationDdList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalCrossCorrelationDdList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalCrossCorrelationDdList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalCrossCorrelationDdList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(TheoreticalCrossCorrelationDdList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_val_units(TheoreticalCrossCorrelationDdList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ TheoreticalCrossCorrelationDdList.ValUnits.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					TheoreticalCrossCorrelationDdList.ValUnits.Enum _enum = TheoreticalCrossCorrelationDdList.ValUnits.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:TheoreticalCrossCorrelationDdList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= TheoreticalCrossCorrelationDdList.ValUnits.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + TheoreticalCrossCorrelationDdList.ValUnits.Enum.forInt(i));
						try {
							errw.write("class_name:TheoreticalCrossCorrelationDdList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= TheoreticalCrossCorrelationDdList.ValUnits.Enum.table.lastInt(); i++)
								errw.write(" enum:" + TheoreticalCrossCorrelationDdList.ValUnits.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
