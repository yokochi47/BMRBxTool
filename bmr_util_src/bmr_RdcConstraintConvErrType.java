...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.RdcConstraintConvErrType.*;

public class bmr_RdcConstraintConvErrType {

	private static final String table_name = "RDC_constraint_conv_err";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		RdcConstraintConvErrType body = RdcConstraintConvErrType.Factory.newInstance();
		RdcConstraintConvErr[] list = new RdcConstraintConvErr[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:rdc_constraint_conv_errCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='RdcConstraintConvErr', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = RdcConstraintConvErr.Factory.newInstance();
					...
					if (!set_integer(list[0], "setParseFileConstraintId", "", true, rset.getString("Parse_file_constraint_ID"), logw))
						continue;
					if (!set_integer(list[0], "setRdcConstrParseFileId", "", true, rset.getString("RDC_constr_parse_file_ID"), logw))
						continue;
					...
					body.setRdcConstraintConvErrArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_RdcConstraintConvErrType.class.getName());
	...
	private static boolean set_string(RdcConstraintConvErr list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='RdcConstraintConvErr', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='RdcConstraintConvErr', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(RdcConstraintConvErr list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
