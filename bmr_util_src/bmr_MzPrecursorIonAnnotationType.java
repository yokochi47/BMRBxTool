...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MzPrecursorIonAnnotationType.*;

public class bmr_MzPrecursorIonAnnotationType {

	private static final String table_name = "MZ_precursor_ion_annotation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MzPrecursorIonAnnotationType body = MzPrecursorIonAnnotationType.Factory.newInstance();
		MzPrecursorIonAnnotation[] list = new MzPrecursorIonAnnotation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"MZ_ratio_data_list_ID\")::decimal,(0 || \"MZ_precursor_ion_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:mz_precursor_ion_annotationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MzPrecursorIonAnnotation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MzPrecursorIonAnnotation.Factory.newInstance();

					if (!set_string(list[0], "setChemicalComposition", "setNilChemicalComposition", false, rset.getString("Chemical_composition"), logw))
						continue;
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setMzPrecursorIonId", "", true, rset.getString("MZ_precursor_ion_ID"), logw))
						continue;
					if (!set_integer(list[0], "setMzRatioDataListId", "", true, rset.getString("MZ_ratio_data_list_ID"), logw))
						continue;

					body.setMzPrecursorIonAnnotationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MzPrecursorIonAnnotationType.class.getName());
	...
	private static boolean set_string(MzPrecursorIonAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MzPrecursorIonAnnotation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MzPrecursorIonAnnotation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MzPrecursorIonAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
