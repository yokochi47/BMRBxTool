...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HExchProtectionFactorListType.*;

public class bmr_HExchProtectionFactorListType {

	private static final String table_name = "H_exch_protection_factor_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HExchProtectionFactorListType body = HExchProtectionFactorListType.Factory.newInstance();
		HExchProtectionFactorList[] list = new HExchProtectionFactorList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:h_exch_protection_factor_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HExchProtectionFactorList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HExchProtectionFactorList.Factory.newInstance();
					...
					if (!set_string(list[0], "setSfFramecode", "", true, rset.getString("Sf_framecode"), logw))
						continue;
					if (!set_integer(list[0], "setStdValuesSourceCitId", "", false, rset.getString("Std_values_source_cit_ID"), logw))
						continue;
					if (!set_string(list[0], "setStdValuesSourceCitLabel", "setNilStdValuesSourceCitLabel", false, rset.getString("Std_values_source_cit_label"), logw))
						continue;
					...
					body.setHExchProtectionFactorListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HExchProtectionFactorListType.class.getName());
	...
	private static boolean set_string(HExchProtectionFactorList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HExchProtectionFactorList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HExchProtectionFactorList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HExchProtectionFactorList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
