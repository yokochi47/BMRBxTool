...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.CrossCorrelationDdListType.*;

public class bmr_CrossCorrelationDdListType {

	private static final String table_name = "Cross_correlation_DD_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		CrossCorrelationDdListType body = CrossCorrelationDdListType.Factory.newInstance();
		CrossCorrelationDdList[] list = new CrossCorrelationDdList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:cross_correlation_dd_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='CrossCorrelationDdList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = CrossCorrelationDdList.Factory.newInstance();
					...
					body.setCrossCorrelationDdListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_CrossCorrelationDdListType.class.getName());
	...
	private static boolean set_string(CrossCorrelationDdList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='CrossCorrelationDdList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='CrossCorrelationDdList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(CrossCorrelationDdList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(CrossCorrelationDdList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_val_units(CrossCorrelationDdList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ CrossCorrelationDdList.ValUnits.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					CrossCorrelationDdList.ValUnits.Enum _enum = CrossCorrelationDdList.ValUnits.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:CrossCorrelationDdList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= CrossCorrelationDdList.ValUnits.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + CrossCorrelationDdList.ValUnits.Enum.forInt(i));
						try {
							errw.write("class_name:CrossCorrelationDdList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= CrossCorrelationDdList.ValUnits.Enum.table.lastInt(); i++)
								errw.write(" enum:" + CrossCorrelationDdList.ValUnits.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
