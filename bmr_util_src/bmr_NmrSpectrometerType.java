...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.NmrSpectrometerType.*;

public class bmr_NmrSpectrometerType {

	private static final String table_name = "NMR_spectrometer";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		NmrSpectrometerType body = NmrSpectrometerType.Factory.newInstance();
		NmrSpectrometer[] list = new NmrSpectrometer[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:nmr_spectrometerCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='NmrSpectrometer', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = NmrSpectrometer.Factory.newInstance();
					...
					body.setNmrSpectrometerArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_NmrSpectrometerType.class.getName());
	...
	private static boolean set_string(NmrSpectrometer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='NmrSpectrometer', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='NmrSpectrometer', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(NmrSpectrometer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_field_strength(NmrSpectrometer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_NMRSpectrometer.getFieldStrength(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrSpectrometer.FieldStrength.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrSpectrometer.FieldStrength.Enum _enum = NmrSpectrometer.FieldStrength.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrSpectrometer method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrSpectrometer.FieldStrength.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrSpectrometer.FieldStrength.Enum.forInt(i));
						try {
							errw.write("class_name:NmrSpectrometer method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrSpectrometer.FieldStrength.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrSpectrometer.FieldStrength.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_manufacturer(NmrSpectrometer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_NMRSpectrometer.getManufacturer(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrSpectrometer.Manufacturer.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrSpectrometer.Manufacturer.Enum _enum = NmrSpectrometer.Manufacturer.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrSpectrometer method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrSpectrometer.Manufacturer.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrSpectrometer.Manufacturer.Enum.forInt(i));
						try {
							errw.write("class_name:NmrSpectrometer method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrSpectrometer.Manufacturer.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrSpectrometer.Manufacturer.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
