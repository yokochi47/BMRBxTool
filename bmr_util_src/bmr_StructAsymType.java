...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.StructAsymType.*;

public class bmr_StructAsymType {

	private static final String table_name = "Struct_asym";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		StructAsymType body = StructAsymType.Factory.newInstance();
		StructAsym[] list = new StructAsym[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:struct_asymCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='StructAsym', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = StructAsym.Factory.newInstance();
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_enum_pdbx_blank_pdb_chainid_flag(list[0], "setPdbxBlankPdbChainidFlag", "setNilPdbxBlankPdbChainidFlag", false, rset.getString("PDBX_blank_PDB_chainid_flag"), logw, errw))
						continue;
					if (!set_enum_pdbx_modified(list[0], "setPdbxModified", "setNilPdbxModified", false, rset.getString("PDBX_modified"), logw, errw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_StructAsymType.class.getName());
	...
	private static boolean set_string(StructAsym list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='StructAsym', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='StructAsym', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(StructAsym list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_pdbx_blank_pdb_chainid_flag(StructAsym list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ StructAsym.PdbxBlankPdbChainidFlag.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					StructAsym.PdbxBlankPdbChainidFlag.Enum _enum = StructAsym.PdbxBlankPdbChainidFlag.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:StructAsym method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= StructAsym.PdbxBlankPdbChainidFlag.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + StructAsym.PdbxBlankPdbChainidFlag.Enum.forInt(i));
						try {
							errw.write("class_name:StructAsym method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= StructAsym.PdbxBlankPdbChainidFlag.Enum.table.lastInt(); i++)
								errw.write(" enum:" + StructAsym.PdbxBlankPdbChainidFlag.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_pdbx_modified(StructAsym list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ StructAsym.PdbxModified.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					StructAsym.PdbxModified.Enum _enum = StructAsym.PdbxModified.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:StructAsym method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= StructAsym.PdbxModified.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + StructAsym.PdbxModified.Enum.forInt(i));
						try {
							errw.write("class_name:StructAsym method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= StructAsym.PdbxModified.Enum.table.lastInt(); i++)
								errw.write(" enum:" + StructAsym.PdbxModified.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
