...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ResonanceLinkerListType.*;

public class bmr_ResonanceLinkerListType {

	private static final String table_name = "Resonance_linker_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ResonanceLinkerListType body = ResonanceLinkerListType.Factory.newInstance();
		ResonanceLinkerList[] list = new ResonanceLinkerList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:resonance_linker_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ResonanceLinkerList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ResonanceLinkerList.Factory.newInstance();
					...
					body.setResonanceLinkerListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ResonanceLinkerListType.class.getName());
	...
	private static boolean set_string(ResonanceLinkerList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ResonanceLinkerList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ResonanceLinkerList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ResonanceLinkerList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
