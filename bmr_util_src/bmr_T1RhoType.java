...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.T1RhoType.*;

public class bmr_T1RhoType {

	private static final String table_name = "T1rho";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		T1RhoType body = T1RhoType.Factory.newInstance();
		T1Rho[] list = new T1Rho[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Heteronucl_T1rho_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:t1rhoCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='T1Rho', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = T1Rho.Factory.newInstance();
					...
					if (!set_integer(list[0], "setSeqId", "", true, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setT1RhoVal", "", true, rset.getString("T1rho_val"), logw))
						continue;
					if (!set_decimal(list[0], "setT1RhoValErr", "setNilT1RhoValErr", false, rset.getString("T1rho_val_err"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_T1RhoType.class.getName());
	...
	private static boolean set_string(T1Rho list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='T1Rho', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='T1Rho', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(T1Rho list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(T1Rho list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
