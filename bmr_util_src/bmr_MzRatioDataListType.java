...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MzRatioDataListType.*;

public class bmr_MzRatioDataListType {

	private static final String table_name = "MZ_ratio_data_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MzRatioDataListType body = MzRatioDataListType.Factory.newInstance();
		MzRatioDataList[] list = new MzRatioDataList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:mz_ratio_data_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MzRatioDataList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MzRatioDataList.Factory.newInstance();
					...
					if (!set_string(list[0], "setErrorDerivationMethod", "setNilErrorDerivationMethod", false, rset.getString("Error_derivation_method"), logw))
						continue;
					if (!set_string(list[0], "setIntensityUnits", "setNilIntensityUnits", false, rset.getString("Intensity_units"), logw))
						continue;
					if (!set_integer(list[0], "setMassSpecRefCompdId", "", true, rset.getString("Mass_spec_ref_compd_ID"), logw))
						continue;
					if (!set_integer(list[0], "setMassSpecRefCompdSetId", "", true, rset.getString("Mass_spec_ref_compd_set_ID"), logw))
						continue;
					if (!set_string(list[0], "setMassSpecRefCompdSetLabel", "setNilMassSpecRefCompdSetLabel", false, rset.getString("Mass_spec_ref_compd_set_label"), logw))
						continue;
					if (!set_string(list[0], "setMzErrorUnits", "setNilMzErrorUnits", false, rset.getString("MZ_error_units"), logw))
						continue;
					...
					body.setMzRatioDataListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MzRatioDataListType.class.getName());
	...
	private static boolean set_string(MzRatioDataList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MzRatioDataList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MzRatioDataList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MzRatioDataList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
