...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.NmrExptSystematicNameType.*;

public class bmr_NmrExptSystematicNameType {

	private static final String table_name = "NMR_expt_systematic_name";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		NmrExptSystematicNameType body = NmrExptSystematicNameType.Factory.newInstance();
		NmrExptSystematicName[] list = new NmrExptSystematicName[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:nmr_expt_systematic_nameCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='NmrExptSystematicName', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = NmrExptSystematicName.Factory.newInstance();
					...
					body.setNmrExptSystematicNameArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_NmrExptSystematicNameType.class.getName());
	...
	private static boolean set_string(NmrExptSystematicName list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='NmrExptSystematicName', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='NmrExptSystematicName', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(NmrExptSystematicName list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_naming_system(NmrExptSystematicName list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrExptSystematicName.NamingSystem.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrExptSystematicName.NamingSystem.Enum _enum = NmrExptSystematicName.NamingSystem.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrExptSystematicName method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrExptSystematicName.NamingSystem.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrExptSystematicName.NamingSystem.Enum.forInt(i));
						try {
							errw.write("class_name:NmrExptSystematicName method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrExptSystematicName.NamingSystem.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrExptSystematicName.NamingSystem.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
