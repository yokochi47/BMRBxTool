...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.FloatingChiralityAssignType.*;

public class bmr_FloatingChiralityAssignType {

	private static final String table_name = "Floating_chirality_assign";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		FloatingChiralityAssignType body = FloatingChiralityAssignType.Factory.newInstance();
		FloatingChiralityAssign[] list = new FloatingChiralityAssign[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:floating_chirality_assignCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='FloatingChiralityAssign', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = FloatingChiralityAssign.Factory.newInstance();
					...
					if (!set_string(list[0], "setSfFramecode", "", true, rset.getString("Sf_framecode"), logw))
						continue;
					if (!set_integer(list[0], "setStereoAssignedCount", "", true, rset.getString("Stereo_assigned_count"), logw))
						continue;
					if (!set_integer(list[0], "setStereoCount", "", true, rset.getString("Stereo_count"), logw))
						continue;
					...
					body.setFloatingChiralityAssignArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_FloatingChiralityAssignType.class.getName());
	...
	private static boolean set_string(FloatingChiralityAssign list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='FloatingChiralityAssign', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='FloatingChiralityAssign', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(FloatingChiralityAssign list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
