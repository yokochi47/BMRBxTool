...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemCompBondType.*;

public class bmr_ChemCompBondType {

	private static final String table_name = "Chem_comp_bond";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemCompBondType body = ChemCompBondType.Factory.newInstance();
		ChemCompBond[] list = new ChemCompBond[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:chem_comp_bondCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemCompBond', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemCompBond.Factory.newInstance();
					...
					if (!set_enum_type(list[0], "setType", "setNilType", false, rset.getString("Type"), logw, errw))
						continue;
					if (!set_enum_value_order(list[0], "setValueOrder", "", true, rset.getString("Value_order"), logw, errw))
						continue;
					...
					body.setChemCompBondArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemCompBondType.class.getName());
	...
	private static boolean set_string(ChemCompBond list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemCompBond', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemCompBond', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChemCompBond list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_aromatic_flag(ChemCompBond list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_ChemCompBond.getAromaticFlag(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemCompBond.AromaticFlag.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemCompBond.AromaticFlag.Enum _enum = ChemCompBond.AromaticFlag.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemCompBond method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemCompBond.AromaticFlag.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemCompBond.AromaticFlag.Enum.forInt(i));
						try {
							errw.write("class_name:ChemCompBond method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemCompBond.AromaticFlag.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemCompBond.AromaticFlag.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_stereo_config(ChemCompBond list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_ChemCompBond.getStereoConfig(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemCompBond.StereoConfig.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemCompBond.StereoConfig.Enum _enum = ChemCompBond.StereoConfig.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemCompBond method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemCompBond.StereoConfig.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemCompBond.StereoConfig.Enum.forInt(i));
						try {
							errw.write("class_name:ChemCompBond method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemCompBond.StereoConfig.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemCompBond.StereoConfig.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_type(ChemCompBond list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_ChemCompBond.getType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemCompBond.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemCompBond.Type.Enum _enum = ChemCompBond.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemCompBond method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemCompBond.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemCompBond.Type.Enum.forInt(i));
						try {
							errw.write("class_name:ChemCompBond method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemCompBond.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemCompBond.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_value_order(ChemCompBond list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_ChemCompBond.getValueOrder(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemCompBond.ValueOrder.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemCompBond.ValueOrder.Enum _enum = ChemCompBond.ValueOrder.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemCompBond method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemCompBond.ValueOrder.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemCompBond.ValueOrder.Enum.forInt(i));
						try {
							errw.write("class_name:ChemCompBond method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemCompBond.ValueOrder.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemCompBond.ValueOrder.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
