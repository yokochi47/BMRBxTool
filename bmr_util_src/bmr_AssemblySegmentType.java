...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblySegmentType.*;

public class bmr_AssemblySegmentType {

	private static final String table_name = "Assembly_segment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssemblySegmentType body = AssemblySegmentType.Factory.newInstance();
		AssemblySegment[] list = new AssemblySegment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:assembly_segmentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssemblySegment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AssemblySegment.Factory.newInstance();
					...
					if (!set_integer(list[0], "setCompIndexId", "", true, rset.getString("Comp_index_ID"), logw))
						continue;
					if (!set_string(list[0], "setCompLabel", "setNilCompLabel", false, rset.getString("Comp_label"), logw))
						continue;
					...
					body.setAssemblySegmentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssemblySegmentType.class.getName());
	...
	private static boolean set_string(AssemblySegment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssemblySegment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssemblySegment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AssemblySegment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
