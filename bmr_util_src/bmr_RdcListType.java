...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.RdcListType.*;

public class bmr_RdcListType {

	private static final String table_name = "RDC_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		RdcListType body = RdcListType.Factory.newInstance();
		RdcList[] list = new RdcList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:rdc_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='RdcList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = RdcList.Factory.newInstance();
					...
					if (!set_decimal(list[0], "setAssumedHNBondLength", "setNilAssumedHNBondLength", false, rset.getString("Assumed_H_N_bond_length"), logw))
						continue;
					if (!set_enum_bond_length_usage_flag(list[0], "setBondLengthUsageFlag", "setNilBondLengthUsageFlag", false, rset.getString("Bond_length_usage_flag"), logw, errw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_RdcListType.class.getName());
	...
	private static boolean set_string(RdcList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='RdcList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='RdcList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(RdcList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_sample_condition_list_id(RdcList list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, FileWriter logw) {
	...
	private static boolean set_decimal(RdcList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_bond_length_usage_flag(RdcList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ RdcList.BondLengthUsageFlag.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					RdcList.BondLengthUsageFlag.Enum _enum = RdcList.BondLengthUsageFlag.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:RdcList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= RdcList.BondLengthUsageFlag.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + RdcList.BondLengthUsageFlag.Enum.forInt(i));
						try {
							errw.write("class_name:RdcList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= RdcList.BondLengthUsageFlag.Enum.table.lastInt(); i++)
								errw.write(" enum:" + RdcList.BondLengthUsageFlag.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
