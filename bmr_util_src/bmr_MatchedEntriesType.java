...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MatchedEntriesType.*;

public class bmr_MatchedEntriesType {

	private static final String table_name = "Matched_entries";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MatchedEntriesType body = MatchedEntriesType.Factory.newInstance();
		MatchedEntries[] list = new MatchedEntries[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:matched_entriesCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MatchedEntries', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MatchedEntries.Factory.newInstance();

					if (!set_string(list[0], "setDatabaseAccessionCode", "", true, rset.getString("Database_accession_code"), logw))
						continue;
					if (!set_enum_database_name(list[0], "setDatabaseName", "", true, rset.getString("Database_name"), logw, errw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;

					body.setMatchedEntriesArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MatchedEntriesType.class.getName());
	...
	private static boolean set_string(MatchedEntries list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MatchedEntries', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MatchedEntries', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_enum_database_name(MatchedEntries list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ MatchedEntries.DatabaseName.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					MatchedEntries.DatabaseName.Enum _enum = MatchedEntries.DatabaseName.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:MatchedEntries method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= MatchedEntries.DatabaseName.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + MatchedEntries.DatabaseName.Enum.forInt(i));
						try {
							errw.write("class_name:MatchedEntries method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= MatchedEntries.DatabaseName.Enum.table.lastInt(); i++)
								errw.write(" enum:" + MatchedEntries.DatabaseName.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
