...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EntryExperimentalMethodsType.*;

public class bmr_EntryExperimentalMethodsType {

	private static final String table_name = "Entry_experimental_methods";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EntryExperimentalMethodsType body = EntryExperimentalMethodsType.Factory.newInstance();
		EntryExperimentalMethods[] list = new EntryExperimentalMethods[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:entry_experimental_methodsCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EntryExperimentalMethods', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EntryExperimentalMethods.Factory.newInstance();

					if (!set_enum_method(list[0], "setMethod", "", true, rset.getString("Method"), logw, errw))
						continue;
					if (!set_enum_subtype(list[0], "setSubtype", "setNilSubtype", false, rset.getString("Subtype"), logw, errw))
						continue;
					...
					body.setEntryExperimentalMethodsArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_EntryExperimentalMethodsType.class.getName());
	...
	private static boolean set_string(EntryExperimentalMethods list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EntryExperimentalMethods', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EntryExperimentalMethods', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EntryExperimentalMethods list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_method(EntryExperimentalMethods list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_EntryExperimentalMethods.getMethod(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ EntryExperimentalMethods.Method.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					EntryExperimentalMethods.Method.Enum _enum = EntryExperimentalMethods.Method.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:EntryExperimentalMethods method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= EntryExperimentalMethods.Method.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + EntryExperimentalMethods.Method.Enum.forInt(i));
						try {
							errw.write("class_name:EntryExperimentalMethods method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= EntryExperimentalMethods.Method.Enum.table.lastInt(); i++)
								errw.write(" enum:" + EntryExperimentalMethods.Method.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_subtype(EntryExperimentalMethods list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_EntryExperimentalMethods.getSubtype(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ EntryExperimentalMethods.Subtype.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					EntryExperimentalMethods.Subtype.Enum _enum = EntryExperimentalMethods.Subtype.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:EntryExperimentalMethods method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= EntryExperimentalMethods.Subtype.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + EntryExperimentalMethods.Subtype.Enum.forInt(i));
						try {
							errw.write("class_name:EntryExperimentalMethods method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= EntryExperimentalMethods.Subtype.Enum.table.lastInt(); i++)
								errw.write(" enum:" + EntryExperimentalMethods.Subtype.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
