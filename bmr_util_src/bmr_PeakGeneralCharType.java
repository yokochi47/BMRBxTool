...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.PeakGeneralCharType.*;

public class bmr_PeakGeneralCharType {

	private static final String table_name = "Peak_general_char";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		PeakGeneralCharType body = PeakGeneralCharType.Factory.newInstance();
		PeakGeneralChar[] list = new PeakGeneralChar[1];
		...
		try {
			...
			if (rset.next() && bmr_Util_Main.remediate_xml && !entry_id.equals("6693")) {
				...
				buffw.write("  <BMRBx:peak_general_charCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='PeakGeneralChar', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = PeakGeneralChar.Factory.newInstance();
					...
					body.setPeakGeneralCharArray(list);
			...
			} else if (bmr_Util_Main.remediate_xml && entry_id.equals("6693")) {

						try {
							logw.write("category='PeakGeneralChar', all information content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
			...
			Logger lgr = Logger.getLogger(bmr_PeakGeneralCharType.class.getName());
	...
	private static boolean set_string(PeakGeneralChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='PeakGeneralChar', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PeakGeneralChar', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(PeakGeneralChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(PeakGeneralChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_measurement_method(PeakGeneralChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ PeakGeneralChar.MeasurementMethod.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					PeakGeneralChar.MeasurementMethod.Enum _enum = PeakGeneralChar.MeasurementMethod.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:PeakGeneralChar method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= PeakGeneralChar.MeasurementMethod.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + PeakGeneralChar.MeasurementMethod.Enum.forInt(i));
						try {
							errw.write("class_name:PeakGeneralChar method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= PeakGeneralChar.MeasurementMethod.Enum.table.lastInt(); i++)
								errw.write(" enum:" + PeakGeneralChar.MeasurementMethod.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
