...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalCrossCorrelationDdType.*;

public class bmr_TheoreticalCrossCorrelationDdType {

	private static final String table_name = "Theoretical_cross_correlation_DD";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalCrossCorrelationDdType body = TheoreticalCrossCorrelationDdType.Factory.newInstance();
		TheoreticalCrossCorrelationDd[] list = new TheoreticalCrossCorrelationDd[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Theoretical_cross_correlation_DD_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:theoretical_cross_correlation_ddCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalCrossCorrelationDd', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalCrossCorrelationDd.Factory.newInstance();

					if (!set_integer(list[0], "setDipole1AssemblyAtomId1", "", true, rset.getString("Dipole_1_assembly_atom_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setDipole1AssemblyAtomId2", "", true, rset.getString("Dipole_1_assembly_atom_ID_2"), logw))
						continue;
					...
					if (!set_integer(list[0], "setDipole1EntityId2", "", true, rset.getString("Dipole_1_entity_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setDipole1ResonanceId1", "", true, rset.getString("Dipole_1_Resonance_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setDipole1ResonanceId2", "", true, rset.getString("Dipole_1_Resonance_ID_2"), logw))
						continue;
					...
					if (!set_integer(list[0], "setDipole1SeqId2", "", true, rset.getString("Dipole_1_seq_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2AssemblyAtomId1", "", true, rset.getString("Dipole_2_assembly_atom_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2AssemblyAtomId2", "", true, rset.getString("Dipole_2_assembly_atom_ID_2"), logw))
						continue;
					...
					if (!set_string(list[0], "setDipole2AuthSeqId2", "setNilDipole2AuthSeqId2", false, rset.getString("Dipole_2_auth_seq_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2ChemCompIndexId2", "", true, rset.getString("Dipole_2_chem_comp_index_ID_2"), logw))
						continue;
					...
					if (!set_string(list[0], "setDipole2CompId2", "", true, rset.getString("Dipole_2_comp_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2CompIndexId1", "", true, rset.getString("Dipole_2_comp_index_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2EntityAssemblyId1", "", true, rset.getString("Dipole_2_entity_assembly_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2EntityAssemblyId2", "", true, rset.getString("Dipole_2_entity_assembly_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2EntityId1", "", true, rset.getString("Dipole_2_entity_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2EntityId2", "", true, rset.getString("Dipole_2_entity_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2ResonanceId1", "", true, rset.getString("Dipole_2_Resonance_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2ResonanceId2", "", true, rset.getString("Dipole_2_Resonance_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2SeqId1", "", true, rset.getString("Dipole_2_seq_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setDipole2SeqId2", "", true, rset.getString("Dipole_2_seq_ID_2"), logw))
						continue;
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setTheoreticalCrossCorrelationDdListId", "", true, rset.getString("Theoretical_cross_correlation_DD_list_ID"), logw))
						continue;

					body.setTheoreticalCrossCorrelationDdArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalCrossCorrelationDdType.class.getName());
	...
	private static boolean set_string(TheoreticalCrossCorrelationDd list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalCrossCorrelationDd', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalCrossCorrelationDd', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalCrossCorrelationDd list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(TheoreticalCrossCorrelationDd list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
