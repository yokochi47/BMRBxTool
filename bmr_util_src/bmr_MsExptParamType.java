...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MsExptParamType.*;

public class bmr_MsExptParamType {

	private static final String table_name = "MS_expt_param";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MsExptParamType body = MsExptParamType.Factory.newInstance();
		MsExptParam[] list = new MsExptParam[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"MS_expt_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:ms_expt_paramCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MsExptParam', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MsExptParam.Factory.newInstance();

					if (!set_enum_name(list[0], "setName", "setNilName", false, rset.getString("Name"), logw, errw))
						continue;
					...
					if (!set_string(list[0], "setValue", "setNilValue", false, rset.getString("Value"), logw))
						continue;
					if (!set_decimal(list[0], "setValueErr", "setNilValueErr", false, rset.getString("Value_err"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setMsExptId", "", true, rset.getString("MS_expt_ID"), logw))
						continue;

					body.setMsExptParamArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MsExptParamType.class.getName());
	...
	private static boolean set_string(MsExptParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MsExptParam', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MsExptParam', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MsExptParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(MsExptParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_name(MsExptParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ MsExptParam.Name.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					MsExptParam.Name.Enum _enum = MsExptParam.Name.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:MsExptParam method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= MsExptParam.Name.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + MsExptParam.Name.Enum.forInt(i));
						try {
							errw.write("class_name:MsExptParam method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= MsExptParam.Name.Enum.table.lastInt(); i++)
								errw.write(" enum:" + MsExptParam.Name.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
