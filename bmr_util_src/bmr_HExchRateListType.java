...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HExchRateListType.*;

public class bmr_HExchRateListType {

	private static final String table_name = "H_exch_rate_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HExchRateListType body = HExchRateListType.Factory.newInstance();
		HExchRateList[] list = new HExchRateList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:h_exch_rate_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HExchRateList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HExchRateList.Factory.newInstance();
					...
					body.setHExchRateListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HExchRateListType.class.getName());
	...
	private static boolean set_string(HExchRateList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HExchRateList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HExchRateList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HExchRateList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_val_units(HExchRateList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_HExchRateList.getValUnits(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ HExchRateList.ValUnits.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					HExchRateList.ValUnits.Enum _enum = HExchRateList.ValUnits.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:HExchRateList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= HExchRateList.ValUnits.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + HExchRateList.ValUnits.Enum.forInt(i));
						try {
							errw.write("class_name:HExchRateList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= HExchRateList.ValUnits.Enum.table.lastInt(); i++)
								errw.write(" enum:" + HExchRateList.ValUnits.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
