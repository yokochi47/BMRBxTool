...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.PeakCharType.*;

public class bmr_PeakCharType {

	private static final String table_name = "Peak_char";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		PeakCharType body = PeakCharType.Factory.newInstance();
		PeakChar[] list = new PeakChar[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:peak_charCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='PeakChar', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = PeakChar.Factory.newInstance();

					if (!set_decimal(list[0], "setBoundingBoxLowerVal", "setNilBoundingBoxLowerVal", false, rset.getString("Bounding_box_lower_val"), logw))
						continue;
					if (!set_decimal(list[0], "setBoundingBoxRangeVal", "setNilBoundingBoxRangeVal", false, rset.getString("Bounding_box_range_val"), logw))
						continue;
					if (!set_decimal(list[0], "setBoundingBoxUpperVal", "setNilBoundingBoxUpperVal", false, rset.getString("Bounding_box_upper_val"), logw))
						continue;
					...
					if (!set_decimal(list[0], "setChemShiftValErr", "setNilChemShiftValErr", false, rset.getString("Chem_shift_val_err"), logw))
						continue;
					if (!set_enum_coupling_pattern(list[0], "setCouplingPattern", "setNilCouplingPattern", false, rset.getString("Coupling_pattern"), logw, errw))
						continue;
					if (!set_decimal(list[0], "setDecayRateVal", "setNilDecayRateVal", false, rset.getString("Decay_rate_val"), logw))
						continue;
					if (!set_decimal(list[0], "setDecayRateValErr", "setNilDecayRateValErr", false, rset.getString("Decay_rate_val_err"), logw))
						continue;
					if (!set_integer(list[0], "setDerivationMethodId", "", false, rset.getString("Derivation_method_ID"), logw))
						continue;
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_decimal(list[0], "setLineWidthVal", "setNilLineWidthVal", false, rset.getString("Line_width_val"), logw))
						continue;
					if (!set_decimal(list[0], "setLineWidthValErr", "setNilLineWidthValErr", false, rset.getString("Line_width_val_err"), logw))
						continue;
					if (!set_string(list[0], "setPhaseVal", "setNilPhaseVal", false, rset.getString("Phase_val"), logw))
						continue;
					if (!set_decimal(list[0], "setPhaseValErr", "setNilPhaseValErr", false, rset.getString("Phase_val_err"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_PeakCharType.class.getName());
	...
	private static boolean set_string(PeakChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='PeakChar', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PeakChar', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(PeakChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(PeakChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_coupling_pattern(PeakChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ PeakChar.CouplingPattern.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					PeakChar.CouplingPattern.Enum _enum = PeakChar.CouplingPattern.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:PeakChar method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= PeakChar.CouplingPattern.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + PeakChar.CouplingPattern.Enum.forInt(i));
						try {
							errw.write("class_name:PeakChar method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= PeakChar.CouplingPattern.Enum.table.lastInt(); i++)
								errw.write(" enum:" + PeakChar.CouplingPattern.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
