...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.OrgConstrFileCommentType.*;

public class bmr_OrgConstrFileCommentType {

	private static final String table_name = "Org_constr_file_comment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		OrgConstrFileCommentType body = OrgConstrFileCommentType.Factory.newInstance();
		OrgConstrFileComment[] list = new OrgConstrFileComment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:org_constr_file_commentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='OrgConstrFileComment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = OrgConstrFileComment.Factory.newInstance();

					if (!set_integer(list[0], "setBlockId", "setNilBlockId", false, rset.getString("Block_ID"), logw))
						continue;
					if (!set_string(list[0], "setComment", "setNilComment", false, rset.getString("Comment"), logw))
						continue;
					...
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setSfCategory", "setNilSfCategory", false, rset.getString("Sf_category"), logw))
						continue;
					...
					body.setOrgConstrFileCommentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_OrgConstrFileCommentType.class.getName());
	...
	private static boolean set_string(OrgConstrFileComment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='OrgConstrFileComment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='OrgConstrFileComment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(OrgConstrFileComment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
