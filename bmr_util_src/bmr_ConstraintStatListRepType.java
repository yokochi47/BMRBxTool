...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ConstraintStatListRepType.*;

public class bmr_ConstraintStatListRepType {

	private static final String table_name = "Constraint_stat_list_rep";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ConstraintStatListRepType body = ConstraintStatListRepType.Factory.newInstance();
		ConstraintStatListRep[] list = new ConstraintStatListRep[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:constraint_stat_list_repCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ConstraintStatListRep', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ConstraintStatListRep.Factory.newInstance();
					...
					if (!set_decimal(list[0], "setDipolar1H1HRmsdErr", "setNilDipolar1H1HRmsdErr", false, rset.getString("Dipolar_1H_1H_rmsd_err"), logw))
						continue;
					if (!set_decimal(list[0], "setDistConstraintViolMax", "setNilDistConstraintViolMax", false, rset.getString("Dist_constraint_viol_max"), logw))
						continue;
					...
					body.setConstraintStatListRepArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ConstraintStatListRepType.class.getName());
	...
	private static boolean set_string(ConstraintStatListRep list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ConstraintStatListRep', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ConstraintStatListRep', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ConstraintStatListRep list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(ConstraintStatListRep list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_constraint_stats_not_available(ConstraintStatListRep list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ConstraintStatListRep.ConstraintStatsNotAvailable.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ConstraintStatListRep.ConstraintStatsNotAvailable.Enum _enum = ConstraintStatListRep.ConstraintStatsNotAvailable.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ConstraintStatListRep method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ConstraintStatListRep.ConstraintStatsNotAvailable.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ConstraintStatListRep.ConstraintStatsNotAvailable.Enum.forInt(i));
						try {
							errw.write("class_name:ConstraintStatListRep method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ConstraintStatListRep.ConstraintStatsNotAvailable.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ConstraintStatListRep.ConstraintStatsNotAvailable.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
