...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DistanceConstraintSoftwareType.*;

public class bmr_DistanceConstraintSoftwareType {

	private static final String table_name = "Distance_constraint_software";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DistanceConstraintSoftwareType body = DistanceConstraintSoftwareType.Factory.newInstance();
		DistanceConstraintSoftware[] list = new DistanceConstraintSoftware[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:distance_constraint_softwareCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DistanceConstraintSoftware', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DistanceConstraintSoftware.Factory.newInstance();
					...
					body.setDistanceConstraintSoftwareArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DistanceConstraintSoftwareType.class.getName());
	...
	private static boolean set_string(DistanceConstraintSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DistanceConstraintSoftware', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DistanceConstraintSoftware', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DistanceConstraintSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_software_id(DistanceConstraintSoftware list, String method_name, String nil_method_name, boolean required, String val_name, int id, FileWriter logw) {
