...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemicalRateExperimentType.*;

public class bmr_ChemicalRateExperimentType {

	private static final String table_name = "Chemical_rate_experiment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemicalRateExperimentType body = ChemicalRateExperimentType.Factory.newInstance();
		ChemicalRateExperiment[] list = new ChemicalRateExperiment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Chemical_rate_list_ID\")::decimal,(0 || \"Experiment_ID\")::decimal");
				...
				buffw.write("  <BMRBx:chemical_rate_experimentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemicalRateExperiment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemicalRateExperiment.Factory.newInstance();
					...
					if (!set_enum_sample_state(list[0], "setSampleState", "setNilSampleState", false, rset.getString("Sample_state"), logw, errw))
						continue;
					if (!set_integer(list[0], "setChemicalRateListId", "", true, rset.getString("Chemical_rate_list_ID"), logw))
						continue;
					...
					body.setChemicalRateExperimentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemicalRateExperimentType.class.getName());
	...
	private static boolean set_string(ChemicalRateExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemicalRateExperiment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemicalRateExperiment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChemicalRateExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_sample_state(ChemicalRateExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemicalRateExperiment.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemicalRateExperiment.SampleState.Enum _enum = ChemicalRateExperiment.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemicalRateExperiment method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemicalRateExperiment.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemicalRateExperiment.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:ChemicalRateExperiment method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemicalRateExperiment.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemicalRateExperiment.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
