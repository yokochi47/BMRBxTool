...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemShiftPerturbationListType.*;

public class bmr_ChemShiftPerturbationListType {

	private static final String table_name = "Chem_shift_perturbation_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemShiftPerturbationListType body = ChemShiftPerturbationListType.Factory.newInstance();
		ChemShiftPerturbationList[] list = new ChemShiftPerturbationList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:chem_shift_perturbation_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemShiftPerturbationList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemShiftPerturbationList.Factory.newInstance();

					if (!set_integer(list[0], "setChemShiftRefSetId", "", true, rset.getString("Chem_shift_ref_set_ID"), logw))
						continue;
					if (!set_string(list[0], "setChemShiftRefSetLabel", "setNilChemShiftRefSetLabel", false, rset.getString("Chem_shift_ref_set_label"), logw))
						continue;
					...
					body.setChemShiftPerturbationListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemShiftPerturbationListType.class.getName());
	...
	private static boolean set_string(ChemShiftPerturbationList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemShiftPerturbationList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemShiftPerturbationList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChemShiftPerturbationList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(ChemShiftPerturbationList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemShiftPerturbationList.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemShiftPerturbationList.Type.Enum _enum = ChemShiftPerturbationList.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemShiftPerturbationList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemShiftPerturbationList.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemShiftPerturbationList.Type.Enum.forInt(i));
						try {
							errw.write("class_name:ChemShiftPerturbationList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemShiftPerturbationList.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemShiftPerturbationList.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
