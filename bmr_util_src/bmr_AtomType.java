...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomType.*;

public class bmr_AtomType {

	private static final String table_name = "Atom";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AtomType body = AtomType.Factory.newInstance();
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Assembly_atom_ID\")::decimal");
				...
				buffw.write("  <BMRBx:atomCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='Atom', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = Atom.Factory.newInstance();

					if (!set_string(list[0], "setAtomId", "", true, rset.getString("Atom_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAtomTypeId", "", false, rset.getString("Atom_type_ID"), logw))
						continue;
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_string(list[0], "setPdbAsymId", "setNilPdbAsymId", false, rset.getString("PDB_asym_ID"), logw))
						continue;
					if (!set_string(list[0], "setPdbAtomId", "setNilPdbAtomId", false, rset.getString("PDB_atom_ID"), logw))
						continue;
					if (!set_string(list[0], "setPdbAtomType", "setNilPdbAtomType", false, rset.getString("PDB_atom_type"), logw))
						continue;
					if (!set_string(list[0], "setPdbCompId", "setNilPdbCompId", false, rset.getString("PDB_comp_ID"), logw))
						continue;
					if (!set_string(list[0], "setPdbGroup", "setNilPdbGroup", false, rset.getString("PDB_group"), logw))
						continue;
					if (!set_string(list[0], "setPdbInsCode", "setNilPdbInsCode", false, rset.getString("PDB_ins_code"), logw))
						continue;
					if (!set_string(list[0], "setPdbOneLetterCode", "setNilPdbOneLetterCode", false, rset.getString("PDB_one_letter_code"), logw))
						continue;
					if (!set_integer(list[0], "setPdbSeqId", "setNilPdbSeqId", false, rset.getString("PDB_seq_ID"), logw))
						continue;
					if (!set_string(list[0], "setPdbStrandId", "setNilPdbStrandId", false, rset.getString("PDB_strand_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSeqId", "setNilSeqId", false, rset.getString("Seq_ID"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_AtomType.class.getName());
	...
	private static boolean set_string(Atom list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='Atom', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='Atom', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(Atom list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
