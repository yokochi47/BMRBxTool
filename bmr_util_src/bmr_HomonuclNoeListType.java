...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HomonuclNoeListType.*;

public class bmr_HomonuclNoeListType {

	private static final String table_name = "Homonucl_NOE_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HomonuclNoeListType body = HomonuclNoeListType.Factory.newInstance();
		HomonuclNoeList[] list = new HomonuclNoeList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:homonucl_noe_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HomonuclNoeList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HomonuclNoeList.Factory.newInstance();

					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_enum_homonuclear_noe_val_type(list[0], "setHomonuclearNoeValType", "", true, rset.getString("Homonuclear_NOE_val_type"), logw, errw))
						continue;
					...
					body.setHomonuclNoeListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HomonuclNoeListType.class.getName());
	...
	private static boolean set_string(HomonuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HomonuclNoeList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HomonuclNoeList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HomonuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(HomonuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_homonuclear_noe_val_type(HomonuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_HomonuclNOEList.getHomonuclearNOEValType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ HomonuclNoeList.HomonuclearNoeValType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					HomonuclNoeList.HomonuclearNoeValType.Enum _enum = HomonuclNoeList.HomonuclearNoeValType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:HomonuclNoeList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= HomonuclNoeList.HomonuclearNoeValType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + HomonuclNoeList.HomonuclearNoeValType.Enum.forInt(i));
						try {
							errw.write("class_name:HomonuclNoeList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= HomonuclNoeList.HomonuclearNoeValType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + HomonuclNoeList.HomonuclearNoeValType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
