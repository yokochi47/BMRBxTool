...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.StudyType.*;

public class bmr_StudyType {
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		StudyType body = StudyType.Factory.newInstance();
		Study[] list = new Study[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Study_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:studyCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='Study', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = Study.Factory.newInstance();
			...
			Logger lgr = Logger.getLogger(bmr_StudyType.class.getName());
	...
	private static boolean set_string(Study list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='Study', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='Study', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(Study list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(Study list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_Study.getType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ Study.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					Study.Type.Enum _enum = Study.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:Study method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= Study.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + Study.Type.Enum.forInt(i));
						try {
							errw.write("class_name:Study method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= Study.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + Study.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
