...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ConfFamilyCoordSetConstrListType.*;

public class bmr_ConfFamilyCoordSetConstrListType {

	private static final String table_name = "Conf_family_coord_set_constr_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ConfFamilyCoordSetConstrListType body = ConfFamilyCoordSetConstrListType.Factory.newInstance();
		ConfFamilyCoordSetConstrList[] list = new ConfFamilyCoordSetConstrList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Constraint_list_ID\")::decimal,(0 || \"Conformer_family_coord_set_ID\")::decimal");
				...
				buffw.write("  <BMRBx:conf_family_coord_set_constr_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ConfFamilyCoordSetConstrList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ConfFamilyCoordSetConstrList.Factory.newInstance();

					if (!set_string(list[0], "setConstraintListLabel", "", true, rset.getString("Constraint_list_label"), logw))
						continue;
					if (!set_integer(list[0], "setConformerFamilyCoordSetId", "", true, rset.getString("Conformer_family_coord_set_ID"), logw))
						continue;
					if (!set_string(list[0], "setConstraintListCategory", "", true, rset.getString("Constraint_list_category"), logw))
						continue;
					...
					body.setConfFamilyCoordSetConstrListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ConfFamilyCoordSetConstrListType.class.getName());
	...
	private static boolean set_string(ConfFamilyCoordSetConstrList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ConfFamilyCoordSetConstrList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ConfFamilyCoordSetConstrList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ConfFamilyCoordSetConstrList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
