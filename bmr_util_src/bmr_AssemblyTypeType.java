...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType;

public class bmr_AssemblyTypeType {

	private static final String table_name = "Assembly_type";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssemblyTypeType body = AssemblyTypeType.Factory.newInstance();
		org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType[] list = new org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:assembly_typeCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssemblyType', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_enum_type(list[0], "setType", "", true, rset.getString("Type"), conn_bmrb, entry_id, rset.getString("Assembly_ID"), logw, errw))
						continue;

					body.setAssemblyTypeArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssemblyTypeType.class.getName());
	...
	private static boolean set_string(org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssemblyType', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssemblyType', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String assembly_id, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_AssemblyType.getType(val_name);

		if (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?"))
			val_name = bmr_Util_AssemblyType.getType(val_name, entry_id, assembly_id);

		if (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?"))
			val_name = bmr_Util_AssemblyType.guessType(conn_bmrb, entry_id, assembly_id);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType.Type.Enum _enum = org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:AssemblyType method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType.Type.Enum.forInt(i));
						try {
							errw.write("class_name:AssemblyType method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyTypeType.AssemblyType.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
