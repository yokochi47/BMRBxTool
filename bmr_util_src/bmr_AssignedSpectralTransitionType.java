...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssignedSpectralTransitionType.*;

public class bmr_AssignedSpectralTransitionType {

	private static final String table_name = "Assigned_spectral_transition";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssignedSpectralTransitionType body = AssignedSpectralTransitionType.Factory.newInstance();
		AssignedSpectralTransition[] list = new AssignedSpectralTransition[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Spectral_peak_list_ID\")::decimal,(0 || \"Spectral_transition_ID\")::decimal,(0 || \"Spectral_dim_ID\")::decimal");
				...
				buffw.write("  <BMRBx:assigned_spectral_transitionCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssignedSpectralTransition', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AssignedSpectralTransition.Factory.newInstance();
					...
					if (!set_integer(list[0], "setAssignedChemShiftListId", "", true, rset.getString("Assigned_chem_shift_list_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAtomChemShiftId", "", true, rset.getString("Atom_chem_shift_ID"), logw))
						continue;
					...
					body.setAssignedSpectralTransitionArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssignedSpectralTransitionType.class.getName());
	...
	private static boolean set_string(AssignedSpectralTransition list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssignedSpectralTransition', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssignedSpectralTransition', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AssignedSpectralTransition list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(AssignedSpectralTransition list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
