...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MethodParamType.*;

public class bmr_MethodParamType {

	private static final String table_name = "Method_param";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MethodParamType body = MethodParamType.Factory.newInstance();
		MethodParam[] list = new MethodParam[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Method_ID\")::decimal");
				...
				buffw.write("  <BMRBx:method_paramCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MethodParam', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MethodParam.Factory.newInstance();

					if (!set_string(list[0], "setText", "setNilText", false, rset.getString("Text"), logw))
						continue;
					if (!set_string(list[0], "setTextFormat", "setNilTextFormat", false, rset.getString("Text_format"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_string(list[0], "setFileName", "", true, rset.getString("File_name"), logw))
						continue;
					if (!set_integer(list[0], "setMethodId", "", true, rset.getString("Method_ID"), logw))
						continue;

					body.setMethodParamArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MethodParamType.class.getName());
	...
	private static boolean set_string(MethodParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MethodParam', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MethodParam', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MethodParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
