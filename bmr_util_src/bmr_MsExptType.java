...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MsExptType.*;

public class bmr_MsExptType {

	private static final String table_name = "MS_expt";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MsExptType body = MsExptType.Factory.newInstance();
		MsExpt[] list = new MsExpt[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:ms_exptCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MsExpt', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MsExpt.Factory.newInstance();

					if (!set_integer(list[0], "setMassSpecId", "", true, rset.getString("Mass_spec_ID"), logw))
						continue;
					if (!set_string(list[0], "setMassSpecLabel", "", true, rset.getString("Mass_spec_label"), logw))
						continue;
					...
					if (!set_string(list[0], "setSfFramecode", "", true, rset.getString("Sf_framecode"), logw))
						continue;
					if (!set_enum_spectrum_type(list[0], "setSpectrumType", "setNilSpectrumType", false, rset.getString("Spectrum_type"), logw, errw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_MsExptType.class.getName());
	...
	private static boolean set_string(MsExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MsExpt', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MsExpt', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MsExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_spectrum_type(MsExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ MsExpt.SpectrumType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					MsExpt.SpectrumType.Enum _enum = MsExpt.SpectrumType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:MsExpt method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= MsExpt.SpectrumType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + MsExpt.SpectrumType.Enum.forInt(i));
						try {
							errw.write("class_name:MsExpt method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= MsExpt.SpectrumType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + MsExpt.SpectrumType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
