...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemShiftExperimentType.*;

public class bmr_ChemShiftExperimentType {

	private static final String table_name = "Chem_shift_experiment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemShiftExperimentType body = ChemShiftExperimentType.Factory.newInstance();
		ChemShiftExperiment[] list = new ChemShiftExperiment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Assigned_chem_shift_list_ID\")::decimal,(0 || \"Experiment_ID\")::decimal");
				...
				buffw.write("  <BMRBx:chem_shift_experimentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemShiftExperiment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemShiftExperiment.Factory.newInstance();
					...
					body.setChemShiftExperimentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemShiftExperimentType.class.getName());
	...
	private static boolean set_string(ChemShiftExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemShiftExperiment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemShiftExperiment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChemShiftExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_experiment_id(ChemShiftExperiment list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String sample_id, FileWriter logw) {
	...
	private static boolean set_integer_sample_id(ChemShiftExperiment list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String experiment_id, FileWriter logw) {
	...
	private static boolean set_enum_sample_state(ChemShiftExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null && (val_name.startsWith("aniso") || val_name.equals("solid") || val_name.contains("crystal")))
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemShiftExperiment.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemShiftExperiment.SampleState.Enum _enum = ChemShiftExperiment.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemShiftExperiment method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemShiftExperiment.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemShiftExperiment.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:ChemShiftExperiment method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemShiftExperiment.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemShiftExperiment.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
