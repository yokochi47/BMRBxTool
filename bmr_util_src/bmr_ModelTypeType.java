...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType;

public class bmr_ModelTypeType {

	private static final String table_name = "Model_type";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ModelTypeType body = ModelTypeType.Factory.newInstance();
		org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType[] list = new org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Atom_site_model_ID\")::decimal");
				...
				buffw.write("  <BMRBx:model_typeCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ModelType', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAtomSiteModelId", "", true, rset.getString("Atom_site_model_ID"), logw))
						continue;
					if (!set_enum_type(list[0], "setType", "", true, rset.getString("Type"), logw, errw))
						continue;

					body.setModelTypeArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ModelTypeType.class.getName());
	...
	private static boolean set_string(org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ModelType', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ModelType', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType.Type.Enum _enum = org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ModelType method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType.Type.Enum.forInt(i));
						try {
							errw.write("class_name:ModelType method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + org.pdbj.bmrbpub.schema.mmcifNmrStar.ModelTypeType.ModelType.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
