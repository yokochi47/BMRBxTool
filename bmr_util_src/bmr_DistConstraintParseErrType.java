...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DistConstraintParseErrType.*;

public class bmr_DistConstraintParseErrType {

	private static final String table_name = "Dist_constraint_parse_err";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DistConstraintParseErrType body = DistConstraintParseErrType.Factory.newInstance();
		DistConstraintParseErr[] list = new DistConstraintParseErr[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:dist_constraint_parse_errCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DistConstraintParseErr', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DistConstraintParseErr.Factory.newInstance();

					if (!set_integer(list[0], "setBeginColumn", "setNilBeginColumn", false, rset.getString("Begin_column"), logw))
						continue;
					if (!set_integer(list[0], "setBeginLine", "setNilBeginLine", false, rset.getString("Begin_line"), logw))
						continue;
					if (!set_string(list[0], "setContent", "", true, rset.getString("Content"), logw))
						continue;
					if (!set_integer(list[0], "setEndColumn", "setNilEndColumn", false, rset.getString("End_column"), logw))
						continue;
					if (!set_integer(list[0], "setEndLine", "setNilEndLine", false, rset.getString("End_line"), logw))
						continue;
					...
					body.setDistConstraintParseErrArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DistConstraintParseErrType.class.getName());
	...
	private static boolean set_string(DistConstraintParseErr list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DistConstraintParseErr', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DistConstraintParseErr', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DistConstraintParseErr list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
