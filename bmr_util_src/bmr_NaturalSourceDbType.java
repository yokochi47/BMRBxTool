...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.NaturalSourceDbType.*;

public class bmr_NaturalSourceDbType {

	private static final String table_name = "Natural_source_db";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		NaturalSourceDbType body = NaturalSourceDbType.Factory.newInstance();
		NaturalSourceDb[] list = new NaturalSourceDb[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Entity_natural_src_list_ID\")::decimal,(0 || \"Entity_natural_src_ID\")::decimal");
				...
				buffw.write("  <BMRBx:natural_source_dbCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='NaturalSourceDb', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = NaturalSourceDb.Factory.newInstance();

					if (!set_string(list[0], "setDatabaseType", "setNilDatabaseType", false, rset.getString("Database_type"), logw))
						continue;
					...
					if (!set_string(list[0], "setEntityLabel", "", true, rset.getString("Entity_label"), logw))
						continue;
					if (!set_string(list[0], "setEntryType", "setNilEntryType", false, rset.getString("Entry_type"), logw))
						continue;
					if (!set_string(list[0], "setGeneCdnaCode", "setNilGeneCdnaCode", false, rset.getString("Gene_cDNA_code"), logw))
						continue;
					if (!set_string(list[0], "setGeneLocusCode", "setNilGeneLocusCode", false, rset.getString("Gene_locus_code"), logw))
						continue;
					if (!set_string(list[0], "setOrfCode", "setNilOrfCode", false, rset.getString("ORF_code"), logw))
						continue;
					if (!set_string_database_code(list[0], "setDatabaseCode", "", true, rset.getString("Database_code"), rset.getString("Entry_code"), logw))
						continue;
					if (!set_integer_entity_natural_src_id(list[0], "setEntityNaturalSrcId", "", true, rset.getString("Entity_natural_src_ID"), i++, logw))
						continue;
					if (!set_integer(list[0], "setEntityNaturalSrcListId", "", true, rset.getString("Entity_natural_src_list_ID"), logw))
						continue;
					if (!set_string(list[0], "setEntryCode", "", true, rset.getString("Entry_code"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;

					body.setNaturalSourceDbArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_NaturalSourceDbType.class.getName());
	...
	private static boolean set_string(NaturalSourceDb list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='NaturalSourceDb', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='NaturalSourceDb', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_string_database_code(NaturalSourceDb list, String method_name, String nil_method_name, boolean required, String val_name, String accession_code, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")))
			val_name = bmr_Util_NaturalSourceDb.getDatabaseCode(val_name, accession_code);
	...
	private static boolean set_integer(NaturalSourceDb list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_entity_natural_src_id(NaturalSourceDb list, String method_name, String nil_method_name, boolean required, String val_name, int id, FileWriter logw) {
