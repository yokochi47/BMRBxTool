...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemCompDbLinkType.*;

public class bmr_ChemCompDbLinkType {

	private static final String table_name = "Chem_comp_db_link";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemCompDbLinkType body = ChemCompDbLinkType.Factory.newInstance();
		ChemCompDbLink[] list = new ChemCompDbLink[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:chem_comp_db_linkCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemCompDbLink', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemCompDbLink.Factory.newInstance();

					if (!set_string(list[0], "setAccessionCodeType", "setNilAccessionCodeType", false, rset.getString("Accession_code_type"), logw))
						continue;
					...
					body.setChemCompDbLinkArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemCompDbLinkType.class.getName());
	...
	private static boolean set_string(ChemCompDbLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemCompDbLink', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemCompDbLink', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_enum_author_supplied(ChemCompDbLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemCompDbLink.AuthorSupplied.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemCompDbLink.AuthorSupplied.Enum _enum = ChemCompDbLink.AuthorSupplied.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemCompDbLink method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemCompDbLink.AuthorSupplied.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemCompDbLink.AuthorSupplied.Enum.forInt(i));
						try {
							errw.write("class_name:ChemCompDbLink method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemCompDbLink.AuthorSupplied.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemCompDbLink.AuthorSupplied.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
