...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MassSpecConfigType.*;

public class bmr_MassSpecConfigType {

	private static final String table_name = "Mass_spec_config";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MassSpecConfigType body = MassSpecConfigType.Factory.newInstance();
		MassSpecConfig[] list = new MassSpecConfig[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Mass_spec_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:mass_spec_configCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MassSpecConfig', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MassSpecConfig.Factory.newInstance();

					if (!set_string(list[0], "setComponentName", "setNilComponentName", false, rset.getString("Component_name"), logw))
						continue;
					if (!set_enum_component_type(list[0], "setComponentType", "", true, rset.getString("Component_type"), logw, errw))
						continue;
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setMassSpecId", "", true, rset.getString("Mass_spec_ID"), logw))
						continue;

					body.setMassSpecConfigArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MassSpecConfigType.class.getName());
	...
	private static boolean set_string(MassSpecConfig list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MassSpecConfig', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MassSpecConfig', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MassSpecConfig list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_component_type(MassSpecConfig list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ MassSpecConfig.ComponentType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					MassSpecConfig.ComponentType.Enum _enum = MassSpecConfig.ComponentType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:MassSpecConfig method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= MassSpecConfig.ComponentType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + MassSpecConfig.ComponentType.Enum.forInt(i));
						try {
							errw.write("class_name:MassSpecConfig method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= MassSpecConfig.ComponentType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + MassSpecConfig.ComponentType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
