...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SpectralTransitionGeneralCharType.*;

public class bmr_SpectralTransitionGeneralCharType {

	private static final String table_name = "Spectral_transition_general_char";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SpectralTransitionGeneralCharType body = SpectralTransitionGeneralCharType.Factory.newInstance();
		SpectralTransitionGeneralChar[] list = new SpectralTransitionGeneralChar[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Spectral_peak_list_ID\")::decimal,(0 || \"Spectral_transition_ID\")::decimal");
				...
				buffw.write("  <BMRBx:spectral_transition_general_charCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SpectralTransitionGeneralChar', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SpectralTransitionGeneralChar.Factory.newInstance();

					if (!set_decimal(list[0], "setIntensityValErr", "setNilIntensityValErr", false, rset.getString("Intensity_val_err"), logw))
						continue;
					if (!set_enum_measurement_method(list[0], "setMeasurementMethod", "setNilMeasurementMethod", false, rset.getString("Measurement_method"), logw, errw))
						continue;
					if (!set_integer(list[0], "setPeakId", "setNilPeakId", false, rset.getString("Peak_ID"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setIntensityVal", "", true, rset.getString("Intensity_val"), logw))
						continue;
					if (!set_integer(list[0], "setSpectralPeakListId", "", true, rset.getString("Spectral_peak_list_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSpectralTransitionId", "", true, rset.getString("Spectral_transition_ID"), logw))
						continue;

					body.setSpectralTransitionGeneralCharArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SpectralTransitionGeneralCharType.class.getName());
	...
	private static boolean set_string(SpectralTransitionGeneralChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SpectralTransitionGeneralChar', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SpectralTransitionGeneralChar', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SpectralTransitionGeneralChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(SpectralTransitionGeneralChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_measurement_method(SpectralTransitionGeneralChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ SpectralTransitionGeneralChar.MeasurementMethod.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					SpectralTransitionGeneralChar.MeasurementMethod.Enum _enum = SpectralTransitionGeneralChar.MeasurementMethod.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:SpectralTransitionGeneralChar method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= SpectralTransitionGeneralChar.MeasurementMethod.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + SpectralTransitionGeneralChar.MeasurementMethod.Enum.forInt(i));
						try {
							errw.write("class_name:SpectralTransitionGeneralChar method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= SpectralTransitionGeneralChar.MeasurementMethod.Enum.table.lastInt(); i++)
								errw.write(" enum:" + SpectralTransitionGeneralChar.MeasurementMethod.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
