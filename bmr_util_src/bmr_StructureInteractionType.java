...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.StructureInteractionType.*;

public class bmr_StructureInteractionType {

	private static final String table_name = "Structure_interaction";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		StructureInteractionType body = StructureInteractionType.Factory.newInstance();
		StructureInteraction[] list = new StructureInteraction[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Structure_interaction_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:structure_interactionCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='StructureInteraction', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = StructureInteraction.Factory.newInstance();
					...
					if (!set_string(list[0], "setAtomId2", "", true, rset.getString("Atom_ID_2"), logw))
						continue;
					if (!set_string(list[0], "setAtomType1", "setNilAtomType1", false, rset.getString("Atom_type_1"), logw))
						continue;
					if (!set_string(list[0], "setAtomType2", "setNilAtomType2", false, rset.getString("Atom_type_2"), logw))
						continue;
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setStructureInteractionListId", "", true, rset.getString("Structure_interaction_list_ID"), logw))
						continue;

					body.setStructureInteractionArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_StructureInteractionType.class.getName());
	...
	private static boolean set_string(StructureInteraction list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='StructureInteraction', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='StructureInteraction', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(StructureInteraction list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
