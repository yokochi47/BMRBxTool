...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DHFractFactorExperimentType.*;

public class bmr_DHFractFactorExperimentType {

	private static final String table_name = "D_H_fract_factor_experiment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DHFractFactorExperimentType body = DHFractFactorExperimentType.Factory.newInstance();
		DHFractFactorExperiment[] list = new DHFractFactorExperiment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"D_H_fractionation_factor_list_ID\")::decimal,(0 || \"Experiment_ID\")::decimal");
				...
				buffw.write("  <BMRBx:d_h_fract_factor_experimentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DHFractFactorExperiment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DHFractFactorExperiment.Factory.newInstance();
					...
					if (!set_enum_sample_state(list[0], "setSampleState", "setNilSampleState", false, rset.getString("Sample_state"), logw, errw))
						continue;
					if (!set_integer(list[0], "setDHFractionationFactorListId", "", true, rset.getString("D_H_fractionation_factor_list_ID"), logw))
						continue;
					...
					body.setDHFractFactorExperimentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DHFractFactorExperimentType.class.getName());
	...
	private static boolean set_string(DHFractFactorExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DHFractFactorExperiment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DHFractFactorExperiment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DHFractFactorExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_sample_state(DHFractFactorExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ DHFractFactorExperiment.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					DHFractFactorExperiment.SampleState.Enum _enum = DHFractFactorExperiment.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:DHFractFactorExperiment method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= DHFractFactorExperiment.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + DHFractFactorExperiment.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:DHFractFactorExperiment method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= DHFractFactorExperiment.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + DHFractFactorExperiment.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
