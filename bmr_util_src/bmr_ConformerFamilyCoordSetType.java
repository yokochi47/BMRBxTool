...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ConformerFamilyCoordSetType.*;

public class bmr_ConformerFamilyCoordSetType {

	private static final String table_name = "Conformer_family_coord_set";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ConformerFamilyCoordSetType body = ConformerFamilyCoordSetType.Factory.newInstance();
		ConformerFamilyCoordSet[] list = new ConformerFamilyCoordSet[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:conformer_family_coord_setCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ConformerFamilyCoordSet', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ConformerFamilyCoordSet.Factory.newInstance();

					if (!set_string(list[0], "setAtomSiteOrderedFlagDesc", "setNilAtomSiteOrderedFlagDesc", false, rset.getString("Atom_site_ordered_flag_desc"), logw))
						continue;
					if (!set_string(list[0], "setAtomSiteUncertaintyDesc", "setNilAtomSiteUncertaintyDesc", false, rset.getString("Atom_site_uncertainty_desc"), logw))
						continue;
					if (!set_string(list[0], "setConstraintsPdbFileId", "setNilConstraintsPdbFileId", false, rset.getString("Constraints_PDB_file_ID"), logw))
						continue;
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setFileName", "setNilFileName", false, rset.getString("File_name"), logw))
						continue;
					...
					body.setConformerFamilyCoordSetArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ConformerFamilyCoordSetType.class.getName());
	...
	private static boolean set_string(ConformerFamilyCoordSet list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ConformerFamilyCoordSet', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ConformerFamilyCoordSet', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ConformerFamilyCoordSet list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_sample_condition_list_id(ConformerFamilyCoordSet list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, FileWriter logw) {
