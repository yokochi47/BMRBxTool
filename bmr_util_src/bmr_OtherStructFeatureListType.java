...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.OtherStructFeatureListType.*;

public class bmr_OtherStructFeatureListType {

	private static final String table_name = "Other_struct_feature_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		OtherStructFeatureListType body = OtherStructFeatureListType.Factory.newInstance();
		OtherStructFeatureList[] list = new OtherStructFeatureList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:other_struct_feature_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='OtherStructFeatureList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = OtherStructFeatureList.Factory.newInstance();
					...
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_string(list[0], "setSelectionMethod", "setNilSelectionMethod", false, rset.getString("Selection_method"), logw))
						continue;
					...
					if (!set_string(list[0], "setSfFramecode", "", true, rset.getString("Sf_framecode"), logw))
						continue;
					if (!set_string(list[0], "setStructFeatureDefinition", "setNilStructFeatureDefinition", false, rset.getString("Struct_feature_definition"), logw))
						continue;
					if (!set_string(list[0], "setStructFeatureName", "", true, rset.getString("Struct_feature_name"), logw))
						continue;
					...
					body.setOtherStructFeatureListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_OtherStructFeatureListType.class.getName());
	...
	private static boolean set_string(OtherStructFeatureList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='OtherStructFeatureList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='OtherStructFeatureList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(OtherStructFeatureList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
