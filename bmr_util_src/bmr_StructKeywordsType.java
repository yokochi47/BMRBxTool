...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.StructKeywordsType.*;

public class bmr_StructKeywordsType {

	private static final String table_name = "Struct_keywords";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		StructKeywordsType body = StructKeywordsType.Factory.newInstance();
		StructKeywords[] list = new StructKeywords[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:struct_keywordsCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='StructKeywords', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = StructKeywords.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_string(list[0], "setKeywords", "", true, rset.getString("Keywords"), logw))
						continue;

					body.setStructKeywordsArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_StructKeywordsType.class.getName());
	...
	private static boolean set_string(StructKeywords list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='StructKeywords', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='StructKeywords', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
