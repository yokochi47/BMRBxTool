...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemShiftIsotopeEffectListType.*;

public class bmr_ChemShiftIsotopeEffectListType {

	private static final String table_name = "Chem_shift_isotope_effect_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemShiftIsotopeEffectListType body = ChemShiftIsotopeEffectListType.Factory.newInstance();
		ChemShiftIsotopeEffectList[] list = new ChemShiftIsotopeEffectList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:chem_shift_isotope_effect_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemShiftIsotopeEffectList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemShiftIsotopeEffectList.Factory.newInstance();

					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setIsotopeEffectType", "setNilIsotopeEffectType", false, rset.getString("Isotope_effect_type"), logw))
						continue;
					if (!set_string(list[0], "setIsotopeEffectValUnits", "setNilIsotopeEffectValUnits", false, rset.getString("Isotope_effect_val_units"), logw))
						continue;
					...
					body.setChemShiftIsotopeEffectListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemShiftIsotopeEffectListType.class.getName());
	...
	private static boolean set_string(ChemShiftIsotopeEffectList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemShiftIsotopeEffectList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemShiftIsotopeEffectList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChemShiftIsotopeEffectList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
