...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EntityDeletedAtomType.*;

public class bmr_EntityDeletedAtomType {

	private static final String table_name = "Entity_deleted_atom";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EntityDeletedAtomType body = EntityDeletedAtomType.Factory.newInstance();
		EntityDeletedAtom[] list = new EntityDeletedAtom[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:entity_deleted_atomCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EntityDeletedAtom', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EntityDeletedAtom.Factory.newInstance();
					...
					if (!set_string(list[0], "setCompId", "setNilCompId", false, rset.getString("Comp_ID"), logw))
						continue;
					if (!set_integer(list[0], "setCompIndexId", "setNilCompIndexId", false, rset.getString("Comp_index_ID"), logw))
						continue;
					if (!set_integer(list[0], "setEntityAssemblyId", "", false, rset.getString("Entity_assembly_ID"), logw))
						continue;
					if (!set_integer(list[0], "setEntityAtomListId", "setNilEntityAtomListId", false, rset.getString("Entity_atom_list_ID"), logw))
						continue;
					...
					body.setEntityDeletedAtomArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_EntityDeletedAtomType.class.getName());
	...
	private static boolean set_string(EntityDeletedAtom list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EntityDeletedAtom', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EntityDeletedAtom', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EntityDeletedAtom list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
