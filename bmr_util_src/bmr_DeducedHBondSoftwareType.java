...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DeducedHBondSoftwareType.*;

public class bmr_DeducedHBondSoftwareType {

	private static final String table_name = "Deduced_H_bond_software";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DeducedHBondSoftwareType body = DeducedHBondSoftwareType.Factory.newInstance();
		DeducedHBondSoftware[] list = new DeducedHBondSoftware[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Deduced_H_bond_list_ID\")::decimal,(0 || \"Software_ID\")::decimal");
				...
				buffw.write("  <BMRBx:deduced_h_bond_softwareCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DeducedHBondSoftware', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DeducedHBondSoftware.Factory.newInstance();
					...
					body.setDeducedHBondSoftwareArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DeducedHBondSoftwareType.class.getName());
	...
	private static boolean set_string(DeducedHBondSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DeducedHBondSoftware', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DeducedHBondSoftware', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DeducedHBondSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_software_id(DeducedHBondSoftware list, String method_name, String nil_method_name, boolean required, String val_name, int id, FileWriter logw) {
