...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TorsionAngleType.*;

public class bmr_TorsionAngleType {

	private static final String table_name = "Torsion_angle";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TorsionAngleType body = TorsionAngleType.Factory.newInstance();
		TorsionAngle[] list = new TorsionAngle[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:torsion_angleCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TorsionAngle', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TorsionAngle.Factory.newInstance();
					...
					if (!set_integer(list[0], "setCompIndexId4", "", true, rset.getString("Comp_index_ID_4"), logw))
						continue;
					if (!set_string(list[0], "setCompLabel1", "", true, rset.getString("Comp_label_1"), logw))
						continue;
					if (!set_string(list[0], "setCompLabel2", "", true, rset.getString("Comp_label_2"), logw))
						continue;
					if (!set_string(list[0], "setCompLabel3", "", true, rset.getString("Comp_label_3"), logw))
						continue;
					if (!set_string(list[0], "setCompLabel4", "", true, rset.getString("Comp_label_4"), logw))
						continue;
					...
					if (!set_integer(list[0], "setEntityId4", "", true, rset.getString("Entity_ID_4"), logw))
						continue;
					if (!set_string(list[0], "setEntityLabel1", "", true, rset.getString("Entity_label_1"), logw))
						continue;
					if (!set_string(list[0], "setEntityLabel2", "", true, rset.getString("Entity_label_2"), logw))
						continue;
					if (!set_string(list[0], "setEntityLabel3", "", true, rset.getString("Entity_label_3"), logw))
						continue;
					if (!set_string(list[0], "setEntityLabel4", "", true, rset.getString("Entity_label_4"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSeqId4", "", true, rset.getString("Seq_ID_4"), logw))
						continue;
					if (!set_string(list[0], "setTorsionAngleName", "", true, rset.getString("Torsion_angle_name"), logw))
						continue;
					...
					body.setTorsionAngleArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TorsionAngleType.class.getName());
	...
	private static boolean set_string(TorsionAngle list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TorsionAngle', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TorsionAngle', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TorsionAngle list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
