...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomSitesFootnoteType.*;

public class bmr_AtomSitesFootnoteType {

	private static final String table_name = "Atom_sites_footnote";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AtomSitesFootnoteType body = AtomSitesFootnoteType.Factory.newInstance();
		AtomSitesFootnote[] list = new AtomSitesFootnote[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Footnote_ID\")::decimal");
				...
				buffw.write("  <BMRBx:atom_sites_footnoteCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AtomSitesFootnote', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AtomSitesFootnote.Factory.newInstance();

					if (!set_string(list[0], "setText", "", true, rset.getString("Text"), logw))
						continue;
					...
					body.setAtomSitesFootnoteArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AtomSitesFootnoteType.class.getName());
	...
	private static boolean set_string(AtomSitesFootnote list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AtomSitesFootnote', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AtomSitesFootnote', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AtomSitesFootnote list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
