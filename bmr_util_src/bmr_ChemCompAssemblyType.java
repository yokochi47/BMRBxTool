...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemCompAssemblyType.*;

public class bmr_ChemCompAssemblyType {

	private static final String table_name = "Chem_comp_assembly";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemCompAssemblyType body = ChemCompAssemblyType.Factory.newInstance();
		ChemCompAssembly[] list = new ChemCompAssembly[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Entity_assembly_ID\")::decimal,(0 || \"Comp_index_ID\")::decimal,(0 || \"Assembly_ID\")::decimal");
				...
				buffw.write("  <BMRBx:chem_comp_assemblyCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemCompAssembly', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemCompAssembly.Factory.newInstance();

					if (!set_string(list[0], "setAssemblyChemCompId", "setNilAssemblyChemCompId", false, rset.getString("Assembly_chem_comp_ID"), logw))
						continue;
					...
					if (!set_string(list[0], "setAuthSeqId", "setNilAuthSeqId", false, rset.getString("Auth_seq_ID"), logw))
						continue;
					if (!set_string(list[0], "setAuthVariantId", "setNilAuthVariantId", false, rset.getString("Auth_variant_ID"), logw))
						continue;
					if (!set_enum_cis_residue(list[0], "setCisResidue", "setNilCisResidue", false, rset.getString("Cis_residue"), logw, errw))
						continue;
					if (!set_string(list[0], "setCompId", "setNilCompId", false, rset.getString("Comp_ID"), logw))
						continue;
					if (!set_integer(list[0], "setEntityId", "setNilEntityId", false, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_integer(list[0], "setNefIndex", "setNilNefIndex", false, rset.getString("NEF_index"), logw))
						continue;
					if (!set_integer(list[0], "setSeqId", "", true, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_string(list[0], "setSequenceLinking", "setNilSequenceLinking", false, rset.getString("Sequence_linking"), logw))
						continue;
					...
					body.setChemCompAssemblyArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemCompAssemblyType.class.getName());
	...
	private static boolean set_string(ChemCompAssembly list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemCompAssembly', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemCompAssembly', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChemCompAssembly list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_cis_residue(ChemCompAssembly list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemCompAssembly.CisResidue.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemCompAssembly.CisResidue.Enum _enum = ChemCompAssembly.CisResidue.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemCompAssembly method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemCompAssembly.CisResidue.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemCompAssembly.CisResidue.Enum.forInt(i));
						try {
							errw.write("class_name:ChemCompAssembly method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemCompAssembly.CisResidue.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemCompAssembly.CisResidue.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
