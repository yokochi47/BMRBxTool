...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SaxsConstraintExptType.*;

public class bmr_SaxsConstraintExptType {

	private static final String table_name = "SAXS_constraint_expt";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SaxsConstraintExptType body = SaxsConstraintExptType.Factory.newInstance();
		SaxsConstraintExpt[] list = new SaxsConstraintExpt[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"SAXS_constraint_list_ID\")::decimal");
				...
				buffw.write("  <BMRBx:saxs_constraint_exptCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SaxsConstraintExpt', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SaxsConstraintExpt.Factory.newInstance();

					if (!set_string(list[0], "setExperimentName", "", true, rset.getString("Experiment_name"), logw))
						continue;
					if (!set_string(list[0], "setSampleLabel", "", true, rset.getString("Sample_label"), logw))
						continue;
					...
					body.setSaxsConstraintExptArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SaxsConstraintExptType.class.getName());
	...
	private static boolean set_string(SaxsConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SaxsConstraintExpt', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SaxsConstraintExpt', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SaxsConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
