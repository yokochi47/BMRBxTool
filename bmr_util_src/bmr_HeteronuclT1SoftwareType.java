...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HeteronuclT1SoftwareType.*;

public class bmr_HeteronuclT1SoftwareType {

	private static final String table_name = "Heteronucl_T1_software";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HeteronuclT1SoftwareType body = HeteronuclT1SoftwareType.Factory.newInstance();
		HeteronuclT1Software[] list = new HeteronuclT1Software[1];
		...
		try {
			...
			if (rset.next() && bmr_Util_Main.remediate_xml && !entry_id.equals("5505")) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Heteronucl_T1_list_ID\")::decimal,(0 || \"Software_ID\")::decimal");
				...
				buffw.write("  <BMRBx:heteronucl_t1_softwareCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HeteronuclT1Software', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HeteronuclT1Software.Factory.newInstance();
					...
					body.setHeteronuclT1SoftwareArray(list);
				...
				buffw.write("  </BMRBx:heteronucl_t1_softwareCategory>\n");
						...
						try {
							logw.write("category='HeteronuclT1Software', all information content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
			...
			Logger lgr = Logger.getLogger(bmr_HeteronuclT1SoftwareType.class.getName());
	...
	private static boolean set_string(HeteronuclT1Software list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HeteronuclT1Software', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HeteronuclT1Software', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HeteronuclT1Software list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_software_id(HeteronuclT1Software list, String method_name, String nil_method_name, boolean required, String val_name, int id, FileWriter logw) {
