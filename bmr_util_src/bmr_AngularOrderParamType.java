...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AngularOrderParamType.*;

public class bmr_AngularOrderParamType {

	private static final String table_name = "Angular_order_param";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AngularOrderParamType body = AngularOrderParamType.Factory.newInstance();
		AngularOrderParam[] list = new AngularOrderParam[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Angular_order_parameter_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:angular_order_paramCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AngularOrderParam', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AngularOrderParam.Factory.newInstance();
					...
					if (!set_string(list[0], "setAuthSeqId", "setNilAuthSeqId", false, rset.getString("Auth_seq_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setChi1SAngleVal", "setNilChi1SAngleVal", false, rset.getString("Chi_1_S_angle_val"), logw))
						continue;
					if (!set_decimal(list[0], "setChi2SAngleVal", "setNilChi2SAngleVal", false, rset.getString("Chi_2_S_angle_val"), logw))
						continue;
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setPhiSAngleVal", "setNilPhiSAngleVal", false, rset.getString("Phi_S_angle_val"), logw))
						continue;
					if (!set_decimal(list[0], "setPsiSAngleVal", "setNilPsiSAngleVal", false, rset.getString("Psi_S_angle_val"), logw))
						continue;
					if (!set_integer(list[0], "setSeqId", "", true, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAngularOrderParameterListId", "", true, rset.getString("Angular_order_parameter_list_ID"), logw))
						continue;
					...
					body.setAngularOrderParamArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AngularOrderParamType.class.getName());
	...
	private static boolean set_string(AngularOrderParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AngularOrderParam', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AngularOrderParam', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AngularOrderParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(AngularOrderParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
