...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.OtherStructFeatureType.*;

public class bmr_OtherStructFeatureType {

	private static final String table_name = "Other_struct_feature";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		OtherStructFeatureType body = OtherStructFeatureType.Factory.newInstance();
		OtherStructFeature[] list = new OtherStructFeature[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Other_struct_feature_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:other_struct_featureCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='OtherStructFeature', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = OtherStructFeature.Factory.newInstance();
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setOtherStructFeatureListId", "", true, rset.getString("Other_struct_feature_list_ID"), logw))
						continue;

					body.setOtherStructFeatureArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_OtherStructFeatureType.class.getName());
	...
	private static boolean set_string(OtherStructFeature list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='OtherStructFeature', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='OtherStructFeature', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(OtherStructFeature list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
