...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.CharacteristicType.*;

public class bmr_CharacteristicType {

	private static final String table_name = "Characteristic";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		CharacteristicType body = CharacteristicType.Factory.newInstance();
		Characteristic[] list = new Characteristic[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:characteristicCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='Characteristic', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = Characteristic.Factory.newInstance();

					if (!set_string(list[0], "setAtomId", "", true, rset.getString("Atom_ID"), logw))
						continue;
					if (!set_string(list[0], "setChemicalGroup", "setNilChemicalGroup", false, rset.getString("Chemical_group"), logw))
						continue;
					...
					if (!set_string(list[0], "setName", "", true, rset.getString("Name"), logw))
						continue;
					if (!set_string(list[0], "setSource", "", true, rset.getString("Source"), logw))
						continue;
					if (!set_decimal(list[0], "setVal", "", true, rset.getString("Val"), logw))
						continue;
					if (!set_decimal(list[0], "setValErr", "", true, rset.getString("Val_err"), logw))
						continue;
					...
					body.setCharacteristicArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_CharacteristicType.class.getName());
	...
	private static boolean set_string(Characteristic list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='Characteristic', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='Characteristic', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(Characteristic list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(Characteristic list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
