...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.PhTitrationListType.*;

public class bmr_PhTitrationListType {

	private static final String table_name = "PH_titration_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		PhTitrationListType body = PhTitrationListType.Factory.newInstance();
		PhTitrationList[] list = new PhTitrationList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:ph_titration_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='PhTitrationList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = PhTitrationList.Factory.newInstance();

					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_enum_expt_observed_param(list[0], "setExptObservedParam", "", true, rset.getString("Expt_observed_param"), logw, errw))
						continue;
					...
					body.setPhTitrationListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_PhTitrationListType.class.getName());
	...
	private static boolean set_string(PhTitrationList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='PhTitrationList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PhTitrationList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(PhTitrationList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_expt_observed_param(PhTitrationList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_PHTitrationList.getExptObservedParam(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ PhTitrationList.ExptObservedParam.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					PhTitrationList.ExptObservedParam.Enum _enum = PhTitrationList.ExptObservedParam.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:PhTitrationList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= PhTitrationList.ExptObservedParam.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + PhTitrationList.ExptObservedParam.Enum.forInt(i));
						try {
							errw.write("class_name:PhTitrationList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= PhTitrationList.ExptObservedParam.Enum.table.lastInt(); i++)
								errw.write(" enum:" + PhTitrationList.ExptObservedParam.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
