...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.CsAnisotropyType.*;

public class bmr_CsAnisotropyType {

	private static final String table_name = "CS_anisotropy";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		CsAnisotropyType body = CsAnisotropyType.Factory.newInstance();
		CsAnisotropy[] list = new CsAnisotropy[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Chem_shift_anisotropy_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:cs_anisotropyCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='CsAnisotropy', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = CsAnisotropy.Factory.newInstance();
					...
					if (!set_string(list[0], "setAuthSeqId", "setNilAuthSeqId", false, rset.getString("Auth_seq_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setBondLength", "setNilBondLength", false, rset.getString("Bond_length"), logw))
						continue;
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setPrincipalEulerAngleAlphaVal", "setNilPrincipalEulerAngleAlphaVal", false, rset.getString("Principal_Euler_angle_alpha_val"), logw))
						continue;
					if (!set_decimal(list[0], "setPrincipalEulerAngleBetaVal", "setNilPrincipalEulerAngleBetaVal", false, rset.getString("Principal_Euler_angle_beta_val"), logw))
						continue;
					if (!set_decimal(list[0], "setPrincipalEulerAngleGammaVal", "setNilPrincipalEulerAngleGammaVal", false, rset.getString("Principal_Euler_angle_gamma_val"), logw))
						continue;
					if (!set_decimal(list[0], "setPrincipalValueSigma11Val", "setNilPrincipalValueSigma11Val", false, rset.getString("Principal_value_sigma_11_val"), logw))
						continue;
					if (!set_decimal(list[0], "setPrincipalValueSigma22Val", "setNilPrincipalValueSigma22Val", false, rset.getString("Principal_value_sigma_22_val"), logw))
						continue;
					if (!set_decimal(list[0], "setPrincipalValueSigma33Val", "setNilPrincipalValueSigma33Val", false, rset.getString("Principal_value_sigma_33_val"), logw))
						continue;
					if (!set_integer(list[0], "setResonanceId", "", true, rset.getString("Resonance_ID"), logw))
						continue;
					...
					if (!set_decimal(list[0], "setValErr", "setNilValErr", false, rset.getString("Val_err"), logw))
						continue;
					if (!set_integer(list[0], "setChemShiftAnisotropyId", "", true, rset.getString("Chem_shift_anisotropy_ID"), logw))
						continue;
					...
					body.setCsAnisotropyArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_CsAnisotropyType.class.getName());
	...
	private static boolean set_string(CsAnisotropy list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='CsAnisotropy', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='CsAnisotropy', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(CsAnisotropy list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(CsAnisotropy list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
