...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType;

public class bmr_AtomTypeType {

	private static final String table_name = "Atom_type";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AtomTypeType body = AtomTypeType.Factory.newInstance();
		org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType[] list = new org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:atom_typeCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AtomType', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType.Factory.newInstance();

					if (!set_decimal(list[0], "setAtomicMass", "setNilAtomicMass", false, rset.getString("Atomic_mass"), logw))
						continue;
					if (!set_integer(list[0], "setAtomicNumber", "setNilAtomicNumber", false, rset.getString("Atomic_number"), logw))
						continue;
					if (!set_string(list[0], "setElectronConfiguration", "setNilElectronConfiguration", false, rset.getString("Electron_configuration"), logw))
						continue;
					...
					if (!set_enum_paramagnetic(list[0], "setParamagnetic", "setNilParamagnetic", false, rset.getString("Paramagnetic"), logw, errw))
						continue;
					if (!set_string(list[0], "setSymbol", "setNilSymbol", false, rset.getString("Symbol"), logw))
						continue;
					if (!set_integer(list[0], "setUnpairedElectronNumber", "setNilUnpairedElectronNumber", false, rset.getString("Unpaired_electron_number"), logw))
						continue;
					if (!set_decimal(list[0], "setVanDerVaalsRadii", "setNilVanDerVaalsRadii", false, rset.getString("Van_der_Vaals_radii"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_AtomTypeType.class.getName());
	...
	private static boolean set_string(org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AtomType', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AtomType', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_paramagnetic(org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType.Paramagnetic.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType.Paramagnetic.Enum _enum = org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType.Paramagnetic.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:AtomType method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType.Paramagnetic.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType.Paramagnetic.Enum.forInt(i));
						try {
							errw.write("class_name:AtomType method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType.Paramagnetic.Enum.table.lastInt(); i++)
								errw.write(" enum:" + org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomTypeType.AtomType.Paramagnetic.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
