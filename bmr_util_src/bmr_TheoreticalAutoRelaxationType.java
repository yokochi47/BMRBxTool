...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalAutoRelaxationType.*;

public class bmr_TheoreticalAutoRelaxationType {

	private static final String table_name = "Theoretical_auto_relaxation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalAutoRelaxationType body = TheoreticalAutoRelaxationType.Factory.newInstance();
		TheoreticalAutoRelaxation[] list = new TheoreticalAutoRelaxation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Theoretical_auto_relaxation_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:theoretical_auto_relaxationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalAutoRelaxation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalAutoRelaxation.Factory.newInstance();
					...
					if (!set_string(list[0], "setAuthSeqId", "setNilAuthSeqId", false, rset.getString("Auth_seq_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setAutoRelaxationVal", "", true, rset.getString("Auto_relaxation_val"), logw))
						continue;
					if (!set_decimal(list[0], "setAutoRelaxationValErr", "setNilAutoRelaxationValErr", false, rset.getString("Auto_relaxation_val_err"), logw))
						continue;
					...
					if (!set_decimal(list[0], "setRexVal", "setNilRexVal", false, rset.getString("Rex_val"), logw))
						continue;
					if (!set_decimal(list[0], "setRexValErr", "setNilRexValErr", false, rset.getString("Rex_val_err"), logw))
						continue;
					...
					body.setTheoreticalAutoRelaxationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalAutoRelaxationType.class.getName());
	...
	private static boolean set_string(TheoreticalAutoRelaxation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalAutoRelaxation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalAutoRelaxation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalAutoRelaxation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(TheoreticalAutoRelaxation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
