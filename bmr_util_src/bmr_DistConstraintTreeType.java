...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DistConstraintTreeType.*;

public class bmr_DistConstraintTreeType {

	private static final String table_name = "Dist_constraint_tree";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DistConstraintTreeType body = DistConstraintTreeType.Factory.newInstance();
		DistConstraintTree[] list = new DistConstraintTree[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:dist_constraint_treeCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DistConstraintTree', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DistConstraintTree.Factory.newInstance();

					if (!set_integer(list[0], "setDownNodeId", "setNilDownNodeId", false, rset.getString("Down_node_ID"), logw))
						continue;
					if (!set_string(list[0], "setLogicOperation", "setNilLogicOperation", false, rset.getString("Logic_operation"), logw))
						continue;
					if (!set_integer(list[0], "setRightNodeId", "setNilRightNodeId", false, rset.getString("Right_node_ID"), logw))
						continue;
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setNodeId", "", true, rset.getString("Node_ID"), logw))
						continue;

					body.setDistConstraintTreeArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DistConstraintTreeType.class.getName());
	...
	private static boolean set_string(DistConstraintTree list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DistConstraintTree', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DistConstraintTree', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DistConstraintTree list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
