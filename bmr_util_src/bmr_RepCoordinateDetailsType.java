...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.RepCoordinateDetailsType.*;

public class bmr_RepCoordinateDetailsType {

	private static final String table_name = "Rep_coordinate_details";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		RepCoordinateDetailsType body = RepCoordinateDetailsType.Factory.newInstance();
		RepCoordinateDetails[] list = new RepCoordinateDetails[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:rep_coordinate_detailsCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='RepCoordinateDetails', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = RepCoordinateDetails.Factory.newInstance();

					if (!set_string(list[0], "setFootnote", "", true, rset.getString("Footnote"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setFootnoteId", "", true, rset.getString("Footnote_ID"), logw))
						continue;
					if (!set_integer(list[0], "setRepresentativeConformerId", "", true, rset.getString("Representative_conformer_ID"), logw))
						continue;

					body.setRepCoordinateDetailsArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_RepCoordinateDetailsType.class.getName());
	...
	private static boolean set_string(RepCoordinateDetails list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='RepCoordinateDetails', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='RepCoordinateDetails', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(RepCoordinateDetails list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
