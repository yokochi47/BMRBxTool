...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DipolarCouplingListType.*;

public class bmr_DipolarCouplingListType {

	private static final String table_name = "Dipolar_coupling_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DipolarCouplingListType body = DipolarCouplingListType.Factory.newInstance();
		DipolarCouplingList[] list = new DipolarCouplingList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:dipolar_coupling_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DipolarCouplingList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DipolarCouplingList.Factory.newInstance();

					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setFittingProcedure", "setNilFittingProcedure", false, rset.getString("Fitting_procedure"), logw))
						continue;
					...
					if (!set_string(list[0], "setSampleConditionListLabel", "", true, rset.getString("Sample_condition_list_label"), logw))
						continue;
					if (!set_decimal(list[0], "setScalingFactor", "setNilScalingFactor", false, rset.getString("Scaling_factor"), logw))
						continue;
					...
					body.setDipolarCouplingListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DipolarCouplingListType.class.getName());
	...
	private static boolean set_string(DipolarCouplingList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DipolarCouplingList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DipolarCouplingList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DipolarCouplingList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(DipolarCouplingList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
