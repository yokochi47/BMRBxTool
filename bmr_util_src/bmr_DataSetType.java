...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DataSetType.*;

public class bmr_DataSetType {

	private static final String table_name = "Data_set";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DataSetType body = DataSetType.Factory.newInstance();
		DataSet[] list = new DataSet[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:data_setCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DataSet', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DataSet.Factory.newInstance();
			...
			Logger lgr = Logger.getLogger(bmr_DataSetType.class.getName());
	...
	private static boolean set_string(DataSet list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DataSet', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DataSet', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DataSet list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(DataSet list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_DataSet.getType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ DataSet.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					DataSet.Type.Enum _enum = DataSet.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:DataSet method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= DataSet.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + DataSet.Type.Enum.forInt(i));
						try {
							errw.write("class_name:DataSet method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= DataSet.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + DataSet.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
