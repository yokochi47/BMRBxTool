...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EntityCommonNameType.*;

public class bmr_EntityCommonNameType {

	private static final String table_name = "Entity_common_name";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EntityCommonNameType body = EntityCommonNameType.Factory.newInstance();
		EntityCommonName[] list = new EntityCommonName[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:entity_common_nameCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EntityCommonName', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EntityCommonName.Factory.newInstance();
					...
					if (!set_string(list[0], "setName", "", true, rset.getString("Name"), logw)) {
						...
						try {
							logw.write("item='Name', category='EntityCommonName', value='" + rset.getString("Name") + "' was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					body.setEntityCommonNameArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_EntityCommonNameType.class.getName());
	...
	private static boolean set_string(EntityCommonName list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EntityCommonName', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EntityCommonName', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EntityCommonName list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
