...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ComputerType.*;

public class bmr_ComputerType {
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ComputerType body = ComputerType.Factory.newInstance();
		Computer[] list = new Computer[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:computerCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='Computer', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = Computer.Factory.newInstance();

					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setHardwareCode", "setNilHardwareCode", false, rset.getString("Hardware_code"), logw))
						continue;
					...
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_enum_operating_system(list[0], "setOperatingSystem", "", true, rset.getString("Operating_system"), logw, errw))
						continue;
					if (!set_string(list[0], "setOperatingSystemVersion", "", true, rset.getString("Operating_system_version"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_ComputerType.class.getName());
	...
	private static boolean set_string(Computer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='Computer', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='Computer', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(Computer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_operating_system(Computer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ Computer.OperatingSystem.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					Computer.OperatingSystem.Enum _enum = Computer.OperatingSystem.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:Computer method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= Computer.OperatingSystem.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + Computer.OperatingSystem.Enum.forInt(i));
						try {
							errw.write("class_name:Computer method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= Computer.OperatingSystem.Enum.table.lastInt(); i++)
								errw.write(" enum:" + Computer.OperatingSystem.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
