...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemShiftPerturbationType.*;

public class bmr_ChemShiftPerturbationType {

	private static final String table_name = "Chem_shift_perturbation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemShiftPerturbationType body = ChemShiftPerturbationType.Factory.newInstance();
		ChemShiftPerturbation[] list = new ChemShiftPerturbation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Chem_shift_perturbation_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:chem_shift_perturbationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemShiftPerturbation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemShiftPerturbation.Factory.newInstance();
					...
					if (!set_integer(list[0], "setCompIndexId", "", true, rset.getString("Comp_index_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setDifferenceChemShiftVal", "setNilDifferenceChemShiftVal", false, rset.getString("Difference_chem_shift_val"), logw))
						continue;
					if (!set_decimal(list[0], "setDifferenceChemShiftValErr", "setNilDifferenceChemShiftValErr", false, rset.getString("Difference_chem_shift_val_err"), logw))
						continue;
					...
					body.setChemShiftPerturbationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemShiftPerturbationType.class.getName());
	...
	private static boolean set_string(ChemShiftPerturbation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemShiftPerturbation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemShiftPerturbation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChemShiftPerturbation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(ChemShiftPerturbation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
