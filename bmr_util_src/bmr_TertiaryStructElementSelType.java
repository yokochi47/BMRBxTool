...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TertiaryStructElementSelType.*;

public class bmr_TertiaryStructElementSelType {

	private static final String table_name = "Tertiary_struct_element_sel";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TertiaryStructElementSelType body = TertiaryStructElementSelType.Factory.newInstance();
		TertiaryStructElementSel[] list = new TertiaryStructElementSel[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Tertiary_struct_element_list_ID\")::decimal");
				...
				buffw.write("  <BMRBx:tertiary_struct_element_selCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TertiaryStructElementSel', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TertiaryStructElementSel.Factory.newInstance();
					...
					body.setTertiaryStructElementSelArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TertiaryStructElementSelType.class.getName());
	...
	private static boolean set_string(TertiaryStructElementSel list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TertiaryStructElementSel', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TertiaryStructElementSel', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TertiaryStructElementSel list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
