...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.CsAnisotropyExperimentType.*;

public class bmr_CsAnisotropyExperimentType {

	private static final String table_name = "CS_anisotropy_experiment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		CsAnisotropyExperimentType body = CsAnisotropyExperimentType.Factory.newInstance();
		CsAnisotropyExperiment[] list = new CsAnisotropyExperiment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Chem_shift_anisotropy_ID\")::decimal,(0 || \"Experiment_ID\")::decimal");
				...
				buffw.write("  <BMRBx:cs_anisotropy_experimentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='CsAnisotropyExperiment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = CsAnisotropyExperiment.Factory.newInstance();
					...
					body.setCsAnisotropyExperimentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_CsAnisotropyExperimentType.class.getName());
	...
	private static boolean set_string(CsAnisotropyExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='CsAnisotropyExperiment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='CsAnisotropyExperiment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(CsAnisotropyExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_sample_state(CsAnisotropyExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ CsAnisotropyExperiment.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					CsAnisotropyExperiment.SampleState.Enum _enum = CsAnisotropyExperiment.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:CsAnisotropyExperiment method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= CsAnisotropyExperiment.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + CsAnisotropyExperiment.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:CsAnisotropyExperiment method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= CsAnisotropyExperiment.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + CsAnisotropyExperiment.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
