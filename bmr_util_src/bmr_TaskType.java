...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TaskType.*;

public class bmr_TaskType {
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TaskType body = TaskType.Factory.newInstance();
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:taskCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='Task', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = Task.Factory.newInstance();
					...
					if (!set_integer(list[0], "setSoftwareId", "", true, rset.getString("Software_ID"), logw))
						continue;
					if (!set_string(list[0], "setTask", "", true, rset.getString("Task"), logw))
						continue;

					body.setTaskArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TaskType.class.getName());
	...
	private static boolean set_string(Task list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='Task', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='Task', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(Task list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
