...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.FloatingChiralityType.*;

public class bmr_FloatingChiralityType {

	private static final String table_name = "Floating_chirality";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		FloatingChiralityType body = FloatingChiralityType.Factory.newInstance();
		FloatingChirality[] list = new FloatingChirality[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Floating_chirality_assign_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:floating_chiralityCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='FloatingChirality', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = FloatingChirality.Factory.newInstance();
					...
					if (!set_integer(list[0], "setSeqId2", "", true, rset.getString("Seq_ID_2"), logw))
						continue;
					if (!set_string(list[0], "setStereospecificAssignmentCode", "", true, rset.getString("Stereospecific_assignment_code"), logw))
						continue;
					...
					body.setFloatingChiralityArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_FloatingChiralityType.class.getName());
	...
	private static boolean set_string(FloatingChirality list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='FloatingChirality', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='FloatingChirality', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(FloatingChirality list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
