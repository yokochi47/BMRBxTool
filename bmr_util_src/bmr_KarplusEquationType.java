...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.KarplusEquationType.*;

public class bmr_KarplusEquationType {

	private static final String table_name = "Karplus_equation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		KarplusEquationType body = KarplusEquationType.Factory.newInstance();
		KarplusEquation[] list = new KarplusEquation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:karplus_equationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='KarplusEquation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = KarplusEquation.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_string(list[0], "setTorsionAngleCode", "", true, rset.getString("Torsion_angle_code"), logw))
						continue;
					if (!set_integer(list[0], "setTorsionAngleConstraintListId", "", true, rset.getString("Torsion_angle_constraint_list_ID"), logw))
						continue;

					body.setKarplusEquationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_KarplusEquationType.class.getName());
	...
	private static boolean set_string(KarplusEquation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='KarplusEquation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='KarplusEquation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(KarplusEquation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
