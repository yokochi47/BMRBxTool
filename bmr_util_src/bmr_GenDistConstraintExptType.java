...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.GenDistConstraintExptType.*;

public class bmr_GenDistConstraintExptType {

	private static final String table_name = "Gen_dist_constraint_expt";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		GenDistConstraintExptType body = GenDistConstraintExptType.Factory.newInstance();
		GenDistConstraintExpt[] list = new GenDistConstraintExpt[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Gen_dist_constraint_list_ID\")::decimal");
				...
				buffw.write("  <BMRBx:gen_dist_constraint_exptCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='GenDistConstraintExpt', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = GenDistConstraintExpt.Factory.newInstance();
					...
					body.setGenDistConstraintExptArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_GenDistConstraintExptType.class.getName());
	...
	private static boolean set_string(GenDistConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='GenDistConstraintExpt', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='GenDistConstraintExpt', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(GenDistConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_sample_state(GenDistConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ GenDistConstraintExpt.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					GenDistConstraintExpt.SampleState.Enum _enum = GenDistConstraintExpt.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:GenDistConstraintExpt method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= GenDistConstraintExpt.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + GenDistConstraintExpt.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:GenDistConstraintExpt method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= GenDistConstraintExpt.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + GenDistConstraintExpt.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
