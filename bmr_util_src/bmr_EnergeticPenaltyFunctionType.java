...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EnergeticPenaltyFunctionType.*;

public class bmr_EnergeticPenaltyFunctionType {

	private static final String table_name = "Energetic_penalty_function";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EnergeticPenaltyFunctionType body = EnergeticPenaltyFunctionType.Factory.newInstance();
		EnergeticPenaltyFunction[] list = new EnergeticPenaltyFunction[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:energetic_penalty_functionCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EnergeticPenaltyFunction', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EnergeticPenaltyFunction.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_string(list[0], "setFunction", "", true, rset.getString("Function"), logw))
						continue;

					body.setEnergeticPenaltyFunctionArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_EnergeticPenaltyFunctionType.class.getName());
	...
	private static boolean set_string(EnergeticPenaltyFunction list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EnergeticPenaltyFunction', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EnergeticPenaltyFunction', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EnergeticPenaltyFunction list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
