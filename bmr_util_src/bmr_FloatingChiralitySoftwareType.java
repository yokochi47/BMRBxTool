...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.FloatingChiralitySoftwareType.*;

public class bmr_FloatingChiralitySoftwareType {

	private static final String table_name = "Floating_chirality_software";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		FloatingChiralitySoftwareType body = FloatingChiralitySoftwareType.Factory.newInstance();
		FloatingChiralitySoftware[] list = new FloatingChiralitySoftware[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Floating_chirality_assign_ID\")::decimal,(0 || \"Software_ID\")::decimal");
				...
				buffw.write("  <BMRBx:floating_chirality_softwareCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='FloatingChiralitySoftware', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = FloatingChiralitySoftware.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setFloatingChiralityAssignId", "", true, rset.getString("Floating_chirality_assign_ID"), logw))
						continue;
					if (!set_integer_software_id(list[0], "setSoftwareId", "", true, rset.getString("Software_ID"), i++, logw))
						continue;

					body.setFloatingChiralitySoftwareArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_FloatingChiralitySoftwareType.class.getName());
	...
	private static boolean set_string(FloatingChiralitySoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='FloatingChiralitySoftware', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='FloatingChiralitySoftware', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(FloatingChiralitySoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_software_id(FloatingChiralitySoftware list, String method_name, String nil_method_name, boolean required, String val_name, int id, FileWriter logw) {
