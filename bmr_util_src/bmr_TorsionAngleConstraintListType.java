...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TorsionAngleConstraintListType.*;

public class bmr_TorsionAngleConstraintListType {

	private static final String table_name = "Torsion_angle_constraint_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TorsionAngleConstraintListType body = TorsionAngleConstraintListType.Factory.newInstance();
		TorsionAngleConstraintList[] list = new TorsionAngleConstraintList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:torsion_angle_constraint_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TorsionAngleConstraintList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TorsionAngleConstraintList.Factory.newInstance();
					...
					body.setTorsionAngleConstraintListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TorsionAngleConstraintListType.class.getName());
	...
	private static boolean set_string(TorsionAngleConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TorsionAngleConstraintList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TorsionAngleConstraintList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TorsionAngleConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_constraint_type(TorsionAngleConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ TorsionAngleConstraintList.ConstraintType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					TorsionAngleConstraintList.ConstraintType.Enum _enum = TorsionAngleConstraintList.ConstraintType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:TorsionAngleConstraintList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= TorsionAngleConstraintList.ConstraintType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + TorsionAngleConstraintList.ConstraintType.Enum.forInt(i));
						try {
							errw.write("class_name:TorsionAngleConstraintList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= TorsionAngleConstraintList.ConstraintType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + TorsionAngleConstraintList.ConstraintType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_potential_type(TorsionAngleConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ TorsionAngleConstraintList.PotentialType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					TorsionAngleConstraintList.PotentialType.Enum _enum = TorsionAngleConstraintList.PotentialType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:TorsionAngleConstraintList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= TorsionAngleConstraintList.PotentialType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + TorsionAngleConstraintList.PotentialType.Enum.forInt(i));
						try {
							errw.write("class_name:TorsionAngleConstraintList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= TorsionAngleConstraintList.PotentialType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + TorsionAngleConstraintList.PotentialType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
