...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalCouplingConstantListType.*;

public class bmr_TheoreticalCouplingConstantListType {

	private static final String table_name = "Theoretical_coupling_constant_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalCouplingConstantListType body = TheoreticalCouplingConstantListType.Factory.newInstance();
		TheoreticalCouplingConstantList[] list = new TheoreticalCouplingConstantList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:theoretical_coupling_constant_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalCouplingConstantList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalCouplingConstantList.Factory.newInstance();
					...
					body.setTheoreticalCouplingConstantListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalCouplingConstantListType.class.getName());
	...
	private static boolean set_string(TheoreticalCouplingConstantList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalCouplingConstantList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalCouplingConstantList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalCouplingConstantList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_spectrometer_frequency_1h(TheoreticalCouplingConstantList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
