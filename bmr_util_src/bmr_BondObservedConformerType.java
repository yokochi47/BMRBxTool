...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.BondObservedConformerType.*;

public class bmr_BondObservedConformerType {

	private static final String table_name = "Bond_observed_conformer";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		BondObservedConformerType body = BondObservedConformerType.Factory.newInstance();
		BondObservedConformer[] list = new BondObservedConformer[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Bond_annotation_list_ID\")::decimal,(0 || \"Conformer_family_coord_set_ID\")::decimal,(0 || \"Bond_annotation_ID\")::decimal");
				...
				buffw.write("  <BMRBx:bond_observed_conformerCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='BondObservedConformer', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = BondObservedConformer.Factory.newInstance();

					if (!set_integer(list[0], "setAtomSiteModelId", "", true, rset.getString("Atom_site_model_ID"), logw))
						continue;
					if (!set_integer(list[0], "setBondAnnotationId", "", true, rset.getString("Bond_annotation_ID"), logw))
						continue;
					...
					body.setBondObservedConformerArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_BondObservedConformerType.class.getName());
	...
	private static boolean set_string(BondObservedConformer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='BondObservedConformer', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='BondObservedConformer', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(BondObservedConformer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
