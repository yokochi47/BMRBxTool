...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SubsystemDbLinkType.*;

public class bmr_SubsystemDbLinkType {

	private static final String table_name = "Subsystem_db_link";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SubsystemDbLinkType body = SubsystemDbLinkType.Factory.newInstance();
		SubsystemDbLink[] list = new SubsystemDbLink[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Assembly_subsystem_ID\")::decimal");
				...
				buffw.write("  <BMRBx:subsystem_db_linkCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SubsystemDbLink', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SubsystemDbLink.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntryDetails", "setNilEntryDetails", false, rset.getString("Entry_details"), logw))
						continue;
					if (!set_string(list[0], "setEntryExperimentalMethod", "setNilEntryExperimentalMethod", false, rset.getString("Entry_experimental_method"), logw))
						continue;
					...
					if (!set_string(list[0], "setEntryRelationType", "setNilEntryRelationType", false, rset.getString("Entry_relation_type"), logw))
						continue;
					if (!set_string(list[0], "setAccessionCode", "", true, rset.getString("Accession_code"), logw))
						continue;
					if (!set_integer(list[0], "setAssemblySubsystemId", "", true, rset.getString("Assembly_subsystem_ID"), logw))
						continue;
					if (!set_string(list[0], "setDatabaseCode", "", true, rset.getString("Database_code"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;

					body.setSubsystemDbLinkArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SubsystemDbLinkType.class.getName());
	...
	private static boolean set_string(SubsystemDbLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SubsystemDbLink', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SubsystemDbLink', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SubsystemDbLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_author_supplied(SubsystemDbLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ SubsystemDbLink.AuthorSupplied.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					SubsystemDbLink.AuthorSupplied.Enum _enum = SubsystemDbLink.AuthorSupplied.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:SubsystemDbLink method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= SubsystemDbLink.AuthorSupplied.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + SubsystemDbLink.AuthorSupplied.Enum.forInt(i));
						try {
							errw.write("class_name:SubsystemDbLink method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= SubsystemDbLink.AuthorSupplied.Enum.table.lastInt(); i++)
								errw.write(" enum:" + SubsystemDbLink.AuthorSupplied.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
