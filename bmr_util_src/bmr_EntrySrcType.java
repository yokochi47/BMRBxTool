...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EntrySrcType.*;

public class bmr_EntrySrcType {

	private static final String table_name = "Entry_src";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EntrySrcType body = EntrySrcType.Factory.newInstance();
		EntrySrc[] list = new EntrySrc[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:entry_srcCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EntrySrc', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EntrySrc.Factory.newInstance();

					if (!set_string(list[0], "setOrganizationFullName", "setNilOrganizationFullName", false, rset.getString("Organization_full_name"), logw))
						continue;
					if (!set_string(list[0], "setOrganizationInitials", "setNilOrganizationInitials", false, rset.getString("Organization_initials"), logw))
						continue;
					if (!set_string(list[0], "setProjectName", "setNilProjectName", false, rset.getString("Project_name"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_EntrySrcType.class.getName());
	...
	private static boolean set_string(EntrySrc list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EntrySrc', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EntrySrc', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EntrySrc list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
