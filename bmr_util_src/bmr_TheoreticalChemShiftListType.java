...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalChemShiftListType.*;

public class bmr_TheoreticalChemShiftListType {

	private static final String table_name = "Theoretical_chem_shift_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalChemShiftListType body = TheoreticalChemShiftListType.Factory.newInstance();
		TheoreticalChemShiftList[] list = new TheoreticalChemShiftList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:theoretical_chem_shift_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalChemShiftList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalChemShiftList.Factory.newInstance();

					if (!set_decimal(list[0], "setChemShift13CErr", "setNilChemShift13CErr", false, rset.getString("Chem_shift_13C_err"), logw))
						continue;
					if (!set_decimal(list[0], "setChemShift15NErr", "setNilChemShift15NErr", false, rset.getString("Chem_shift_15N_err"), logw))
						continue;
					if (!set_decimal(list[0], "setChemShift19FErr", "setNilChemShift19FErr", false, rset.getString("Chem_shift_19F_err"), logw))
						continue;
					if (!set_decimal(list[0], "setChemShift1HErr", "setNilChemShift1HErr", false, rset.getString("Chem_shift_1H_err"), logw))
						continue;
					if (!set_decimal(list[0], "setChemShift2HErr", "setNilChemShift2HErr", false, rset.getString("Chem_shift_2H_err"), logw))
						continue;
					if (!set_decimal(list[0], "setChemShift31PErr", "setNilChemShift31PErr", false, rset.getString("Chem_shift_31P_err"), logw))
						continue;
					if (!set_integer(list[0], "setChemShiftsCalcTypeId", "", false, rset.getString("Chem_shifts_calc_type_ID"), logw))
						continue;
					if (!set_string(list[0], "setChemShiftsCalcTypeLabel", "setNilChemShiftsCalcTypeLabel", false, rset.getString("Chem_shifts_calc_type_label"), logw))
						continue;
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setFermiContactSpinDensityUnits", "setNilFermiContactSpinDensityUnits", false, rset.getString("Fermi_contact_spin_density_units"), logw))
						continue;
					if (!set_integer(list[0], "setModelAtomicCoordinatesId", "", false, rset.getString("Model_atomic_coordinates_ID"), logw))
						continue;
					if (!set_string(list[0], "setModelAtomicCoordinatesLabel", "setNilModelAtomicCoordinatesLabel", false, rset.getString("Model_atomic_coordinates_label"), logw))
						continue;
					...
					if (!set_integer(list[0], "setTensorListId", "", true, rset.getString("Tensor_list_ID"), logw))
						continue;
					if (!set_string(list[0], "setTensorListLabel", "setNilTensorListLabel", false, rset.getString("Tensor_list_label"), logw))
						continue;
					...
					body.setTheoreticalChemShiftListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalChemShiftListType.class.getName());
	...
	private static boolean set_string(TheoreticalChemShiftList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalChemShiftList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalChemShiftList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalChemShiftList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(TheoreticalChemShiftList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
