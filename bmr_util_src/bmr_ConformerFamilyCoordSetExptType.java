...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ConformerFamilyCoordSetExptType.*;

public class bmr_ConformerFamilyCoordSetExptType {

	private static final String table_name = "Conformer_family_coord_set_expt";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ConformerFamilyCoordSetExptType body = ConformerFamilyCoordSetExptType.Factory.newInstance();
		ConformerFamilyCoordSetExpt[] list = new ConformerFamilyCoordSetExpt[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Conformer_family_coord_set_ID\")::decimal");
				...
				buffw.write("  <BMRBx:conformer_family_coord_set_exptCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ConformerFamilyCoordSetExpt', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ConformerFamilyCoordSetExpt.Factory.newInstance();
					...
					body.setConformerFamilyCoordSetExptArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ConformerFamilyCoordSetExptType.class.getName());
	...
	private static boolean set_string(ConformerFamilyCoordSetExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ConformerFamilyCoordSetExpt', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ConformerFamilyCoordSetExpt', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ConformerFamilyCoordSetExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_experiment_id(ConformerFamilyCoordSetExpt list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String sample_id, FileWriter logw) {
	...
	private static boolean set_integer_sample_id(ConformerFamilyCoordSetExpt list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String experiment_id, FileWriter logw) {
	...
	private static boolean set_enum_sample_state(ConformerFamilyCoordSetExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ConformerFamilyCoordSetExpt.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ConformerFamilyCoordSetExpt.SampleState.Enum _enum = ConformerFamilyCoordSetExpt.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ConformerFamilyCoordSetExpt method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ConformerFamilyCoordSetExpt.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ConformerFamilyCoordSetExpt.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:ConformerFamilyCoordSetExpt method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ConformerFamilyCoordSetExpt.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ConformerFamilyCoordSetExpt.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
