...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.GenDistConstraintConvErrType.*;

public class bmr_GenDistConstraintConvErrType {

	private static final String table_name = "Gen_dist_constraint_conv_err";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		GenDistConstraintConvErrType body = GenDistConstraintConvErrType.Factory.newInstance();
		GenDistConstraintConvErr[] list = new GenDistConstraintConvErr[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:gen_dist_constraint_conv_errCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='GenDistConstraintConvErr', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = GenDistConstraintConvErr.Factory.newInstance();
					...
					if (!set_string(list[0], "setConvErrorType", "", true, rset.getString("Conv_error_type"), logw))
						continue;
					if (!set_integer(list[0], "setGenDistConstraintParseFileId", "", true, rset.getString("Gen_dist_constraint_parse_file_ID"), logw))
						continue;
					...
					body.setGenDistConstraintConvErrArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_GenDistConstraintConvErrType.class.getName());
	...
	private static boolean set_string(GenDistConstraintConvErr list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='GenDistConstraintConvErr', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='GenDistConstraintConvErr', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(GenDistConstraintConvErr list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
