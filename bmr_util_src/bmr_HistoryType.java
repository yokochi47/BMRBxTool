...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HistoryType.*;

public class bmr_HistoryType {
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HistoryType body = HistoryType.Factory.newInstance();
		History[] list = new History[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Software_applied_history_ID\")::decimal");
				...
				buffw.write("  <BMRBx:historyCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='History', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = History.Factory.newInstance();
					...
					if (!set_enum_software_name(list[0], "setSoftwareName", "", true, rset.getString("Software_name"), logw, errw))
						continue;
					if (!set_string(list[0], "setSoftwareVersion", "setNilSoftwareVersion", false, rset.getString("Software_version"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setRunNumber", "", true, rset.getString("Run_number"), logw))
						continue;
					if (!set_integer(list[0], "setSoftwareAppliedHistoryId", "", true, rset.getString("Software_applied_history_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSoftwareId", "", true, rset.getString("Software_ID"), logw))
						continue;

					body.setHistoryArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HistoryType.class.getName());
	...
	private static boolean set_string(History list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='History', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='History', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(History list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_software_name(History list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ History.SoftwareName.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					History.SoftwareName.Enum _enum = History.SoftwareName.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:History method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= History.SoftwareName.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + History.SoftwareName.Enum.forInt(i));
						try {
							errw.write("class_name:History method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= History.SoftwareName.Enum.table.lastInt(); i++)
								errw.write(" enum:" + History.SoftwareName.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
