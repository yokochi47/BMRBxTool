...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MassSpecRefCompdType.*;

public class bmr_MassSpecRefCompdType {

	private static final String table_name = "Mass_spec_ref_compd";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MassSpecRefCompdType body = MassSpecRefCompdType.Factory.newInstance();
		MassSpecRefCompd[] list = new MassSpecRefCompd[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Mass_spec_ref_compd_set_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:mass_spec_ref_compdCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MassSpecRefCompd', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MassSpecRefCompd.Factory.newInstance();

					if (!set_decimal(list[0], "setMonoIsotopicMass", "setNilMonoIsotopicMass", false, rset.getString("Mono_isotopic_mass"), logw))
						continue;
					...
					body.setMassSpecRefCompdArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MassSpecRefCompdType.class.getName());
	...
	private static boolean set_string(MassSpecRefCompd list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MassSpecRefCompd', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MassSpecRefCompd', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MassSpecRefCompd list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(MassSpecRefCompd list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
