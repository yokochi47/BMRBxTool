...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalCrossCorrelationDdSoftwareType.*;

public class bmr_TheoreticalCrossCorrelationDdSoftwareType {

	private static final String table_name = "Theoretical_cross_correlation_DD_software";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalCrossCorrelationDdSoftwareType body = TheoreticalCrossCorrelationDdSoftwareType.Factory.newInstance();
		TheoreticalCrossCorrelationDdSoftware[] list = new TheoreticalCrossCorrelationDdSoftware[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Theoretical_cross_correlation_DD_list_ID\")::decimal,(0 || \"Software_ID\")::decimal");
				...
				buffw.write("  <BMRBx:theoretical_cross_correlation_dd_softwareCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalCrossCorrelationDdSoftware', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalCrossCorrelationDdSoftware.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer_software_id(list[0], "setSoftwareId", "", true, rset.getString("Software_ID"), i++, logw))
						continue;
					...
					body.setTheoreticalCrossCorrelationDdSoftwareArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalCrossCorrelationDdSoftwareType.class.getName());
	...
	private static boolean set_string(TheoreticalCrossCorrelationDdSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalCrossCorrelationDdSoftware', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalCrossCorrelationDdSoftware', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalCrossCorrelationDdSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_software_id(TheoreticalCrossCorrelationDdSoftware list, String method_name, String nil_method_name, boolean required, String val_name, int id, FileWriter logw) {
