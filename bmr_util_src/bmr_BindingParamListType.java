...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.BindingParamListType.*;

public class bmr_BindingParamListType {

	private static final String table_name = "Binding_param_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		BindingParamListType body = BindingParamListType.Factory.newInstance();
		BindingParamList[] list = new BindingParamList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:binding_param_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='BindingParamList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = BindingParamList.Factory.newInstance();

					if (!set_integer(list[0], "setBindingValueListId", "", true, rset.getString("Binding_value_list_ID"), logw))
						continue;
					if (!set_string(list[0], "setBindingValueListLabel", "", true, rset.getString("Binding_value_list_label"), logw))
						continue;
					if (!set_string(list[0], "setConcentrationValUnits", "", true, rset.getString("Concentration_val_units"), logw))
						continue;
					...
					body.setBindingParamListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_BindingParamListType.class.getName());
	...
	private static boolean set_string(BindingParamList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='BindingParamList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='BindingParamList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(BindingParamList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
