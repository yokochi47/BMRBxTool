...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DipolarCouplingType.*;

public class bmr_DipolarCouplingType {

	private static final String table_name = "Dipolar_coupling";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DipolarCouplingType body = DipolarCouplingType.Factory.newInstance();
		DipolarCoupling[] list = new DipolarCoupling[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Dipolar_coupling_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:dipolar_couplingCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DipolarCoupling', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DipolarCoupling.Factory.newInstance();

					if (!set_string(list[0], "setAmbiguityCode1", "setNilAmbiguityCode1", false, rset.getString("Ambiguity_code_1"), logw))
						continue;
					if (!set_string(list[0], "setAmbiguityCode2", "setNilAmbiguityCode2", false, rset.getString("Ambiguity_code_2"), logw))
						continue;
					...
					if (!set_integer(list[0], "setCompIndexId2", "", true, rset.getString("Comp_index_ID_2"), logw))
						continue;
					if (!set_string(list[0], "setDipolarCouplingCode", "", true, rset.getString("Dipolar_coupling_code"), logw))
						continue;
					...
					if (!set_decimal(list[0], "setValErr", "setNilValErr", false, rset.getString("Val_err"), logw))
						continue;
					if (!set_decimal(list[0], "setValMax", "setNilValMax", false, rset.getString("Val_max"), logw))
						continue;
					if (!set_decimal(list[0], "setValMin", "setNilValMin", false, rset.getString("Val_min"), logw))
						continue;
					...
					body.setDipolarCouplingArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DipolarCouplingType.class.getName());
	...
	private static boolean set_string(DipolarCoupling list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DipolarCoupling', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DipolarCoupling', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DipolarCoupling list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(DipolarCoupling list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
