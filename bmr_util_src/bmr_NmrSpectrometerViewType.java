...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.NmrSpectrometerViewType.*;

public class bmr_NmrSpectrometerViewType {

	private static final String table_name = "NMR_spectrometer_view";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		NmrSpectrometerViewType body = NmrSpectrometerViewType.Factory.newInstance();
		NmrSpectrometerView[] list = new NmrSpectrometerView[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"NMR_spectrometer_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:nmr_spectrometer_viewCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='NmrSpectrometerView', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = NmrSpectrometerView.Factory.newInstance();
					...
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_enum_field_strength(list[0], "setFieldStrength", "", true, rset.getString("Field_strength"), logw, errw))
						continue;
					...
					if (!set_string(list[0], "setName", "", true, rset.getString("Name"), logw))
						continue;
					if (!set_integer(list[0], "setNmrSpectrometerListId", "", true, rset.getString("NMR_spectrometer_list_ID"), logw))
						continue;

					body.setNmrSpectrometerViewArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_NmrSpectrometerViewType.class.getName());
	...
	private static boolean set_string(NmrSpectrometerView list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='NmrSpectrometerView', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='NmrSpectrometerView', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(NmrSpectrometerView list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_field_strength(NmrSpectrometerView list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_NMRSpectrometerView.getFieldStrength(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrSpectrometerView.FieldStrength.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrSpectrometerView.FieldStrength.Enum _enum = NmrSpectrometerView.FieldStrength.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrSpectrometerView method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrSpectrometerView.FieldStrength.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrSpectrometerView.FieldStrength.Enum.forInt(i));
						try {
							errw.write("class_name:NmrSpectrometerView method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrSpectrometerView.FieldStrength.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrSpectrometerView.FieldStrength.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_manufacturer(NmrSpectrometerView list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_NMRSpectrometerView.getManufacturer(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrSpectrometerView.Manufacturer.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrSpectrometerView.Manufacturer.Enum _enum = NmrSpectrometerView.Manufacturer.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrSpectrometerView method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrSpectrometerView.Manufacturer.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrSpectrometerView.Manufacturer.Enum.forInt(i));
						try {
							errw.write("class_name:NmrSpectrometerView method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrSpectrometerView.Manufacturer.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrSpectrometerView.Manufacturer.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
