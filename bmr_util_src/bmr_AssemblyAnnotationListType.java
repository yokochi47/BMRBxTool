...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyAnnotationListType.*;

public class bmr_AssemblyAnnotationListType {

	private static final String table_name = "Assembly_annotation_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssemblyAnnotationListType body = AssemblyAnnotationListType.Factory.newInstance();
		AssemblyAnnotationList[] list = new AssemblyAnnotationList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:assembly_annotation_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssemblyAnnotationList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AssemblyAnnotationList.Factory.newInstance();
					...
					body.setAssemblyAnnotationListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssemblyAnnotationListType.class.getName());
	...
	private static boolean set_string(AssemblyAnnotationList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssemblyAnnotationList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssemblyAnnotationList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AssemblyAnnotationList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
