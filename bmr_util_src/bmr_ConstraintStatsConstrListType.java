...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ConstraintStatsConstrListType.*;

public class bmr_ConstraintStatsConstrListType {

	private static final String table_name = "Constraint_stats_constr_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ConstraintStatsConstrListType body = ConstraintStatsConstrListType.Factory.newInstance();
		ConstraintStatsConstrList[] list = new ConstraintStatsConstrList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Constraint_stat_list_ID\")::decimal,(0 || \"Constraint_list_ID\")::decimal");
				...
				buffw.write("  <BMRBx:constraint_stats_constr_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ConstraintStatsConstrList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ConstraintStatsConstrList.Factory.newInstance();
					...
					body.setConstraintStatsConstrListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ConstraintStatsConstrListType.class.getName());
	...
	private static boolean set_string(ConstraintStatsConstrList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ConstraintStatsConstrList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ConstraintStatsConstrList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ConstraintStatsConstrList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
