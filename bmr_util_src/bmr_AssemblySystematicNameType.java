...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblySystematicNameType.*;

public class bmr_AssemblySystematicNameType {

	private static final String table_name = "Assembly_systematic_name";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssemblySystematicNameType body = AssemblySystematicNameType.Factory.newInstance();
		AssemblySystematicName[] list = new AssemblySystematicName[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:assembly_systematic_nameCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssemblySystematicName', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AssemblySystematicName.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_enum_naming_system(list[0], "setNamingSystem", "", true, rset.getString("Naming_system"), logw, errw)) {

						try {
							logw.write("item='NamingSystem', category='AssemblySystematicName', value='" + rset.getString("Naming_system") + "' was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					body.setAssemblySystematicNameArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssemblySystematicNameType.class.getName());
	...
	private static boolean set_string(AssemblySystematicName list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssemblySystematicName', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssemblySystematicName', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AssemblySystematicName list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_naming_system(AssemblySystematicName list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_AssemblySystematicName.getNamingSystem(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ AssemblySystematicName.NamingSystem.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					AssemblySystematicName.NamingSystem.Enum _enum = AssemblySystematicName.NamingSystem.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:AssemblySystematicName method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= AssemblySystematicName.NamingSystem.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + AssemblySystematicName.NamingSystem.Enum.forInt(i));
						try {
							errw.write("class_name:AssemblySystematicName method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= AssemblySystematicName.NamingSystem.Enum.table.lastInt(); i++)
								errw.write(" enum:" + AssemblySystematicName.NamingSystem.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
