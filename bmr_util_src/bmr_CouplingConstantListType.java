...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.CouplingConstantListType.*;

public class bmr_CouplingConstantListType {

	private static final String table_name = "Coupling_constant_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		CouplingConstantListType body = CouplingConstantListType.Factory.newInstance();
		CouplingConstantList[] list = new CouplingConstantList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:coupling_constant_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='CouplingConstantList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = CouplingConstantList.Factory.newInstance();
					...
					if (!set_string(list[0], "setSfFramecode", "", true, rset.getString("Sf_framecode"), logw))
						continue;
					if (!set_enum_spectrometer_frequency_1h(list[0], "setSpectrometerFrequency1H", "setNilSpectrometerFrequency1H", false, rset.getString("Spectrometer_frequency_1H"), logw, errw))
						continue;
					...
					body.setCouplingConstantListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_CouplingConstantListType.class.getName());
	...
	private static boolean set_string(CouplingConstantList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='CouplingConstantList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='CouplingConstantList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(CouplingConstantList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_sample_condition_list_id(CouplingConstantList list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, FileWriter logw) {
	...
	private static boolean set_enum_spectrometer_frequency_1h(CouplingConstantList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (bmr_Util_Main.remediate_xml && val_name != null)
			val_name = bmr_Util_CouplingConstatnList.getSpectrometerFrequency1H(val_name);
