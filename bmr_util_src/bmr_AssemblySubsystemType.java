...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblySubsystemType.*;

public class bmr_AssemblySubsystemType {

	private static final String table_name = "Assembly_subsystem";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssemblySubsystemType body = AssemblySubsystemType.Factory.newInstance();
		AssemblySubsystem[] list = new AssemblySubsystem[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:assembly_subsystemCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssemblySubsystem', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AssemblySubsystem.Factory.newInstance();

					if (!set_string(list[0], "setCasName", "setNilCasName", false, rset.getString("CAS_name"), logw))
						continue;
					if (!set_string(list[0], "setCasRegistryNumber", "setNilCasRegistryNumber", false, rset.getString("CAS_registry_number"), logw))
						continue;
					...
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setEnzymeCommissionNumber", "setNilEnzymeCommissionNumber", false, rset.getString("Enzyme_commission_number"), logw))
						continue;
					...
					body.setAssemblySubsystemArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssemblySubsystemType.class.getName());
	...
	private static boolean set_string(AssemblySubsystem list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssemblySubsystem', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssemblySubsystem', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AssemblySubsystem list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(AssemblySubsystem list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_date(AssemblySubsystem list, String method_name, String nil_method_name, boolean required, java.sql.Date date, FileWriter logw) {
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssemblySubsystem', value='" + date + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
