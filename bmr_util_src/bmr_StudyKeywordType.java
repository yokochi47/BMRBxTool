...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.StudyKeywordType.*;

public class bmr_StudyKeywordType {

	private static final String table_name = "Study_keyword";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		StudyKeywordType body = StudyKeywordType.Factory.newInstance();
		StudyKeyword[] list = new StudyKeyword[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:study_keywordCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='StudyKeyword', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = StudyKeyword.Factory.newInstance();

					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_string(list[0], "setKeyword", "", true, rset.getString("Keyword"), logw))
						continue;
					...
					body.setStudyKeywordArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_StudyKeywordType.class.getName());
	...
	private static boolean set_string(StudyKeyword list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='StudyKeyword', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='StudyKeyword', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(StudyKeyword list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_study_id(StudyKeyword list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String study_list_id, FileWriter logw) {
