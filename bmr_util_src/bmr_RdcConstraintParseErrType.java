...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.RdcConstraintParseErrType.*;

public class bmr_RdcConstraintParseErrType {

	private static final String table_name = "RDC_constraint_parse_err";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		RdcConstraintParseErrType body = RdcConstraintParseErrType.Factory.newInstance();
		RdcConstraintParseErr[] list = new RdcConstraintParseErr[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:rdc_constraint_parse_errCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='RdcConstraintParseErr', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = RdcConstraintParseErr.Factory.newInstance();
					...
					body.setRdcConstraintParseErrArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_RdcConstraintParseErrType.class.getName());
	...
	private static boolean set_string(RdcConstraintParseErr list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='RdcConstraintParseErr', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='RdcConstraintParseErr', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(RdcConstraintParseErr list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
