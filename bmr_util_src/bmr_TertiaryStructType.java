...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TertiaryStructType.*;

public class bmr_TertiaryStructType {

	private static final String table_name = "Tertiary_struct";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TertiaryStructType body = TertiaryStructType.Factory.newInstance();
		TertiaryStruct[] list = new TertiaryStruct[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Tertiary_struct_element_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:tertiary_structCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TertiaryStruct', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TertiaryStruct.Factory.newInstance();
					...
					if (!set_integer(list[0], "setSeqId", "", true, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_string(list[0], "setTertiaryStructCode", "setNilTertiaryStructCode", false, rset.getString("Tertiary_struct_code"), logw))
						continue;
					if (!set_string(list[0], "setTertiaryStructElementCode", "", true, rset.getString("Tertiary_struct_element_code"), logw))
						continue;
					if (!set_string(list[0], "setTertiaryStructName", "", true, rset.getString("Tertiary_struct_name"), logw))
						continue;
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setTertiaryStructElementListId", "", true, rset.getString("Tertiary_struct_element_list_ID"), logw))
						continue;

					body.setTertiaryStructArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TertiaryStructType.class.getName());
	...
	private static boolean set_string(TertiaryStruct list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TertiaryStruct', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TertiaryStruct', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TertiaryStruct list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
