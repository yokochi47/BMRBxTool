...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.RepConfRefinementType.*;

public class bmr_RepConfRefinementType {

	private static final String table_name = "Rep_conf_refinement";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		RepConfRefinementType body = RepConfRefinementType.Factory.newInstance();
		RepConfRefinement[] list = new RepConfRefinement[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:rep_conf_refinementCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='RepConfRefinement', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = RepConfRefinement.Factory.newInstance();
					...
					body.setRepConfRefinementArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_RepConfRefinementType.class.getName());
	...
	private static boolean set_string(RepConfRefinement list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='RepConfRefinement', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='RepConfRefinement', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(RepConfRefinement list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_refine_method(RepConfRefinement list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_RepConfRefinement.getRefineMethod(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ RepConfRefinement.RefineMethod.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					RepConfRefinement.RefineMethod.Enum _enum = RepConfRefinement.RefineMethod.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:RepConfRefinement method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= RepConfRefinement.RefineMethod.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + RepConfRefinement.RefineMethod.Enum.forInt(i));
						try {
							errw.write("class_name:RepConfRefinement method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= RepConfRefinement.RefineMethod.Enum.table.lastInt(); i++)
								errw.write(" enum:" + RepConfRefinement.RefineMethod.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
