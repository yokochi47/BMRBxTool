...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.NmrProbeType.*;

public class bmr_NmrProbeType {

	private static final String table_name = "NMR_probe";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		NmrProbeType body = NmrProbeType.Factory.newInstance();
		NmrProbe[] list = new NmrProbe[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"NMR_spectrometer_probe_ID\")::decimal");
				...
				buffw.write("  <BMRBx:nmr_probeCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='NmrProbe', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = NmrProbe.Factory.newInstance();

					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setNmrSpectrometerProbeId", "", true, rset.getString("NMR_spectrometer_probe_ID"), logw))
						continue;
					if (!set_enum_type(list[0], "setType", "", true, rset.getString("Type"), logw, errw))
						continue;

					body.setNmrProbeArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_NmrProbeType.class.getName());
	...
	private static boolean set_string(NmrProbe list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='NmrProbe', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='NmrProbe', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(NmrProbe list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(NmrProbe list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_NMRProbe.getType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrProbe.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrProbe.Type.Enum _enum = NmrProbe.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrProbe method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrProbe.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrProbe.Type.Enum.forInt(i));
						try {
							errw.write("class_name:NmrProbe method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrProbe.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrProbe.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
