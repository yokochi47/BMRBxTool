...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DatumType.*;

public class bmr_DatumType {

	private static final String table_name = "Datum";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DatumType body = DatumType.Factory.newInstance();
		Datum[] list = new Datum[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:datumCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='Datum', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = Datum.Factory.newInstance();
			...
			Logger lgr = Logger.getLogger(bmr_DatumType.class.getName());
	...
	private static boolean set_string(Datum list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='Datum', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='Datum', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(Datum list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(Datum list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_Datum.getType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ Datum.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					Datum.Type.Enum _enum = Datum.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:Datum method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= Datum.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + Datum.Type.Enum.forInt(i));
						try {
							errw.write("class_name:Datum method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= Datum.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + Datum.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
