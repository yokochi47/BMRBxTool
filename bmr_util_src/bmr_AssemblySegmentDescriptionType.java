...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblySegmentDescriptionType.*;

public class bmr_AssemblySegmentDescriptionType {

	private static final String table_name = "Assembly_segment_description";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssemblySegmentDescriptionType body = AssemblySegmentDescriptionType.Factory.newInstance();
		AssemblySegmentDescription[] list = new AssemblySegmentDescription[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Assembly_segment_ID\")::decimal,(0 || \"Assembly_ID\")::decimal");
				...
				buffw.write("  <BMRBx:assembly_segment_descriptionCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssemblySegmentDescription', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AssemblySegmentDescription.Factory.newInstance();
					...
					if (!set_integer(list[0], "setAssemblyId", "", true, rset.getString("Assembly_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAssemblySegmentId", "", true, rset.getString("Assembly_segment_ID"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;

					body.setAssemblySegmentDescriptionArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssemblySegmentDescriptionType.class.getName());
	...
	private static boolean set_string(AssemblySegmentDescription list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssemblySegmentDescription', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssemblySegmentDescription', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AssemblySegmentDescription list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
