...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.NmrSpectrometerProbeType.*;

public class bmr_NmrSpectrometerProbeType {

	private static final String table_name = "NMR_spectrometer_probe";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		NmrSpectrometerProbeType body = NmrSpectrometerProbeType.Factory.newInstance();
		NmrSpectrometerProbe[] list = new NmrSpectrometerProbe[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:nmr_spectrometer_probeCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='NmrSpectrometerProbe', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = NmrSpectrometerProbe.Factory.newInstance();

					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_decimal(list[0], "setDiameter", "setNilDiameter", false, rset.getString("Diameter"), logw))
						continue;
					if (!set_decimal(list[0], "setInternalVolume", "setNilInternalVolume", false, rset.getString("Internal_volume"), logw))
						continue;
					...
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_string(list[0], "setRotorComposition", "setNilRotorComposition", false, rset.getString("Rotor_composition"), logw))
						continue;
					if (!set_decimal(list[0], "setRotorLength", "setNilRotorLength", false, rset.getString("Rotor_length"), logw))
						continue;
					...
					if (!set_string(list[0], "setSfFramecode", "", true, rset.getString("Sf_framecode"), logw))
						continue;
					if (!set_enum_spacer_present(list[0], "setSpacerPresent", "setNilSpacerPresent", false, rset.getString("Spacer_present"), logw, errw))
						continue;
					...
					body.setNmrSpectrometerProbeArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_NmrSpectrometerProbeType.class.getName());
	...
	private static boolean set_string(NmrSpectrometerProbe list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='NmrSpectrometerProbe', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='NmrSpectrometerProbe', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(NmrSpectrometerProbe list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(NmrSpectrometerProbe list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_manufacturer(NmrSpectrometerProbe list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_NMRSpectrometerProbe.getManufacturer(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrSpectrometerProbe.Manufacturer.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrSpectrometerProbe.Manufacturer.Enum _enum = NmrSpectrometerProbe.Manufacturer.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrSpectrometerProbe method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrSpectrometerProbe.Manufacturer.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrSpectrometerProbe.Manufacturer.Enum.forInt(i));
						try {
							errw.write("class_name:NmrSpectrometerProbe method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrSpectrometerProbe.Manufacturer.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrSpectrometerProbe.Manufacturer.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_spacer_present(NmrSpectrometerProbe list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrSpectrometerProbe.SpacerPresent.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrSpectrometerProbe.SpacerPresent.Enum _enum = NmrSpectrometerProbe.SpacerPresent.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrSpectrometerProbe method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrSpectrometerProbe.SpacerPresent.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrSpectrometerProbe.SpacerPresent.Enum.forInt(i));
						try {
							errw.write("class_name:NmrSpectrometerProbe method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrSpectrometerProbe.SpacerPresent.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrSpectrometerProbe.SpacerPresent.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
