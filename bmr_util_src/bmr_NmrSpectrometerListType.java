...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.NmrSpectrometerListType.*;

public class bmr_NmrSpectrometerListType {

	private static final String table_name = "NMR_spectrometer_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		NmrSpectrometerListType body = NmrSpectrometerListType.Factory.newInstance();
		NmrSpectrometerList[] list = new NmrSpectrometerList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:nmr_spectrometer_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='NmrSpectrometerList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = NmrSpectrometerList.Factory.newInstance();
					...
					body.setNmrSpectrometerListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_NmrSpectrometerListType.class.getName());
	...
	private static boolean set_string(NmrSpectrometerList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='NmrSpectrometerList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='NmrSpectrometerList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(NmrSpectrometerList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
