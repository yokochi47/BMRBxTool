...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.IsotopeEffectType.*;

public class bmr_IsotopeEffectType {

	private static final String table_name = "Isotope_effect";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		IsotopeEffectType body = IsotopeEffectType.Factory.newInstance();
		IsotopeEffect[] list = new IsotopeEffect[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Chem_shift_isotope_effect_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:isotope_effectCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='IsotopeEffect', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = IsotopeEffect.Factory.newInstance();
					...
					if (!set_integer(list[0], "setAtomIsotopeNumber", "setNilAtomIsotopeNumber", false, rset.getString("Atom_isotope_number"), logw))
						continue;
					if (!set_string(list[0], "setAtomType", "setNilAtomType", false, rset.getString("Atom_type"), logw))
						continue;
					...
					if (!set_string(list[0], "setAuthSeqId", "setNilAuthSeqId", false, rset.getString("Auth_seq_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setChemShiftVal", "setNilChemShiftVal", false, rset.getString("Chem_shift_val"), logw))
						continue;
					if (!set_decimal(list[0], "setChemShiftValErr", "setNilChemShiftValErr", false, rset.getString("Chem_shift_val_err"), logw))
						continue;
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setLabel1IdChemShiftVal", "setNilLabel1IdChemShiftVal", false, rset.getString("Label_1_ID_chem_shift_val"), logw))
						continue;
					if (!set_decimal(list[0], "setLabel1IdChemShiftValErr", "setNilLabel1IdChemShiftValErr", false, rset.getString("Label_1_ID_chem_shift_val_err"), logw))
						continue;
					if (!set_decimal(list[0], "setLabel2IdChemShiftVal", "setNilLabel2IdChemShiftVal", false, rset.getString("Label_2_ID_chem_shift_val"), logw))
						continue;
					if (!set_decimal(list[0], "setLabel2IdChemShiftValErr", "setNilLabel2IdChemShiftValErr", false, rset.getString("Label_2_ID_chem_shift_val_err"), logw))
						continue;
					if (!set_integer(list[0], "setLabelPatternId1", "setNilLabelPatternId1", false, rset.getString("Label_pattern_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setLabelPatternId2", "setNilLabelPatternId2", false, rset.getString("Label_pattern_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setLabelSampleId1", "", true, rset.getString("Label_sample_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setLabelSampleId2", "", true, rset.getString("Label_sample_ID_2"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSeqId", "", true, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_integer(list[0], "setChemShiftIsotopeEffectListId", "", true, rset.getString("Chem_shift_isotope_effect_list_ID"), logw))
						continue;
					...
					body.setIsotopeEffectArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_IsotopeEffectType.class.getName());
	...
	private static boolean set_string(IsotopeEffect list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='IsotopeEffect', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='IsotopeEffect', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(IsotopeEffect list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(IsotopeEffect list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
