...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.OrderParamType.*;

public class bmr_OrderParamType {

	private static final String table_name = "Order_param";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		OrderParamType body = OrderParamType.Factory.newInstance();
		OrderParam[] list = new OrderParam[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Order_parameter_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:order_paramCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='OrderParam', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = OrderParam.Factory.newInstance();
					...
					if (!set_integer(list[0], "setEntityId", "", false, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_model_fit(list[0], "setModelFit", "setNilModelFit", false, rset.getString("Model_fit"), logw))
						continue;
					if (!set_decimal(list[0], "setModelFreeSumSquaredErrs", "setNilModelFreeSumSquaredErrs", false, rset.getString("Model_free_sum_squared_errs"), logw))
						continue;
					if (!set_decimal(list[0], "setOrderParamVal", "", true, rset.getString("Order_param_val"), logw))
						continue;
					if (!set_decimal(list[0], "setOrderParamValFitErr", "setNilOrderParamValFitErr", false, rset.getString("Order_param_val_fit_err"), logw))
						continue;
					if (!set_integer(list[0], "setResonanceId", "", false, rset.getString("Resonance_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setRexVal", "setNilRexVal", false, rset.getString("Rex_val"), logw))
						continue;
					if (!set_decimal(list[0], "setRexValFitErr", "setNilRexValFitErr", false, rset.getString("Rex_val_fit_err"), logw))
						continue;
					if (!set_integer(list[0], "setSeqId", "", false, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setSf2Val", "setNilSf2Val", false, rset.getString("Sf2_val"), logw))
						continue;
					if (!set_decimal(list[0], "setSf2ValFitErr", "setNilSf2ValFitErr", false, rset.getString("Sf2_val_fit_err"), logw))
						continue;
					if (!set_decimal(list[0], "setSh2Val", "setNilSh2Val", false, rset.getString("SH2_val"), logw))
						continue;
					if (!set_decimal(list[0], "setSh2ValFitErr", "setNilSh2ValFitErr", false, rset.getString("SH2_val_fit_err"), logw))
						continue;
					if (!set_decimal(list[0], "setSn2Val", "setNilSn2Val", false, rset.getString("SN2_val"), logw))
						continue;
					if (!set_decimal(list[0], "setSn2ValFitErr", "setNilSn2ValFitErr", false, rset.getString("SN2_val_fit_err"), logw))
						continue;
					if (!set_decimal(list[0], "setSs2Val", "setNilSs2Val", false, rset.getString("Ss2_val"), logw))
						continue;
					if (!set_decimal(list[0], "setSs2ValFitErr", "setNilSs2ValFitErr", false, rset.getString("Ss2_val_fit_err"), logw))
						continue;
					if (!set_decimal_tau_e_val(list[0], "setTauEVal", "setNilTauEVal", false, rset.getString("Tau_e_val"), conn_bmrb, entry_id, logw))
						continue;
					if (!set_decimal_tau_e_val(list[0], "setTauEValFitErr", "setNilTauEValFitErr", false, rset.getString("Tau_e_val_fit_err"), conn_bmrb, entry_id, logw))
						continue;
					if (!set_decimal(list[0], "setTauFVal", "setNilTauFVal", false, rset.getString("Tau_f_val"), logw))
						continue;
					if (!set_decimal(list[0], "setTauFValFitErr", "setNilTauFValFitErr", false, rset.getString("Tau_f_val_fit_err"), logw))
						continue;
					if (!set_decimal(list[0], "setTauSVal", "setNilTauSVal", false, rset.getString("Tau_s_val"), logw))
						continue;
					if (!set_decimal(list[0], "setTauSValFitErr", "setNilTauSValFitErr", false, rset.getString("Tau_s_val_fit_err"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setOrderParameterListId", "", true, rset.getString("Order_parameter_list_ID"), logw))
						continue;

					body.setOrderParamArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_OrderParamType.class.getName());
	...
	private static boolean set_string(OrderParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='OrderParam', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='OrderParam', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_model_fit(OrderParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")))
			val_name = bmr_Util_OrderParam.getModelFit(val_name);
	...
	private static boolean set_integer(OrderParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(OrderParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?") || (!val_name.matches("^[-+]?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)?$") && !val_name.matches("^[-+]?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)([eE][-+]?[0-9]+)?$"))) && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (method_name.equalsIgnoreCase("setOrderParamVal"))
				val_name = "0.0";
	...
	private static boolean set_decimal_tau_e_val(OrderParam list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml)
			val_name = bmr_Util_OrderParam.getTauEVal(val_name, conn_bmrb, entry_id);
