...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.PdbxNonpolySchemeType.*;

public class bmr_PdbxNonpolySchemeType {

	private static final String table_name = "PDBX_nonpoly_scheme";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		PdbxNonpolySchemeType body = PdbxNonpolySchemeType.Factory.newInstance();
		PdbxNonpolyScheme[] list = new PdbxNonpolyScheme[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:pdbx_nonpoly_schemeCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='PdbxNonpolyScheme', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = PdbxNonpolyScheme.Factory.newInstance();
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_string(list[0], "setMonId", "", true, rset.getString("Mon_ID"), logw))
						continue;
					...
					body.setPdbxNonpolySchemeArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_PdbxNonpolySchemeType.class.getName());
	...
	private static boolean set_string(PdbxNonpolyScheme list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='PdbxNonpolyScheme', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PdbxNonpolyScheme', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(PdbxNonpolyScheme list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
