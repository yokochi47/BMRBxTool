...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.NmrExperimentFileType.*;

public class bmr_NmrExperimentFileType {

	private static final String table_name = "NMR_experiment_file";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		NmrExperimentFileType body = NmrExperimentFileType.Factory.newInstance();
		NmrExperimentFile[] list = new NmrExperimentFile[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"NMR_spec_expt_ID\")::decimal");
				...
				buffw.write("  <BMRBx:nmr_experiment_fileCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='NmrExperimentFile', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = NmrExperimentFile.Factory.newInstance();

					if (!set_string(list[0], "setByteOrder", "setNilByteOrder", false, rset.getString("Byte_order"), logw))
						continue;
					if (!set_integer(list[0], "setBytesPerDataPoint", "setNilBytesPerDataPoint", false, rset.getString("Bytes_per_data_point"), logw))
						continue;
					if (!set_string(list[0], "setCompressionAlgorithm", "setNilCompressionAlgorithm", false, rset.getString("Compression_algorithm"), logw))
						continue;
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setDirectoryPath", "setNilDirectoryPath", false, rset.getString("Directory_path"), logw))
						continue;
					if (!set_integer(list[0], "setFileHeaderSize", "setNilFileHeaderSize", false, rset.getString("File_header_size"), logw))
						continue;
					if (!set_integer(list[0], "setRecordHeaderSize", "setNilRecordHeaderSize", false, rset.getString("Record_header_size"), logw))
						continue;
					if (!set_integer(list[0], "setRecordTrailerSize", "setNilRecordTrailerSize", false, rset.getString("Record_trailer_size"), logw))
						continue;
					...
					if (!set_string(list[0], "setName", "", true, rset.getString("Name"), logw))
						continue;
					if (!set_integer(list[0], "setNmrSpecExptId", "", true, rset.getString("NMR_spec_expt_ID"), logw))
						continue;

					body.setNmrExperimentFileArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_NmrExperimentFileType.class.getName());
	...
	private static boolean set_string(NmrExperimentFile list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='NmrExperimentFile', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='NmrExperimentFile', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(NmrExperimentFile list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(NmrExperimentFile list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_NMRExperimentFile.getType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrExperimentFile.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrExperimentFile.Type.Enum _enum = NmrExperimentFile.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrExperimentFile method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrExperimentFile.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrExperimentFile.Type.Enum.forInt(i));
						try {
							errw.write("class_name:NmrExperimentFile method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrExperimentFile.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrExperimentFile.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
