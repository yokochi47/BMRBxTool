...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.OtherDataTypeListType.*;

public class bmr_OtherDataTypeListType {

	private static final String table_name = "Other_data_type_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		OtherDataTypeListType body = OtherDataTypeListType.Factory.newInstance();
		OtherDataTypeList[] list = new OtherDataTypeList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:other_data_type_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='OtherDataTypeList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = OtherDataTypeList.Factory.newInstance();

					if (!set_string(list[0], "setDefinition", "", true, rset.getString("Definition"), logw))
						continue;
					...
					body.setOtherDataTypeListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_OtherDataTypeListType.class.getName());
	...
	private static boolean set_string(OtherDataTypeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='OtherDataTypeList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='OtherDataTypeList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(OtherDataTypeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_sample_condition_list_id(OtherDataTypeList list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, FileWriter logw) {
