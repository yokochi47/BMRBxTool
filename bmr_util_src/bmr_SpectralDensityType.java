...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SpectralDensityType.*;

public class bmr_SpectralDensityType {

	private static final String table_name = "Spectral_density";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SpectralDensityType body = SpectralDensityType.Factory.newInstance();
		SpectralDensity[] list = new SpectralDensity[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Spectral_density_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:spectral_densityCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SpectralDensity', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SpectralDensity.Factory.newInstance();
					...
					if (!set_integer(list[0], "setSeqId", "", true, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setW13CVal", "setNilW13CVal", false, rset.getString("W_13C_val"), logw))
						continue;
					if (!set_decimal(list[0], "setW13CValErr", "setNilW13CValErr", false, rset.getString("W_13C_val_err"), logw))
						continue;
					if (!set_decimal(list[0], "setW15NVal", "setNilW15NVal", false, rset.getString("W_15N_val"), logw))
						continue;
					if (!set_decimal(list[0], "setW15NValErr", "setNilW15NValErr", false, rset.getString("W_15N_val_err"), logw))
						continue;
					if (!set_decimal(list[0], "setW1HVal", "setNilW1HVal", false, rset.getString("W_1H_val"), logw))
						continue;
					if (!set_decimal(list[0], "setW1HValErr", "setNilW1HValErr", false, rset.getString("W_1H_val_err"), logw))
						continue;
					if (!set_decimal(list[0], "setWZeroVal", "setNilWZeroVal", false, rset.getString("W_zero_val"), logw))
						continue;
					if (!set_decimal(list[0], "setWZeroValErr", "setNilWZeroValErr", false, rset.getString("W_zero_val_err"), logw))
						continue;
					...
					body.setSpectralDensityArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SpectralDensityType.class.getName());
	...
	private static boolean set_string(SpectralDensity list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SpectralDensity', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SpectralDensity', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SpectralDensity list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(SpectralDensity list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
