...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EmrExptType.*;

public class bmr_EmrExptType {

	private static final String table_name = "EMR_expt";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EmrExptType body = EmrExptType.Factory.newInstance();
		EmrExpt[] list = new EmrExpt[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:emr_exptCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EmrExpt', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EmrExpt.Factory.newInstance();

					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_integer(list[0], "setEmrInstrumentId", "", true, rset.getString("EMR_instrument_ID"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_EmrExptType.class.getName());
	...
	private static boolean set_string(EmrExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EmrExpt', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EmrExpt', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EmrExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(EmrExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_sample_volume_units(EmrExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ EmrExpt.SampleVolumeUnits.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					EmrExpt.SampleVolumeUnits.Enum _enum = EmrExpt.SampleVolumeUnits.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:EmrExpt method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= EmrExpt.SampleVolumeUnits.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + EmrExpt.SampleVolumeUnits.Enum.forInt(i));
						try {
							errw.write("class_name:EmrExpt method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= EmrExpt.SampleVolumeUnits.Enum.table.lastInt(); i++)
								errw.write(" enum:" + EmrExpt.SampleVolumeUnits.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
