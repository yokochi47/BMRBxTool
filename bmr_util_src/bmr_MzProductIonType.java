...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MzProductIonType.*;

public class bmr_MzProductIonType {

	private static final String table_name = "MZ_product_ion";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MzProductIonType body = MzProductIonType.Factory.newInstance();
		MzProductIon[] list = new MzProductIon[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:mz_product_ionCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MzProductIon', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MzProductIon.Factory.newInstance();
					...
					body.setMzProductIonArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MzProductIonType.class.getName());
	...
	private static boolean set_string(MzProductIon list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MzProductIon', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MzProductIon', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MzProductIon list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(MzProductIon list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
