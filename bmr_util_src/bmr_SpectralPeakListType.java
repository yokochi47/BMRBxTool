...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SpectralPeakListType.*;

public class bmr_SpectralPeakListType {

	private static final String table_name = "Spectral_peak_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SpectralPeakListType body = SpectralPeakListType.Factory.newInstance();
		SpectralPeakList[] list = new SpectralPeakList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:spectral_peak_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SpectralPeakList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SpectralPeakList.Factory.newInstance();

					if (!set_integer(list[0], "setAssignedChemShiftListId", "setNilAssignedChemShiftListId", false, rset.getString("Assigned_chem_shift_list_ID"), logw))
						continue;
					if (!set_string(list[0], "setAssignedChemShiftListLabel", "setNilAssignedChemShiftListLabel", false, rset.getString("Assigned_chem_shift_list_label"), logw))
						continue;
					if (!set_string(list[0], "setChemicalShiftList", "setNilChemicalShiftList", false, rset.getString("Chemical_shift_list"), logw))
						continue;
					...
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_integer(list[0], "setNumberOfSpectralDimensions", "", false, rset.getString("Number_of_spectral_dimensions"), logw))
						continue;
					if (!set_integer_sample_condition_list_id(list[0], "setSampleConditionListId", "", true, rset.getString("Sample_condition_list_ID"), conn_bmrb, entry_id, logw))
						continue;
					...
					body.setSpectralPeakListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SpectralPeakListType.class.getName());
	...
	private static boolean set_string(SpectralPeakList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer(SpectralPeakList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_experiment_id(SpectralPeakList list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String sample_id, FileWriter logw) {
	...
	private static boolean set_integer_sample_id(SpectralPeakList list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, FileWriter logw) {
	...
	private static boolean set_integer_sample_condition_list_id(SpectralPeakList list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, FileWriter logw) {
