...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HExchProtectionFactSoftwareType.*;

public class bmr_HExchProtectionFactSoftwareType {

	private static final String table_name = "H_exch_protection_fact_software";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HExchProtectionFactSoftwareType body = HExchProtectionFactSoftwareType.Factory.newInstance();
		HExchProtectionFactSoftware[] list = new HExchProtectionFactSoftware[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"H_exch_protection_factor_list_ID\")::decimal,(0 || \"Software_ID\")::decimal");
				...
				buffw.write("  <BMRBx:h_exch_protection_fact_softwareCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HExchProtectionFactSoftware', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HExchProtectionFactSoftware.Factory.newInstance();
					...
					body.setHExchProtectionFactSoftwareArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HExchProtectionFactSoftwareType.class.getName());
	...
	private static boolean set_string(HExchProtectionFactSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HExchProtectionFactSoftware', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HExchProtectionFactSoftware', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HExchProtectionFactSoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_software_id(HExchProtectionFactSoftware list, String method_name, String nil_method_name, boolean required, String val_name, int id, FileWriter logw) {
