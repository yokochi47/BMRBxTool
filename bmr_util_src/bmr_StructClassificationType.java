...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.StructClassificationType.*;

public class bmr_StructClassificationType {

	private static final String table_name = "Struct_classification";

	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, bmr_Util_StructClassification sc, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {

		StructClassificationType body = StructClassificationType.Factory.newInstance();
		StructClassification[] list = new StructClassification[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:struct_classificationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='StructClassification', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = StructClassification.Factory.newInstance();

					if (!set_string(list[0], "setClass1", "setNilClass1", false, rset.getString("Class"), logw))
						continue;
					if (!set_string(list[0], "setCode", "setNilCode", false, rset.getString("Code"), logw))
						continue;
					if (!set_string(list[0], "setDbSourceId", "setNilDbSourceId", false, rset.getString("DB_source_ID"), logw))
						continue;
					if (!set_string(list[0], "setDescription", "setNilDescription", false, rset.getString("Description"), logw))
						continue;
					if (!set_string(list[0], "setFamily", "setNilFamily", false, rset.getString("Family"), logw))
						continue;
					if (!set_string(list[0], "setFold", "setNilFold", false, rset.getString("Fold"), logw))
						continue;
					if (!set_string(list[0], "setSuperfamily", "setNilSuperfamily", false, rset.getString("Superfamily"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setStructureAnnotationId", "", true, rset.getString("Structure_annotation_ID"), logw))
						continue;
					set_integer_sunid(list[0], "setSunid", "setNilSunid", false, rset.getString("DB_source_ID"), sc, logw);

					body.setStructClassificationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_StructClassificationType.class.getName());
	...
	private static boolean set_string(StructClassification list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='StructClassification', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='StructClassification', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(StructClassification list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_sunid(StructClassification list, String method_name, String nil_method_name, boolean required, String db_source_id, bmr_Util_StructClassification sc, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && !(db_source_id == null || db_source_id.isEmpty() || db_source_id.equals(".") || db_source_id.equals("?")))
			val_name = sc.getSunIDByKey(db_source_id);
