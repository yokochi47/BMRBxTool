...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.CaCbConstraintType.*;

public class bmr_CaCbConstraintType {

	private static final String table_name = "CA_CB_constraint";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		CaCbConstraintType body = CaCbConstraintType.Factory.newInstance();
		CaCbConstraint[] list = new CaCbConstraint[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"CA_CB_constraint_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:ca_cb_constraintCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='CaCbConstraint', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = CaCbConstraint.Factory.newInstance();
					...
					if (!set_integer(list[0], "setAssemblyAtomId2", "", true, rset.getString("Assembly_atom_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setAssemblyAtomId3", "", true, rset.getString("Assembly_atom_ID_3"), logw))
						continue;
					if (!set_integer(list[0], "setAssemblyAtomId4", "", true, rset.getString("Assembly_atom_ID_4"), logw))
						continue;
					if (!set_integer(list[0], "setAssemblyAtomId5", "", true, rset.getString("Assembly_atom_ID_5"), logw))
						continue;
					...
					if (!set_string(list[0], "setAtomType4", "", true, rset.getString("Atom_type_4"), logw))
						continue;
					if (!set_string(list[0], "setAtomType5", "", true, rset.getString("Atom_type_5"), logw))
						continue;
					...
					if (!set_string(list[0], "setAuthAsymId4", "setNilAuthAsymId4", false, rset.getString("Auth_asym_ID_4"), logw))
						continue;
					if (!set_string(list[0], "setAuthAsymId5", "setNilAuthAsymId5", false, rset.getString("Auth_asym_ID_5"), logw))
						continue;
					...
					if (!set_string(list[0], "setAuthAtomId4", "setNilAuthAtomId4", false, rset.getString("Auth_atom_ID_4"), logw))
						continue;
					if (!set_string(list[0], "setAuthAtomId5", "setNilAuthAtomId5", false, rset.getString("Auth_atom_ID_5"), logw))
						continue;
					...
					if (!set_string(list[0], "setAuthCompId4", "setNilAuthCompId4", false, rset.getString("Auth_comp_ID_4"), logw))
						continue;
					if (!set_string(list[0], "setAuthCompId5", "setNilAuthCompId5", false, rset.getString("Auth_comp_ID_5"), logw))
						continue;
					...
					if (!set_integer(list[0], "setAuthEntityAssemblyId4", "setNilAuthEntityAssemblyId4", false, rset.getString("Auth_entity_assembly_ID_4"), logw))
						continue;
					if (!set_integer(list[0], "setAuthEntityAssemblyId5", "setNilAuthEntityAssemblyId5", false, rset.getString("Auth_entity_assembly_ID_5"), logw))
						continue;
					...
					if (!set_string(list[0], "setAuthSeqId4", "setNilAuthSeqId4", false, rset.getString("Auth_seq_ID_4"), logw))
						continue;
					if (!set_string(list[0], "setAuthSeqId5", "setNilAuthSeqId5", false, rset.getString("Auth_seq_ID_5"), logw))
						continue;
					if (!set_decimal(list[0], "setCaChemShiftVal", "", true, rset.getString("CA_chem_shift_val"), logw))
						continue;
					if (!set_decimal(list[0], "setCaChemShiftValErr", "", true, rset.getString("CA_chem_shift_val_err"), logw))
						continue;
					if (!set_decimal(list[0], "setCbChemShiftVal", "", true, rset.getString("CB_chem_shift_val"), logw))
						continue;
					if (!set_decimal(list[0], "setCbChemShiftValErr", "", true, rset.getString("CB_chem_shift_val_err"), logw))
						continue;
					...
					if (!set_integer(list[0], "setResonanceId2", "", true, rset.getString("Resonance_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setResonanceId3", "", true, rset.getString("Resonance_ID_3"), logw))
						continue;
					if (!set_integer(list[0], "setResonanceId4", "", true, rset.getString("Resonance_ID_4"), logw))
						continue;
					if (!set_integer(list[0], "setResonanceId5", "", true, rset.getString("Resonance_ID_5"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSeqId5", "", true, rset.getString("Seq_ID_5"), logw))
						continue;
					if (!set_integer(list[0], "setSourceExperimentId", "", true, rset.getString("Source_experiment_ID"), logw))
						continue;
					if (!set_integer(list[0], "setCaCbConstraintListId", "", true, rset.getString("CA_CB_constraint_list_ID"), logw))
						continue;
					...
					body.setCaCbConstraintArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_CaCbConstraintType.class.getName());
	...
	private static boolean set_string(CaCbConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='CaCbConstraint', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='CaCbConstraint', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(CaCbConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(CaCbConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
