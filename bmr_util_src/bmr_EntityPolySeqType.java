...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EntityPolySeqType.*;

public class bmr_EntityPolySeqType {

	private static final String table_name = "Entity_poly_seq";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EntityPolySeqType body = EntityPolySeqType.Factory.newInstance();
		EntityPolySeq[] list = new EntityPolySeq[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:entity_poly_seqCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EntityPolySeq', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EntityPolySeq.Factory.newInstance();

					String mon_id = rset.getString("Mon_ID");

					if (mon_id == null || mon_id.isEmpty() || mon_id.equals(".") || mon_id.equals("?")) {


						try {
							logw.write("category='EntityPolySeq', content of null mon_id was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					if (!set_integer(list[0], "setCompIndexId", "", true, rset.getString("Comp_index_ID"), logw))
						continue;
					if (!set_string(list[0], "setHetero", "setNilHetero", false, rset.getString("Hetero"), logw))
						continue;
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setNum", "", true, rset.getString("Num"), logw))
						continue;

					body.setEntityPolySeqArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_EntityPolySeqType.class.getName());
	...
	private static boolean set_string(EntityPolySeq list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EntityPolySeq', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EntityPolySeq', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EntityPolySeq list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
