...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ResonanceAssignmentType.*;

public class bmr_ResonanceAssignmentType {

	private static final String table_name = "Resonance_assignment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ResonanceAssignmentType body = ResonanceAssignmentType.Factory.newInstance();
		ResonanceAssignment[] list = new ResonanceAssignment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Resonance_linker_list_ID\")::decimal,(0 || \"Resonance_set_ID\")::decimal");
				...
				buffw.write("  <BMRBx:resonance_assignmentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ResonanceAssignment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ResonanceAssignment.Factory.newInstance();
					...
					if (!set_integer(list[0], "setAtomIsotopeNumber", "setNilAtomIsotopeNumber", false, rset.getString("Atom_isotope_number"), logw))
						continue;
					if (!set_integer(list[0], "setAtomSetId", "setNilAtomSetId", false, rset.getString("Atom_set_ID"), logw))
						continue;
					...
					if (!set_integer(list[0], "setResonanceLinkerListId", "", true, rset.getString("Resonance_linker_list_ID"), logw))
						continue;
					if (!set_integer(list[0], "setResonanceSetId", "", true, rset.getString("Resonance_set_ID"), logw))
						continue;

					body.setResonanceAssignmentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ResonanceAssignmentType.class.getName());
	...
	private static boolean set_string(ResonanceAssignment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ResonanceAssignment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ResonanceAssignment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ResonanceAssignment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
