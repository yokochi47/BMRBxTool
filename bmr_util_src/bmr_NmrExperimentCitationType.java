...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.NmrExperimentCitationType.*;

public class bmr_NmrExperimentCitationType {

	private static final String table_name = "NMR_experiment_citation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		NmrExperimentCitationType body = NmrExperimentCitationType.Factory.newInstance();
		NmrExperimentCitation[] list = new NmrExperimentCitation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"NMR_spec_expt_ID\")::decimal,(0 || \"Citation_ID\")::decimal");
				...
				buffw.write("  <BMRBx:nmr_experiment_citationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='NmrExperimentCitation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = NmrExperimentCitation.Factory.newInstance();
					...
					body.setNmrExperimentCitationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_NmrExperimentCitationType.class.getName());
	...
	private static boolean set_string(NmrExperimentCitation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='NmrExperimentCitation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='NmrExperimentCitation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(NmrExperimentCitation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
