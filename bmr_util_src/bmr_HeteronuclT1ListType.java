...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HeteronuclT1ListType.*;

public class bmr_HeteronuclT1ListType {

	private static final String table_name = "Heteronucl_T1_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HeteronuclT1ListType body = HeteronuclT1ListType.Factory.newInstance();
		HeteronuclT1List[] list = new HeteronuclT1List[1];
		...
		try {
			...
			if (rset.next() && bmr_Util_Main.remediate_xml && !entry_id.equals("5505")) {
				...
				buffw.write("  <BMRBx:heteronucl_t1_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HeteronuclT1List', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HeteronuclT1List.Factory.newInstance();
					...
					if (!set_decimal(list[0], "setSpectrometerFrequency1H", "", true, rset.getString("Spectrometer_frequency_1H"), logw))
						continue;
					if (!set_enum_t1_coherence_type(list[0], "setT1CoherenceType", "", true, rset.getString("T1_coherence_type"), entry_id, logw, errw))
						continue;
					if (!set_enum_t1_val_units(list[0], "setT1ValUnits", "", true, rset.getString("T1_val_units"), logw, errw))
						continue;
					...
					body.setHeteronuclT1ListArray(list);
				...
				buffw.write("  </BMRBx:heteronucl_t1_listCategory>\n");
						...
						try {
							logw.write("category='HeteronuclT1List', all information content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
			...
			Logger lgr = Logger.getLogger(bmr_HeteronuclT1ListType.class.getName());
	...
	private static boolean set_string(HeteronuclT1List list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HeteronuclT1List', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HeteronuclT1List', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HeteronuclT1List list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(HeteronuclT1List list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_t1_coherence_type(HeteronuclT1List list, String method_name, String nil_method_name, boolean required, String val_name, String entry_id, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_HeteronuclT1List.getT1CoherenceType(val_name, entry_id);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ HeteronuclT1List.T1CoherenceType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					HeteronuclT1List.T1CoherenceType.Enum _enum = HeteronuclT1List.T1CoherenceType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:HeteronuclT1List method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= HeteronuclT1List.T1CoherenceType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + HeteronuclT1List.T1CoherenceType.Enum.forInt(i));
						try {
							errw.write("class_name:HeteronuclT1List method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= HeteronuclT1List.T1CoherenceType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + HeteronuclT1List.T1CoherenceType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_t1_val_units(HeteronuclT1List list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_HeteronuclT1List.getT1ValUnits(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ HeteronuclT1List.T1ValUnits.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					HeteronuclT1List.T1ValUnits.Enum _enum = HeteronuclT1List.T1ValUnits.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:HeteronuclT1List method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= HeteronuclT1List.T1ValUnits.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + HeteronuclT1List.T1ValUnits.Enum.forInt(i));
						try {
							errw.write("class_name:HeteronuclT1List method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= HeteronuclT1List.T1ValUnits.Enum.table.lastInt(); i++)
								errw.write(" enum:" + HeteronuclT1List.T1ValUnits.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
