...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.StructureAnnotationType.*;

public class bmr_StructureAnnotationType {

	private static final String table_name = "Structure_annotation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		StructureAnnotationType body = StructureAnnotationType.Factory.newInstance();
		StructureAnnotation[] list = new StructureAnnotation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:structure_annotationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='StructureAnnotation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = StructureAnnotation.Factory.newInstance();

					if (!set_string(list[0], "setDbElectronicAddress", "setNilDbElectronicAddress", false, rset.getString("DB_electronic_address"), logw))
						continue;
					if (!set_date(list[0], "setDbQueriedDate", "setNilDbQueriedDate", false, rset.getString("DB_queried_date") != null && !rset.getString("DB_queried_date").equals(".") && !rset.getString("DB_queried_date").equals("?") ? rset.getDate("DB_queried_date") : null, logw))
						continue;
					if (!set_string(list[0], "setDbSource", "", true, rset.getString("DB_source"), logw))
						continue;
					if (!set_date(list[0], "setDbSourceReleaseDate", "setNilDbSourceReleaseDate", false, rset.getString("DB_source_release_date") != null && !rset.getString("DB_source_release_date").equals(".") && !rset.getString("DB_source_release_date").equals("?") ? rset.getDate("DB_source_release_date") : null, logw))
						continue;
					if (!set_string(list[0], "setDbSourceReleaseDesignation", "setNilDbSourceReleaseDesignation", false, rset.getString("DB_source_release_designation"), logw))
						continue;
					...
					body.setStructureAnnotationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_StructureAnnotationType.class.getName());
	...
	private static boolean set_string(StructureAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='StructureAnnotation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='StructureAnnotation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(StructureAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_date(StructureAnnotation list, String method_name, String nil_method_name, boolean required, java.sql.Date date, FileWriter logw) {
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='StructureAnnotation', value='" + date + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
