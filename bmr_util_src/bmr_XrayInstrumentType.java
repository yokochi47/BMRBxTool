...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.XrayInstrumentType.*;

public class bmr_XrayInstrumentType {

	private static final String table_name = "Xray_instrument";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		XrayInstrumentType body = XrayInstrumentType.Factory.newInstance();
		XrayInstrument[] list = new XrayInstrument[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:xray_instrumentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='XrayInstrument', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = XrayInstrument.Factory.newInstance();

					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setDetectorType", "setNilDetectorType", false, rset.getString("Detector_type"), logw))
						continue;
					if (!set_string(list[0], "setLocation", "setNilLocation", false, rset.getString("Location"), logw))
						continue;
					if (!set_string(list[0], "setManufacturer", "", true, rset.getString("Manufacturer"), logw))
						continue;
					if (!set_string(list[0], "setModel", "", true, rset.getString("Model"), logw))
						continue;
					if (!set_decimal(list[0], "setSaxsDetectorSampleDistance", "setNilSaxsDetectorSampleDistance", false, rset.getString("SAXS_detector_sample_distance"), logw))
						continue;
					...
					if (!set_string(list[0], "setSfFramecode", "", true, rset.getString("Sf_framecode"), logw))
						continue;
					if (!set_string(list[0], "setSource", "setNilSource", false, rset.getString("Source"), logw))
						continue;
					if (!set_string(list[0], "setType", "setNilType", false, rset.getString("Type"), logw))
						continue;
					if (!set_decimal(list[0], "setWavelength", "setNilWavelength", false, rset.getString("Wavelength"), logw))
						continue;
					if (!set_decimal(list[0], "setWaxsDetectorSampleDistance", "setNilWaxsDetectorSampleDistance", false, rset.getString("WAXS_detector_sample_distance"), logw))
						continue;
					...
					body.setXrayInstrumentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_XrayInstrumentType.class.getName());
	...
	private static boolean set_string(XrayInstrument list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='XrayInstrument', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='XrayInstrument', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(XrayInstrument list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(XrayInstrument list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
