...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MzProductIonAnnotationType.*;

public class bmr_MzProductIonAnnotationType {

	private static final String table_name = "MZ_product_ion_annotation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MzProductIonAnnotationType body = MzProductIonAnnotationType.Factory.newInstance();
		MzProductIonAnnotation[] list = new MzProductIonAnnotation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"MZ_ratio_data_list_ID\")::decimal,(0 || \"MZ_product_ion_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:mz_product_ion_annotationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MzProductIonAnnotation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MzProductIonAnnotation.Factory.newInstance();
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setMzProductIonId", "", true, rset.getString("MZ_product_ion_ID"), logw))
						continue;
					if (!set_integer(list[0], "setMzRatioDataListId", "", true, rset.getString("MZ_ratio_data_list_ID"), logw))
						continue;

					body.setMzProductIonAnnotationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MzProductIonAnnotationType.class.getName());
	...
	private static boolean set_string(MzProductIonAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MzProductIonAnnotation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MzProductIonAnnotation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MzProductIonAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
