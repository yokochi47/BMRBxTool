...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.CsAnisotropySoftwareType.*;

public class bmr_CsAnisotropySoftwareType {

	private static final String table_name = "CS_anisotropy_software";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		CsAnisotropySoftwareType body = CsAnisotropySoftwareType.Factory.newInstance();
		CsAnisotropySoftware[] list = new CsAnisotropySoftware[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Chem_shift_anisotropy_ID\")::decimal,(0 || \"Software_ID\")::decimal");
				...
				buffw.write("  <BMRBx:cs_anisotropy_softwareCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='CsAnisotropySoftware', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = CsAnisotropySoftware.Factory.newInstance();
					...
					body.setCsAnisotropySoftwareArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_CsAnisotropySoftwareType.class.getName());
	...
	private static boolean set_string(CsAnisotropySoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='CsAnisotropySoftware', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='CsAnisotropySoftware', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(CsAnisotropySoftware list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_software_id(CsAnisotropySoftware list, String method_name, String nil_method_name, boolean required, String val_name, int id, FileWriter logw) {
