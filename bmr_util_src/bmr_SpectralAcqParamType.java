...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SpectralAcqParamType.*;

public class bmr_SpectralAcqParamType {

	private static final String table_name = "Spectral_acq_param";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SpectralAcqParamType body = SpectralAcqParamType.Factory.newInstance();
		SpectralAcqParam[] list = new SpectralAcqParam[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"NMR_spec_expt_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:spectral_acq_paramCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SpectralAcqParam', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SpectralAcqParam.Factory.newInstance();

					if (!set_integer(list[0], "setAcquisitionDimensionId", "", true, rset.getString("Acquisition_dimension_ID"), logw))
						continue;
					...
					body.setSpectralAcqParamArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SpectralAcqParamType.class.getName());
	...
	private static boolean set_string(SpectralAcqParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SpectralAcqParam', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SpectralAcqParam', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SpectralAcqParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_name(SpectralAcqParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ SpectralAcqParam.Name.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					SpectralAcqParam.Name.Enum _enum = SpectralAcqParam.Name.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:SpectralAcqParam method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= SpectralAcqParam.Name.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + SpectralAcqParam.Name.Enum.forInt(i));
						try {
							errw.write("class_name:SpectralAcqParam method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= SpectralAcqParam.Name.Enum.table.lastInt(); i++)
								errw.write(" enum:" + SpectralAcqParam.Name.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
