...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ExperimentFileType.*;

public class bmr_ExperimentFileType {

	private static final String table_name = "Experiment_file";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ExperimentFileType body = ExperimentFileType.Factory.newInstance();
		ExperimentFile[] list = new ExperimentFile[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Experiment_ID\")::decimal");
				...
				buffw.write("  <BMRBx:experiment_fileCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ExperimentFile', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ExperimentFile.Factory.newInstance();

					if (!set_enum_content(list[0], "setContent", "setNilContent", false, rset.getString("Content"), logw, errw))
						continue;
					...
					body.setExperimentFileArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ExperimentFileType.class.getName());
	...
	private static boolean set_string(ExperimentFile list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ExperimentFile', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ExperimentFile', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ExperimentFile list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_content(ExperimentFile list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_ExperimentFile.getContent(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ExperimentFile.Content.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ExperimentFile.Content.Enum _enum = ExperimentFile.Content.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ExperimentFile method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ExperimentFile.Content.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ExperimentFile.Content.Enum.forInt(i));
						try {
							errw.write("class_name:ExperimentFile method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ExperimentFile.Content.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ExperimentFile.Content.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
