...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ResonanceType.*;

public class bmr_ResonanceType {

	private static final String table_name = "Resonance";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ResonanceType body = ResonanceType.Factory.newInstance();
		Resonance[] list = new Resonance[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Resonance_linker_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:resonanceCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='Resonance', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = Resonance.Factory.newInstance();

					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_integer(list[0], "setResonanceSetId", "setNilResonanceSetId", false, rset.getString("Resonance_set_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSpinSystemId", "setNilSpinSystemId", false, rset.getString("Spin_system_ID"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_ResonanceType.class.getName());
	...
	private static boolean set_string(Resonance list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='Resonance', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='Resonance', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(Resonance list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
