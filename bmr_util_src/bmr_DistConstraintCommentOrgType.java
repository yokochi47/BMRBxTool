...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DistConstraintCommentOrgType.*;

public class bmr_DistConstraintCommentOrgType {

	private static final String table_name = "Dist_constraint_comment_org";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DistConstraintCommentOrgType body = DistConstraintCommentOrgType.Factory.newInstance();
		DistConstraintCommentOrg[] list = new DistConstraintCommentOrg[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Distance_constraint_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:dist_constraint_comment_orgCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DistConstraintCommentOrg', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DistConstraintCommentOrg.Factory.newInstance();

					if (!set_integer(list[0], "setCommentBeginColumn", "setNilCommentBeginColumn", false, rset.getString("Comment_begin_column"), logw))
						continue;
					if (!set_integer(list[0], "setCommentBeginLine", "setNilCommentBeginLine", false, rset.getString("Comment_begin_line"), logw))
						continue;
					if (!set_integer(list[0], "setCommentEndColumn", "setNilCommentEndColumn", false, rset.getString("Comment_end_column"), logw))
						continue;
					if (!set_integer(list[0], "setCommentEndLine", "setNilCommentEndLine", false, rset.getString("Comment_end_line"), logw))
						continue;
					if (!set_string(list[0], "setCommentText", "", true, rset.getString("Comment_text"), logw))
						continue;
					...
					body.setDistConstraintCommentOrgArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DistConstraintCommentOrgType.class.getName());
	...
	private static boolean set_string(DistConstraintCommentOrg list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DistConstraintCommentOrg', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DistConstraintCommentOrg', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DistConstraintCommentOrg list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
