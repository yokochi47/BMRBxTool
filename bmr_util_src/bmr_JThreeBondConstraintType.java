...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.JThreeBondConstraintType.*;

public class bmr_JThreeBondConstraintType {

	private static final String table_name = "J_three_bond_constraint";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		JThreeBondConstraintType body = JThreeBondConstraintType.Factory.newInstance();
		JThreeBondConstraint[] list = new JThreeBondConstraint[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"J_three_bond_constraint_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:j_three_bond_constraintCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='JThreeBondConstraint', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = JThreeBondConstraint.Factory.newInstance();
					...
					if (!set_integer(list[0], "setCompIndexId4", "", true, rset.getString("Comp_index_ID_4"), logw))
						continue;
					if (!set_decimal(list[0], "setCouplingConstantErr", "setNilCouplingConstantErr", false, rset.getString("Coupling_constant_err"), logw))
						continue;
					if (!set_decimal(list[0], "setCouplingConstantLowerBound", "setNilCouplingConstantLowerBound", false, rset.getString("Coupling_constant_lower_bound"), logw))
						continue;
					if (!set_decimal(list[0], "setCouplingConstantUpperBound", "setNilCouplingConstantUpperBound", false, rset.getString("Coupling_constant_upper_bound"), logw))
						continue;
					if (!set_decimal(list[0], "setCouplingConstantVal", "setNilCouplingConstantVal", false, rset.getString("Coupling_constant_val"), logw))
						continue;
					...
					body.setJThreeBondConstraintArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_JThreeBondConstraintType.class.getName());
	...
	private static boolean set_string(JThreeBondConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='JThreeBondConstraint', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='JThreeBondConstraint', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(JThreeBondConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(JThreeBondConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
