...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyCitationType.*;

public class bmr_AssemblyCitationType {

	private static final String table_name = "Assembly_citation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssemblyCitationType body = AssemblyCitationType.Factory.newInstance();
		AssemblyCitation[] list = new AssemblyCitation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Assembly_ID\")::decimal,(0 || \"Citation_ID\")::decimal");
				...
				buffw.write("  <BMRBx:assembly_citationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssemblyCitation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AssemblyCitation.Factory.newInstance();
					...
					body.setAssemblyCitationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssemblyCitationType.class.getName());
	...
	private static boolean set_string(AssemblyCitation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssemblyCitation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssemblyCitation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AssemblyCitation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
