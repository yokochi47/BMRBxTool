...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HChemShiftConstraintExptType.*;

public class bmr_HChemShiftConstraintExptType {

	private static final String table_name = "H_chem_shift_constraint_expt";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HChemShiftConstraintExptType body = HChemShiftConstraintExptType.Factory.newInstance();
		HChemShiftConstraintExpt[] list = new HChemShiftConstraintExpt[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"H_chem_shift_constraint_list_ID\")::decimal");
				...
				buffw.write("  <BMRBx:h_chem_shift_constraint_exptCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HChemShiftConstraintExpt', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HChemShiftConstraintExpt.Factory.newInstance();
					...
					if (!set_integer(list[0], "setExperimentId", "", true, rset.getString("Experiment_ID"), logw))
						continue;
					if (!set_integer(list[0], "setHChemShiftConstraintListId", "", true, rset.getString("H_chem_shift_constraint_list_ID"), logw))
						continue;

					body.setHChemShiftConstraintExptArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HChemShiftConstraintExptType.class.getName());
	...
	private static boolean set_string(HChemShiftConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HChemShiftConstraintExpt', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HChemShiftConstraintExpt', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HChemShiftConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_sample_state(HChemShiftConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ HChemShiftConstraintExpt.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					HChemShiftConstraintExpt.SampleState.Enum _enum = HChemShiftConstraintExpt.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:HChemShiftConstraintExpt method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= HChemShiftConstraintExpt.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + HChemShiftConstraintExpt.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:HChemShiftConstraintExpt method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= HChemShiftConstraintExpt.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + HChemShiftConstraintExpt.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
