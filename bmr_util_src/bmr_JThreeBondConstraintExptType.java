...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.JThreeBondConstraintExptType.*;

public class bmr_JThreeBondConstraintExptType {

	private static final String table_name = "J_three_bond_constraint_expt";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		JThreeBondConstraintExptType body = JThreeBondConstraintExptType.Factory.newInstance();
		JThreeBondConstraintExpt[] list = new JThreeBondConstraintExpt[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"J_three_bond_constraint_list_ID\")::decimal");
				...
				buffw.write("  <BMRBx:j_three_bond_constraint_exptCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='JThreeBondConstraintExpt', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = JThreeBondConstraintExpt.Factory.newInstance();

					if (!set_string(list[0], "setExperimentName", "", true, rset.getString("Experiment_name"), logw))
						continue;
					if (!set_integer(list[0], "setMethodId", "", true, rset.getString("Method_ID"), logw))
						continue;
					...
					if (!set_integer(list[0], "setExperimentId", "", true, rset.getString("Experiment_ID"), logw))
						continue;
					if (!set_integer(list[0], "setJThreeBondConstraintListId", "", true, rset.getString("J_three_bond_constraint_list_ID"), logw))
						continue;

					body.setJThreeBondConstraintExptArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_JThreeBondConstraintExptType.class.getName());
	...
	private static boolean set_string(JThreeBondConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='JThreeBondConstraintExpt', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='JThreeBondConstraintExpt', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(JThreeBondConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_sample_state(JThreeBondConstraintExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ JThreeBondConstraintExpt.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					JThreeBondConstraintExpt.SampleState.Enum _enum = JThreeBondConstraintExpt.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:JThreeBondConstraintExpt method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= JThreeBondConstraintExpt.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + JThreeBondConstraintExpt.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:JThreeBondConstraintExpt method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= JThreeBondConstraintExpt.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + JThreeBondConstraintExpt.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
