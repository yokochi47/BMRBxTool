...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.NmrSpecExptType.*;

public class bmr_NmrSpecExptType {

	private static final String table_name = "NMR_spec_expt";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		NmrSpecExptType body = NmrSpecExptType.Factory.newInstance();
		NmrSpecExpt[] list = new NmrSpecExpt[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:nmr_spec_exptCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='NmrSpecExpt', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = NmrSpecExpt.Factory.newInstance();

					if (!set_decimal(list[0], "setCarrierFreqSwitchTime", "setNilCarrierFreqSwitchTime", false, rset.getString("Carrier_freq_switch_time"), logw))
						continue;
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_integer(list[0], "setMethodId", "", false, rset.getString("Method_ID"), logw))
						continue;
					if (!set_string(list[0], "setMethodLabel", "setNilMethodLabel", false, rset.getString("Method_label"), logw))
						continue;
					...
					if (!set_integer(list[0], "setNmrSpectrometerId", "", false, rset.getString("NMR_spectrometer_ID"), logw))
						continue;
					if (!set_string(list[0], "setNmrSpectrometerLabel", "", true, rset.getString("NMR_spectrometer_label"), logw))
						continue;
					...
					if (!set_string(list[0], "setNmrSpectrometerProbeLabel", "setNilNmrSpectrometerProbeLabel", false, rset.getString("NMR_spectrometer_probe_label"), logw))
						continue;
					if (!set_enum_nmr_tube_type(list[0], "setNmrTubeType", "setNilNmrTubeType", false, rset.getString("NMR_tube_type"), logw, errw))
						continue;
					if (!set_string(list[0], "setPulseSeqAccessionBmrbCode", "setNilPulseSeqAccessionBmrbCode", false, rset.getString("Pulse_seq_accession_BMRB_code"), logw))
						continue;
					...
					if (!set_decimal(list[0], "setSampleVolume", "setNilSampleVolume", false, rset.getString("Sample_volume"), logw))
						continue;
					if (!set_enum_sample_volume_units(list[0], "setSampleVolumeUnits", "setNilSampleVolumeUnits", false, rset.getString("Sample_volume_units"), logw, errw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_NmrSpecExptType.class.getName());
	...
	private static boolean set_string(NmrSpecExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='NmrSpecExpt', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='NmrSpecExpt', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(NmrSpecExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(NmrSpecExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_nmr_tube_type(NmrSpecExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_NMRSpecExpt.getNMRTubeType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrSpecExpt.NmrTubeType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrSpecExpt.NmrTubeType.Enum _enum = NmrSpecExpt.NmrTubeType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrSpecExpt method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrSpecExpt.NmrTubeType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrSpecExpt.NmrTubeType.Enum.forInt(i));
						try {
							errw.write("class_name:NmrSpecExpt method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrSpecExpt.NmrTubeType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrSpecExpt.NmrTubeType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_sample_volume_units(NmrSpecExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrSpecExpt.SampleVolumeUnits.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrSpecExpt.SampleVolumeUnits.Enum _enum = NmrSpecExpt.SampleVolumeUnits.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrSpecExpt method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrSpecExpt.SampleVolumeUnits.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrSpecExpt.SampleVolumeUnits.Enum.forInt(i));
						try {
							errw.write("class_name:NmrSpecExpt method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrSpecExpt.SampleVolumeUnits.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrSpecExpt.SampleVolumeUnits.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_type(NmrSpecExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_NMRSpecExpt.getType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ NmrSpecExpt.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					NmrSpecExpt.Type.Enum _enum = NmrSpecExpt.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:NmrSpecExpt method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= NmrSpecExpt.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + NmrSpecExpt.Type.Enum.forInt(i));
						try {
							errw.write("class_name:NmrSpecExpt method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= NmrSpecExpt.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + NmrSpecExpt.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
