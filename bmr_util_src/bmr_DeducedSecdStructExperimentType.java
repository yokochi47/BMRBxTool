...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DeducedSecdStructExperimentType.*;

public class bmr_DeducedSecdStructExperimentType {

	private static final String table_name = "Deduced_secd_struct_experiment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DeducedSecdStructExperimentType body = DeducedSecdStructExperimentType.Factory.newInstance();
		DeducedSecdStructExperiment[] list = new DeducedSecdStructExperiment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Deduced_secd_struct_list_ID\")::decimal,(0 || \"Experiment_ID\")::decimal");
				...
				buffw.write("  <BMRBx:deduced_secd_struct_experimentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DeducedSecdStructExperiment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DeducedSecdStructExperiment.Factory.newInstance();
					...
					body.setDeducedSecdStructExperimentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DeducedSecdStructExperimentType.class.getName());
	...
	private static boolean set_string(DeducedSecdStructExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DeducedSecdStructExperiment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DeducedSecdStructExperiment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DeducedSecdStructExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_sample_state(DeducedSecdStructExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ DeducedSecdStructExperiment.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					DeducedSecdStructExperiment.SampleState.Enum _enum = DeducedSecdStructExperiment.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:DeducedSecdStructExperiment method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= DeducedSecdStructExperiment.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + DeducedSecdStructExperiment.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:DeducedSecdStructExperiment method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= DeducedSecdStructExperiment.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + DeducedSecdStructExperiment.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
