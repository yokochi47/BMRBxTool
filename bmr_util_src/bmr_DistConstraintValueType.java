...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DistConstraintValueType.*;

public class bmr_DistConstraintValueType {

	private static final String table_name = "Dist_constraint_value";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DistConstraintValueType body = DistConstraintValueType.Factory.newInstance();
		DistConstraintValue[] list = new DistConstraintValue[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Distance_constraint_list_ID\")::decimal,(0 || \"Constraint_ID\")::decimal");
				...
				buffw.write("  <BMRBx:dist_constraint_valueCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DistConstraintValue', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DistConstraintValue.Factory.newInstance();
					...
					if (!set_integer(list[0], "setSourceExperimentId", "", true, rset.getString("Source_experiment_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSpectralPeakId", "", true, rset.getString("Spectral_peak_ID"), logw))
						continue;
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setTreeNodeId", "", true, rset.getString("Tree_node_ID"), logw))
						continue;

					body.setDistConstraintValueArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DistConstraintValueType.class.getName());
	...
	private static boolean set_string(DistConstraintValue list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DistConstraintValue', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DistConstraintValue', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DistConstraintValue list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(DistConstraintValue list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
