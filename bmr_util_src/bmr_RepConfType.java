...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.RepConfType.*;

public class bmr_RepConfType {

	private static final String table_name = "Rep_conf";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		RepConfType body = RepConfType.Factory.newInstance();
		RepConf[] list = new RepConf[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:rep_confCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='RepConf', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = RepConf.Factory.newInstance();

					if (!set_integer(list[0], "setAssemblyAtomId", "", true, rset.getString("Assembly_atom_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setAtomCartnX", "", true, rset.getString("Atom_cartn_x"), logw))
						continue;
					if (!set_decimal(list[0], "setAtomCartnXEsd", "setNilAtomCartnXEsd", false, rset.getString("Atom_cartn_x_esd"), logw))
						continue;
					if (!set_decimal(list[0], "setAtomCartnY", "", true, rset.getString("Atom_cartn_y"), logw))
						continue;
					if (!set_decimal(list[0], "setAtomCartnYEsd", "setNilAtomCartnYEsd", false, rset.getString("Atom_cartn_y_esd"), logw))
						continue;
					if (!set_decimal(list[0], "setAtomCartnZ", "", true, rset.getString("Atom_cartn_z"), logw))
						continue;
					if (!set_decimal(list[0], "setAtomCartnZEsd", "setNilAtomCartnZEsd", false, rset.getString("Atom_cartn_z_esd"), logw))
						continue;
					if (!set_integer(list[0], "setAtomCoordFootnoteId", "setNilAtomCoordFootnoteId", false, rset.getString("Atom_coord_footnote_ID"), logw))
						continue;
					if (!set_string(list[0], "setAtomId", "", true, rset.getString("Atom_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setAtomPositionUncertainty", "setNilAtomPositionUncertainty", false, rset.getString("Atom_position_uncertainty"), logw))
						continue;
					if (!set_integer(list[0], "setAtomSiteId", "", true, rset.getString("Atom_site_ID"), logw))
						continue;
					...
					if (!set_string(list[0], "setAuthSeqId", "setNilAuthSeqId", false, rset.getString("Auth_seq_ID"), logw))
						continue;
					if (!set_string(list[0], "setChemCompPdbIdCode", "setNilChemCompPdbIdCode", false, rset.getString("Chem_comp_PDB_ID_code"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSeqId", "", true, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAtomCoordinateId", "", true, rset.getString("Atom_coordinate_ID"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setRepresentativeConformerId", "", true, rset.getString("Representative_conformer_ID"), logw))
						continue;

					body.setRepConfArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_RepConfType.class.getName());
	...
	private static boolean set_string(RepConf list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='RepConf', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='RepConf', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(RepConf list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(RepConf list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
