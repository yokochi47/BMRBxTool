...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssignedChemShiftListType.*;

public class bmr_AssignedChemShiftListType {

	private static final String table_name = "Assigned_chem_shift_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssignedChemShiftListType body = AssignedChemShiftListType.Factory.newInstance();
		AssignedChemShiftList[] list = new AssignedChemShiftList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:assigned_chem_shift_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssignedChemShiftList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AssignedChemShiftList.Factory.newInstance();
					...
					if (!set_decimal(list[0], "setChemShift31PErr", "setNilChemShift31PErr", false, rset.getString("Chem_shift_31P_err"), logw))
						continue;
					if (!set_integer(list[0], "setChemShiftReferenceId", "", false, rset.getString("Chem_shift_reference_ID"), logw))
						continue;
					...
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setErrorDerivationMethod", "setNilErrorDerivationMethod", false, rset.getString("Error_derivation_method"), logw))
						continue;
					...
					body.setAssignedChemShiftListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssignedChemShiftListType.class.getName());
	...
	private static boolean set_string(AssignedChemShiftList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssignedChemShiftList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssignedChemShiftList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AssignedChemShiftList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(AssignedChemShiftList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
