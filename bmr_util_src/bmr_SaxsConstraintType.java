...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SaxsConstraintType.*;

public class bmr_SaxsConstraintType {

	private static final String table_name = "SAXS_constraint";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SaxsConstraintType body = SaxsConstraintType.Factory.newInstance();
		SaxsConstraint[] list = new SaxsConstraint[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"SAXS_constraint_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:saxs_constraintCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SaxsConstraint', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SaxsConstraint.Factory.newInstance();
					...
					if (!set_decimal(list[0], "setIntensityValErr", "setNilIntensityValErr", false, rset.getString("Intensity_val_err"), logw))
						continue;
					if (!set_decimal(list[0], "setQValue", "setNilQValue", false, rset.getString("Q_value"), logw))
						continue;
					if (!set_decimal(list[0], "setWeightVal", "setNilWeightVal", false, rset.getString("Weight_val"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setSaxsConstraintListId", "", true, rset.getString("SAXS_constraint_list_ID"), logw))
						continue;

					body.setSaxsConstraintArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SaxsConstraintType.class.getName());
	...
	private static boolean set_string(SaxsConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SaxsConstraint', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SaxsConstraint', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SaxsConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(SaxsConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
