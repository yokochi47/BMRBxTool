...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SpectralTransitionCharType.*;

public class bmr_SpectralTransitionCharType {

	private static final String table_name = "Spectral_transition_char";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SpectralTransitionCharType body = SpectralTransitionCharType.Factory.newInstance();
		SpectralTransitionChar[] list = new SpectralTransitionChar[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:spectral_transition_charCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SpectralTransitionChar', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SpectralTransitionChar.Factory.newInstance();
					...
					if (!set_decimal(list[0], "setBoundingBoxUpperVal", "setNilBoundingBoxUpperVal", false, rset.getString("Bounding_box_upper_val"), logw))
						continue;
					if (!set_decimal(list[0], "setBoundingBoxWidthVal", "setNilBoundingBoxWidthVal", false, rset.getString("Bounding_box_width_val"), logw))
						continue;
					...
					if (!set_decimal(list[0], "setDecayRateValErr", "setNilDecayRateValErr", false, rset.getString("Decay_rate_val_err"), logw))
						continue;
					if (!set_integer(list[0], "setDerivationMethodId", "setNilDerivationMethodId", false, rset.getString("Derivation_method_ID"), logw))
						continue;
					...
					if (!set_integer(list[0], "setPeakId", "setNilPeakId", false, rset.getString("Peak_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setPhaseVal", "setNilPhaseVal", false, rset.getString("Phase_val"), logw))
						continue;
					...
					body.setSpectralTransitionCharArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SpectralTransitionCharType.class.getName());
	...
	private static boolean set_string(SpectralTransitionChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SpectralTransitionChar', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SpectralTransitionChar', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SpectralTransitionChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(SpectralTransitionChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
