...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AssemblyDbLinkType.*;

public class bmr_AssemblyDbLinkType {

	private static final String table_name = "Assembly_db_link";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, bmr_Util_Entry ent, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AssemblyDbLinkType body = AssemblyDbLinkType.Factory.newInstance();
		AssemblyDbLink[] list = new AssemblyDbLink[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Assembly_ID\")::decimal");
				...
				buffw.write("  <BMRBx:assembly_db_linkCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AssemblyDbLink', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AssemblyDbLink.Factory.newInstance();

					if (!set_enum_author_supplied(list[0], "setAuthorSupplied", "", true, rset.getString("Author_supplied"), logw, errw))
						continue;
					...
					if (!set_decimal(list[0], "setEntryStructureResolution", "setNilEntryStructureResolution", false, rset.getString("Entry_structure_resolution"), logw))
						continue;
					if (!set_string_accession_code(list[0], "setAccessionCode", "", true, bmr_Util_AssemblyDbLink.checkAccessionCode(rset.getString("Accession_code"), entry_id), ent, entry_id, rset.getString("Database_code"), logw))
						continue;
					if (!set_integer(list[0], "setAssemblyId", "", true, rset.getString("Assembly_ID"), logw))
						continue;
					if (!set_enum_database_code(list[0], "setDatabaseCode", "", true, rset.getString("Database_code"), entry_id, bmr_Util_AssemblyDbLink.checkAccessionCode(rset.getString("Accession_code"), entry_id), logw, errw)) {

						try {
							logw.write("item='DatabaseCode', category='AssemblyDbLink', value='" + rset.getString("Database_code") + "' was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					body.setAssemblyDbLinkArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AssemblyDbLinkType.class.getName());
	...
	private static boolean set_string(AssemblyDbLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AssemblyDbLink', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AssemblyDbLink', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_string_accession_code(AssemblyDbLink list, String method_name, String nil_method_name, boolean required, String val_name, bmr_Util_Entry ent, String entry_id, String database_code, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && (database_code == null || database_code.isEmpty() || database_code.equals(".") || database_code.equals("?")))
			database_code = bmr_Util_AssemblyDbLink.getDatabaseCode(database_code, val_name);
		...
		if (bmr_Util_Main.remediate_xml && val_name != null && !val_name.isEmpty() && val_name.contains(" ")) {
			...
			for (String val_name_ : val_names) {
				if (bmr_Util_AssemblyDbLink.getDatabaseCode(null, val_name_) != null) {
					val_name = val_name_;
					break;
				}
			}
	...
	private static boolean set_integer(AssemblyDbLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(AssemblyDbLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_author_supplied(AssemblyDbLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_AssemblyDbLink.getAuthorSupplied(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ AssemblyDbLink.AuthorSupplied.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					AssemblyDbLink.AuthorSupplied.Enum _enum = AssemblyDbLink.AuthorSupplied.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:AssemblyDbLink method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= AssemblyDbLink.AuthorSupplied.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + AssemblyDbLink.AuthorSupplied.Enum.forInt(i));
						try {
							errw.write("class_name:AssemblyDbLink method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= AssemblyDbLink.AuthorSupplied.Enum.table.lastInt(); i++)
								errw.write(" enum:" + AssemblyDbLink.AuthorSupplied.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_entry_experimental_method(AssemblyDbLink list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_AssemblyDbLink.getEntryExperimentalMethod(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ AssemblyDbLink.EntryExperimentalMethod.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					AssemblyDbLink.EntryExperimentalMethod.Enum _enum = AssemblyDbLink.EntryExperimentalMethod.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:AssemblyDbLink method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= AssemblyDbLink.EntryExperimentalMethod.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + AssemblyDbLink.EntryExperimentalMethod.Enum.forInt(i));
						try {
							errw.write("class_name:AssemblyDbLink method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= AssemblyDbLink.EntryExperimentalMethod.Enum.table.lastInt(); i++)
								errw.write(" enum:" + AssemblyDbLink.EntryExperimentalMethod.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_database_code(AssemblyDbLink list, String method_name, String nil_method_name, boolean required, String val_name, String entry_id, String accession_code, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null || accession_code != null)
			val_name = bmr_Util_AssemblyDbLink.getDatabaseCode(val_name, accession_code);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ AssemblyDbLink.DatabaseCode.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					AssemblyDbLink.DatabaseCode.Enum _enum = AssemblyDbLink.DatabaseCode.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:AssemblyDbLink method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= AssemblyDbLink.DatabaseCode.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + AssemblyDbLink.DatabaseCode.Enum.forInt(i));
						try {
							errw.write("class_name:AssemblyDbLink method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= AssemblyDbLink.DatabaseCode.Enum.table.lastInt(); i++)
								errw.write(" enum:" + AssemblyDbLink.DatabaseCode.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
