...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.PhTitrationExperimentType.*;

public class bmr_PhTitrationExperimentType {

	private static final String table_name = "PH_titration_experiment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		PhTitrationExperimentType body = PhTitrationExperimentType.Factory.newInstance();
		PhTitrationExperiment[] list = new PhTitrationExperiment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"PH_titration_list_ID\")::decimal,(0 || \"Experiment_ID\")::decimal");
				...
				buffw.write("  <BMRBx:ph_titration_experimentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='PhTitrationExperiment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = PhTitrationExperiment.Factory.newInstance();

					if (!set_string(list[0], "setExperimentName", "", true, rset.getString("Experiment_name"), logw))
						continue;
					if (!set_integer(list[0], "setSampleId", "", false, rset.getString("Sample_ID"), logw))
						continue;
					...
					body.setPhTitrationExperimentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_PhTitrationExperimentType.class.getName());
	...
	private static boolean set_string(PhTitrationExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='PhTitrationExperiment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PhTitrationExperiment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(PhTitrationExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && (nil_method_name == null || nil_method_name.isEmpty()) && (method_name.equalsIgnoreCase("setSampleID") || method_name.equalsIgnoreCase("setExperimentID")) && (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?") || !val_name.matches("^[-+]?[0-9]+$")))
			val_name = "0";
	...
	private static boolean set_enum_sample_state(PhTitrationExperiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ PhTitrationExperiment.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					PhTitrationExperiment.SampleState.Enum _enum = PhTitrationExperiment.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:PhTitrationExperiment method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= PhTitrationExperiment.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + PhTitrationExperiment.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:PhTitrationExperiment method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= PhTitrationExperiment.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + PhTitrationExperiment.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
