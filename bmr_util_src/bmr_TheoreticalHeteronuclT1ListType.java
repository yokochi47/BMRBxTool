...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalHeteronuclT1ListType.*;

public class bmr_TheoreticalHeteronuclT1ListType {

	private static final String table_name = "Theoretical_heteronucl_T1_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalHeteronuclT1ListType body = TheoreticalHeteronuclT1ListType.Factory.newInstance();
		TheoreticalHeteronuclT1List[] list = new TheoreticalHeteronuclT1List[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:theoretical_heteronucl_t1_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalHeteronuclT1List', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalHeteronuclT1List.Factory.newInstance();
					...
					if (!set_decimal(list[0], "setSpectrometerFrequency1H", "", true, rset.getString("Spectrometer_frequency_1H"), logw))
						continue;
					if (!set_enum_t1_coherence_type(list[0], "setT1CoherenceType", "", true, rset.getString("T1_coherence_type"), logw, errw))
						continue;
					...
					body.setTheoreticalHeteronuclT1ListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalHeteronuclT1ListType.class.getName());
	...
	private static boolean set_string(TheoreticalHeteronuclT1List list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalHeteronuclT1List', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalHeteronuclT1List', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalHeteronuclT1List list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(TheoreticalHeteronuclT1List list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_t1_coherence_type(TheoreticalHeteronuclT1List list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_TheoreticalHeteronuclT1List.getT1CoherenceType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ TheoreticalHeteronuclT1List.T1CoherenceType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					TheoreticalHeteronuclT1List.T1CoherenceType.Enum _enum = TheoreticalHeteronuclT1List.T1CoherenceType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:TheoreticalHeteronuclT1List method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= TheoreticalHeteronuclT1List.T1CoherenceType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + TheoreticalHeteronuclT1List.T1CoherenceType.Enum.forInt(i));
						try {
							errw.write("class_name:TheoreticalHeteronuclT1List method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= TheoreticalHeteronuclT1List.T1CoherenceType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + TheoreticalHeteronuclT1List.T1CoherenceType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_t1_val_units(TheoreticalHeteronuclT1List list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ TheoreticalHeteronuclT1List.T1ValUnits.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					TheoreticalHeteronuclT1List.T1ValUnits.Enum _enum = TheoreticalHeteronuclT1List.T1ValUnits.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:TheoreticalHeteronuclT1List method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= TheoreticalHeteronuclT1List.T1ValUnits.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + TheoreticalHeteronuclT1List.T1ValUnits.Enum.forInt(i));
						try {
							errw.write("class_name:TheoreticalHeteronuclT1List method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= TheoreticalHeteronuclT1List.T1ValUnits.Enum.table.lastInt(); i++)
								errw.write(" enum:" + TheoreticalHeteronuclT1List.T1ValUnits.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
