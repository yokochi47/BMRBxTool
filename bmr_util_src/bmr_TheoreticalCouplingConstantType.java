...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalCouplingConstantType.*;

public class bmr_TheoreticalCouplingConstantType {

	private static final String table_name = "Theoretical_coupling_constant";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalCouplingConstantType body = TheoreticalCouplingConstantType.Factory.newInstance();
		TheoreticalCouplingConstant[] list = new TheoreticalCouplingConstant[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Theoretical_coupling_constant_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:theoretical_coupling_constantCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalCouplingConstant', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalCouplingConstant.Factory.newInstance();
					...
					if (!set_integer(list[0], "setEntityId2", "", true, rset.getString("Entity_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setResonanceId1", "setNilResonanceId1", false, rset.getString("Resonance_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setResonanceId2", "setNilResonanceId2", false, rset.getString("Resonance_ID_2"), logw))
						continue;
					...
					body.setTheoreticalCouplingConstantArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalCouplingConstantType.class.getName());
	...
	private static boolean set_string(TheoreticalCouplingConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalCouplingConstant', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalCouplingConstant', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalCouplingConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(TheoreticalCouplingConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
