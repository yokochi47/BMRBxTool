...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EntityChimeraSegmentType.*;

public class bmr_EntityChimeraSegmentType {

	private static final String table_name = "Entity_chimera_segment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EntityChimeraSegmentType body = EntityChimeraSegmentType.Factory.newInstance();
		EntityChimeraSegment[] list = new EntityChimeraSegment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:entity_chimera_segmentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EntityChimeraSegment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EntityChimeraSegment.Factory.newInstance();
					...
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_integer(list[0], "setSeqIdBegin", "", true, rset.getString("Seq_ID_begin"), logw))
						continue;
					...
					body.setEntityChimeraSegmentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_EntityChimeraSegmentType.class.getName());
	...
	private static boolean set_string(EntityChimeraSegment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EntityChimeraSegment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EntityChimeraSegment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EntityChimeraSegment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
