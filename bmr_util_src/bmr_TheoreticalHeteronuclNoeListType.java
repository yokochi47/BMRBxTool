...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.TheoreticalHeteronuclNoeListType.*;

public class bmr_TheoreticalHeteronuclNoeListType {

	private static final String table_name = "Theoretical_heteronucl_NOE_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		TheoreticalHeteronuclNoeListType body = TheoreticalHeteronuclNoeListType.Factory.newInstance();
		TheoreticalHeteronuclNoeList[] list = new TheoreticalHeteronuclNoeList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:theoretical_heteronucl_noe_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='TheoreticalHeteronuclNoeList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = TheoreticalHeteronuclNoeList.Factory.newInstance();

					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_enum_heteronuclear_noe_val_type(list[0], "setHeteronuclearNoeValType", "", true, rset.getString("Heteronuclear_NOE_val_type"), logw, errw))
						continue;
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_string(list[0], "setNoeRefDescription", "setNilNoeRefDescription", false, rset.getString("NOE_ref_description"), logw))
						continue;
					if (!set_decimal(list[0], "setNoeRefVal", "setNilNoeRefVal", false, rset.getString("NOE_ref_val"), logw))
						continue;
					...
					body.setTheoreticalHeteronuclNoeListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_TheoreticalHeteronuclNoeListType.class.getName());
	...
	private static boolean set_string(TheoreticalHeteronuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='TheoreticalHeteronuclNoeList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='TheoreticalHeteronuclNoeList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(TheoreticalHeteronuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(TheoreticalHeteronuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_heteronuclear_noe_val_type(TheoreticalHeteronuclNoeList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ TheoreticalHeteronuclNoeList.HeteronuclearNoeValType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					TheoreticalHeteronuclNoeList.HeteronuclearNoeValType.Enum _enum = TheoreticalHeteronuclNoeList.HeteronuclearNoeValType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:TheoreticalHeteronuclNoeList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= TheoreticalHeteronuclNoeList.HeteronuclearNoeValType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + TheoreticalHeteronuclNoeList.HeteronuclearNoeValType.Enum.forInt(i));
						try {
							errw.write("class_name:TheoreticalHeteronuclNoeList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= TheoreticalHeteronuclNoeList.HeteronuclearNoeValType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + TheoreticalHeteronuclNoeList.HeteronuclearNoeValType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
