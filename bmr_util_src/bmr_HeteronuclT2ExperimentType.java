...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.HeteronuclT2ExperimentType.*;

public class bmr_HeteronuclT2ExperimentType {

	private static final String table_name = "Heteronucl_T2_experiment";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		HeteronuclT2ExperimentType body = HeteronuclT2ExperimentType.Factory.newInstance();
		HeteronuclT2Experiment[] list = new HeteronuclT2Experiment[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Heteronucl_T2_list_ID\")::decimal,(0 || \"Experiment_ID\")::decimal");
				...
				buffw.write("  <BMRBx:heteronucl_t2_experimentCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='HeteronuclT2Experiment', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = HeteronuclT2Experiment.Factory.newInstance();
					...
					if (!set_integer_experiment_id(list[0], "setExperimentId", "", true, rset.getString("Experiment_ID"), conn_bmrb, entry_id, rset.getString("Sample_ID"), logw))
						continue;
					if (!set_integer(list[0], "setHeteronuclT2ListId", "", true, rset.getString("Heteronucl_T2_list_ID"), logw))
						continue;

					body.setHeteronuclT2ExperimentArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_HeteronuclT2ExperimentType.class.getName());
	...
	private static boolean set_string(HeteronuclT2Experiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='HeteronuclT2Experiment', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='HeteronuclT2Experiment', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(HeteronuclT2Experiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_experiment_id(HeteronuclT2Experiment list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String sample_id, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")))
			val_name = bmr_Util_BMRB.getExperimentID(val_name, conn_bmrb, entry_id, sample_id, bmr_Util_BMRB.experiment_type.HeteronuclT2);
	...
	private static boolean set_integer_sample_id(HeteronuclT2Experiment list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String experiment_id, FileWriter logw) {
	...
	private static boolean set_enum_sample_state(HeteronuclT2Experiment list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ HeteronuclT2Experiment.SampleState.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					HeteronuclT2Experiment.SampleState.Enum _enum = HeteronuclT2Experiment.SampleState.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:HeteronuclT2Experiment method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= HeteronuclT2Experiment.SampleState.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + HeteronuclT2Experiment.SampleState.Enum.forInt(i));
						try {
							errw.write("class_name:HeteronuclT2Experiment method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= HeteronuclT2Experiment.SampleState.Enum.table.lastInt(); i++)
								errw.write(" enum:" + HeteronuclT2Experiment.SampleState.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
