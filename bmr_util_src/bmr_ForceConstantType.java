...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ForceConstantType.*;

public class bmr_ForceConstantType {

	private static final String table_name = "Force_constant";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ForceConstantType body = ForceConstantType.Factory.newInstance();
		ForceConstant[] list = new ForceConstant[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Force_constant_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:force_constantCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ForceConstant', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ForceConstant.Factory.newInstance();

					if (!set_enum_term(list[0], "setTerm", "", true, rset.getString("Term"), logw, errw))
						continue;
					if (!set_enum_units(list[0], "setUnits", "", true, rset.getString("Units"), logw, errw))
						continue;
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setForceConstantListId", "", true, rset.getString("Force_constant_list_ID"), logw))
						continue;
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;

					body.setForceConstantArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ForceConstantType.class.getName());
	...
	private static boolean set_string(ForceConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ForceConstant', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ForceConstant', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ForceConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(ForceConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_term(ForceConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ForceConstant.Term.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ForceConstant.Term.Enum _enum = ForceConstant.Term.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ForceConstant method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ForceConstant.Term.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ForceConstant.Term.Enum.forInt(i));
						try {
							errw.write("class_name:ForceConstant method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ForceConstant.Term.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ForceConstant.Term.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_units(ForceConstant list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ForceConstant.Units.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ForceConstant.Units.Enum _enum = ForceConstant.Units.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ForceConstant method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ForceConstant.Units.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ForceConstant.Units.Enum.forInt(i));
						try {
							errw.write("class_name:ForceConstant method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ForceConstant.Units.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ForceConstant.Units.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
