...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.RdcType.*;

public class bmr_RdcType {

	private static final String table_name = "RDC";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		RdcType body = RdcType.Factory.newInstance();
		Rdc[] list = new Rdc[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"RDC_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='Rdc', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = Rdc.Factory.newInstance();
					...
					if (!set_integer(list[0], "setEntityId2", "", false, rset.getString("Entity_ID_2"), logw))
						continue;
					if (!set_string(list[0], "setRdcCode", "", true, rset.getString("RDC_code"), logw))
						continue;
					...
					if (!set_decimal(list[0], "setVal", "setNilVal", false, rset.getString("Val"), logw))
						continue;
					if (!set_decimal(list[0], "setValBondLength", "setNilValBondLength", false, rset.getString("Val_bond_length"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_RdcType.class.getName());
	...
	private static boolean set_string(Rdc list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='Rdc', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='Rdc', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(Rdc list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(Rdc list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
