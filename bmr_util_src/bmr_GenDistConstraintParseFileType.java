...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.GenDistConstraintParseFileType.*;

public class bmr_GenDistConstraintParseFileType {

	private static final String table_name = "Gen_dist_constraint_parse_file";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		GenDistConstraintParseFileType body = GenDistConstraintParseFileType.Factory.newInstance();
		GenDistConstraintParseFile[] list = new GenDistConstraintParseFile[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:gen_dist_constraint_parse_fileCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='GenDistConstraintParseFile', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = GenDistConstraintParseFile.Factory.newInstance();
					...
					body.setGenDistConstraintParseFileArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_GenDistConstraintParseFileType.class.getName());
	...
	private static boolean set_string(GenDistConstraintParseFile list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='GenDistConstraintParseFile', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='GenDistConstraintParseFile', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(GenDistConstraintParseFile list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
