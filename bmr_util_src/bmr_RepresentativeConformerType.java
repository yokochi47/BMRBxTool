...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.RepresentativeConformerType.*;

public class bmr_RepresentativeConformerType {

	private static final String table_name = "Representative_conformer";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		RepresentativeConformerType body = RepresentativeConformerType.Factory.newInstance();
		RepresentativeConformer[] list = new RepresentativeConformer[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:representative_conformerCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='RepresentativeConformer', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = RepresentativeConformer.Factory.newInstance();

					if (!set_string(list[0], "setAtomPosUncertaintyDerivation", "setNilAtomPosUncertaintyDerivation", false, rset.getString("Atom_pos_uncertainty_derivation"), logw))
						continue;
					if (!set_integer(list[0], "setConformerFamilyCoordSetId", "", true, rset.getString("Conformer_family_coord_set_ID"), logw))
						continue;
					if (!set_string(list[0], "setConformerFamilyCoordSetLabel", "", true, rset.getString("Conformer_family_coord_set_label"), logw))
						continue;
					...
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_string(list[0], "setPdbAccessionCode", "setNilPdbAccessionCode", false, rset.getString("PDB_accession_code"), logw))
						continue;
					if (!set_string(list[0], "setRepConformerDerivation", "setNilRepConformerDerivation", false, rset.getString("Rep_conformer_derivation"), logw))
						continue;
					if (!set_string(list[0], "setRepConformerFileName", "setNilRepConformerFileName", false, rset.getString("Rep_conformer_file_name"), logw))
						continue;
					if (!set_string(list[0], "setRepConformerOriginalFile", "setNilRepConformerOriginalFile", false, rset.getString("Rep_conformer_original_file"), logw))
						continue;
					...
					body.setRepresentativeConformerArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_RepresentativeConformerType.class.getName());
	...
	private static boolean set_string(RepresentativeConformer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='RepresentativeConformer', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='RepresentativeConformer', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(RepresentativeConformer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(RepresentativeConformer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ RepresentativeConformer.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					RepresentativeConformer.Type.Enum _enum = RepresentativeConformer.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:RepresentativeConformer method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= RepresentativeConformer.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + RepresentativeConformer.Type.Enum.forInt(i));
						try {
							errw.write("class_name:RepresentativeConformer method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= RepresentativeConformer.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + RepresentativeConformer.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
