...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChromatographicColumnType.*;

public class bmr_ChromatographicColumnType {

	private static final String table_name = "Chromatographic_column";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChromatographicColumnType body = ChromatographicColumnType.Factory.newInstance();
		ChromatographicColumn[] list = new ChromatographicColumn[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:chromatographic_columnCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChromatographicColumn', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChromatographicColumn.Factory.newInstance();

					if (!set_decimal(list[0], "setLength", "setNilLength", false, rset.getString("Length"), logw))
						continue;
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_string(list[0], "setPackingMaterial", "setNilPackingMaterial", false, rset.getString("Packing_material"), logw))
						continue;
					if (!set_decimal(list[0], "setPackingMaterialPoreSize", "setNilPackingMaterialPoreSize", false, rset.getString("Packing_material_pore_size"), logw))
						continue;
					...
					if (!set_string(list[0], "setVendor", "setNilVendor", false, rset.getString("Vendor"), logw))
						continue;
					if (!set_decimal(list[0], "setWidth", "setNilWidth", false, rset.getString("Width"), logw))
						continue;
					...
					body.setChromatographicColumnArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChromatographicColumnType.class.getName());
	...
	private static boolean set_string(ChromatographicColumn list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChromatographicColumn', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChromatographicColumn', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChromatographicColumn list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(ChromatographicColumn list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
