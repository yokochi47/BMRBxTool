...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AuthorAnnotationType.*;

public class bmr_AuthorAnnotationType {

	private static final String table_name = "Author_annotation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AuthorAnnotationType body = AuthorAnnotationType.Factory.newInstance();
		AuthorAnnotation[] list = new AuthorAnnotation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Assembly_annotation_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:author_annotationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AuthorAnnotation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AuthorAnnotation.Factory.newInstance();

					if (!set_string(list[0], "setAnnotationCode", "", true, rset.getString("Annotation_code"), logw))
						continue;
					...
					if (!set_integer(list[0], "setAssemblyId", "", true, rset.getString("Assembly_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAssemblySubsystemId", "setNilAssemblySubsystemId", false, rset.getString("Assembly_subsystem_ID"), logw))
						continue;
					if (!set_string(list[0], "setAtomId", "setNilAtomId", false, rset.getString("Atom_ID"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSeqId", "", true, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSeqIdEnd", "setNilSeqIdEnd", false, rset.getString("Seq_ID_end"), logw))
						continue;
					if (!set_integer(list[0], "setSeqIdStart", "setNilSeqIdStart", false, rset.getString("Seq_ID_start"), logw))
						continue;
					if (!set_integer(list[0], "setAssemblyAnnotationListId", "", true, rset.getString("Assembly_annotation_list_ID"), logw))
						continue;
					...
					body.setAuthorAnnotationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AuthorAnnotationType.class.getName());
	...
	private static boolean set_string(AuthorAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AuthorAnnotation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AuthorAnnotation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AuthorAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
