...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AtomChemShiftType.*;

public class bmr_AtomChemShiftType {

	private static final String table_name = "Atom_chem_shift";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AtomChemShiftType body = AtomChemShiftType.Factory.newInstance();
		AtomChemShift[] list = new AtomChemShift[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Assigned_chem_shift_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:atom_chem_shiftCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AtomChemShift', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AtomChemShift.Factory.newInstance();

					if (!set_enum_ambiguity_code(list[0], "setAmbiguityCode", "setNilAmbiguityCode", false, rset.getString("Ambiguity_code"), entry_id, logw, errw))
						continue;
					...
					if (!set_integer(list[0], "setAssemblyAtomId", "", false, rset.getString("Assembly_atom_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setAssignFigOfMerit", "setNilAssignFigOfMerit", false, rset.getString("Assign_fig_of_merit"), logw))
						continue;
					if (!set_string(list[0], "setAtomId", "", true, rset.getString("Atom_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAtomIsotopeNumber", "", true, rset.getString("Atom_isotope_number"), logw))
						continue;
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_string(list[0], "setOccupancy", "setNilOccupancy", false, rset.getString("Occupancy"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSeqId", "", false, rset.getString("Seq_ID"), logw))
						continue;
					if (!set_decimal_val(list[0], "setVal", "", true, rset.getString("Val"), rset.getString("Ambiguity_code"), logw)) {

						try {
							logw.write("item='Val', category='AtomChemShift', value='" + rset.getString("Val") + "' was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					body.setAtomChemShiftArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AtomChemShiftType.class.getName());
	...
	private static boolean set_string(AtomChemShift list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AtomChemShift', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AtomChemShift', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AtomChemShift list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(AtomChemShift list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal_val(AtomChemShift list, String method_name, String nil_method_name, boolean required, String val_name, String ambiguity_code, FileWriter logw) {

		boolean nil = false;

		String _val_name = val_name;

		if (bmr_Util_Main.remediate_xml && (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?") || (!val_name.matches("^[-+]?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)?$") && !val_name.matches("^[-+]?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)([eE][-+]?[0-9]+)?$")) || (ambiguity_code != null && ambiguity_code.equals("73"))))
			return false;
	...
	private static boolean set_enum_ambiguity_code(AtomChemShift list, String method_name, String nil_method_name, boolean required, String val_name, String entry_id, FileWriter logw, FileWriter errw) {

		boolean nil = false;

		String _val_name = val_name;

		if (bmr_Util_Main.remediate_xml && val_name != null)
			val_name = bmr_Util_AtomChemShift.getAmbiguityCode(val_name, entry_id);
