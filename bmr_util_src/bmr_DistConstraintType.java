...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DistConstraintType.*;

public class bmr_DistConstraintType {

	private static final String table_name = "Dist_constraint";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DistConstraintType body = DistConstraintType.Factory.newInstance();
		DistConstraint[] list = new DistConstraint[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Distance_constraint_list_ID\")::decimal,(0 || \"Tree_node_member_constraint_ID\")::decimal");
				...
				buffw.write("  <BMRBx:dist_constraintCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DistConstraint', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DistConstraint.Factory.newInstance();
					...
					if (!set_integer(list[0], "setCompIndexId", "", true, rset.getString("Comp_index_ID"), logw))
						continue;
					if (!set_integer(list[0], "setConstraintTreeNodeMemberId", "", true, rset.getString("Constraint_tree_node_member_ID"), logw))
						continue;
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setTreeNodeMemberConstraintId", "", true, rset.getString("Tree_node_member_constraint_ID"), logw))
						continue;
					if (!set_integer(list[0], "setTreeNodeMemberNodeId", "", true, rset.getString("Tree_node_member_node_ID"), logw))
						continue;

					body.setDistConstraintArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DistConstraintType.class.getName());
	...
	private static boolean set_string(DistConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DistConstraint', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DistConstraint', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DistConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(DistConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
