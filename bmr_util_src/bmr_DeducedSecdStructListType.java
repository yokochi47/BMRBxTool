...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DeducedSecdStructListType.*;

public class bmr_DeducedSecdStructListType {

	private static final String table_name = "Deduced_secd_struct_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DeducedSecdStructListType body = DeducedSecdStructListType.Factory.newInstance();
		DeducedSecdStructList[] list = new DeducedSecdStructList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:deduced_secd_struct_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DeducedSecdStructList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DeducedSecdStructList.Factory.newInstance();
					...
					if (!set_string(list[0], "setName", "setNilName", false, rset.getString("Name"), logw))
						continue;
					if (!set_string(list[0], "setResidueStructValueDetails", "setNilResidueStructValueDetails", false, rset.getString("Residue_struct_value_details"), logw))
						continue;
					...
					body.setDeducedSecdStructListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DeducedSecdStructListType.class.getName());
	...
	private static boolean set_string(DeducedSecdStructList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DeducedSecdStructList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DeducedSecdStructList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DeducedSecdStructList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
