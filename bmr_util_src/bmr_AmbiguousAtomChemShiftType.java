...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.AmbiguousAtomChemShiftType.*;

public class bmr_AmbiguousAtomChemShiftType {

	private static final String table_name = "Ambiguous_atom_chem_shift";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		AmbiguousAtomChemShiftType body = AmbiguousAtomChemShiftType.Factory.newInstance();
		AmbiguousAtomChemShift[] list = new AmbiguousAtomChemShift[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Assigned_chem_shift_list_ID\")::decimal,(0 || \"Atom_chem_shift_ID\")::decimal");
				...
				buffw.write("  <BMRBx:ambiguous_atom_chem_shiftCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='AmbiguousAtomChemShift', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = AmbiguousAtomChemShift.Factory.newInstance();

					if (!set_integer(list[0], "setAmbiguousShiftSetId", "", false, rset.getString("Ambiguous_shift_set_ID"), logw))
						continue;
					...
					body.setAmbiguousAtomChemShiftArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_AmbiguousAtomChemShiftType.class.getName());
	...
	private static boolean set_string(AmbiguousAtomChemShift list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='AmbiguousAtomChemShift', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='AmbiguousAtomChemShift', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(AmbiguousAtomChemShift list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
