...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EntityPurityType.*;

public class bmr_EntityPurityType {

	private static final String table_name = "Entity_purity";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EntityPurityType body = EntityPurityType.Factory.newInstance();
		EntityPurity[] list = new EntityPurity[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Entity_purity_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:entity_purityCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EntityPurity', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EntityPurity.Factory.newInstance();
					...
					if (!set_string(list[0], "setEntityLabel", "", true, rset.getString("Entity_label"), logw))
						continue;
					if (!set_string(list[0], "setMeasurementMethod", "", true, rset.getString("Measurement_method"), logw))
						continue;
					...
					if (!set_decimal(list[0], "setVal", "", true, rset.getString("Val"), logw))
						continue;
					if (!set_string(list[0], "setValUnits", "", true, rset.getString("Val_units"), logw))
						continue;
					if (!set_integer(list[0], "setEntityPurityListId", "", true, rset.getString("Entity_purity_list_ID"), logw))
						continue;
					...
					body.setEntityPurityArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_EntityPurityType.class.getName());
	...
	private static boolean set_string(EntityPurity list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EntityPurity', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EntityPurity', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EntityPurity list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_sample_id(EntityPurity list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, FileWriter logw) {
	...
	private static boolean set_decimal(EntityPurity list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
