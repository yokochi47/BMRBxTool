...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.StudyEntryListType.*;

public class bmr_StudyEntryListType {

	private static final String table_name = "Study_entry_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		StudyEntryListType body = StudyEntryListType.Factory.newInstance();
		StudyEntryList[] list = new StudyEntryList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Study_list_ID\")::decimal,(0 || \"Study_ID\")::decimal");
				...
				buffw.write("  <BMRBx:study_entry_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='StudyEntryList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = StudyEntryList.Factory.newInstance();

					if (!set_string(list[0], "setBmrbEntryDescription", "setNilBmrbEntryDescription", false, rset.getString("BMRB_entry_description"), logw))
						continue;
					if (!set_string(list[0], "setDetails", "setNilDetails", false, rset.getString("Details"), logw))
						continue;
					if (!set_string(list[0], "setBmrbAccessionCode", "", true, rset.getString("BMRB_accession_code"), logw)) {

						try {
							logw.write("item='BmrbAccessionCode', category='StudyEntryList', value='" + rset.getString("BMRB_accession_code") + "' was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer_study_id(list[0], "setStudyId", "", true, rset.getString("Study_ID"), conn_bmrb, entry_id, rset.getString("Study_list_ID"), logw))
						continue;
					if (!set_integer(list[0], "setStudyListId", "", true, rset.getString("Study_list_ID"), logw))
						continue;

					body.setStudyEntryListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_StudyEntryListType.class.getName());
	...
	private static boolean set_string(StudyEntryList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='StudyEntryList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='StudyEntryList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(StudyEntryList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_study_id(StudyEntryList list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, String study_list_id, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")))
			val_name = bmr_Util_BMRB.getStudyID(val_name, conn_bmrb, entry_id, study_list_id);
