...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemStructDescriptorType.*;

public class bmr_ChemStructDescriptorType {

	private static final String table_name = "Chem_struct_descriptor";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemStructDescriptorType body = ChemStructDescriptorType.Factory.newInstance();
		ChemStructDescriptor[] list = new ChemStructDescriptor[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:chem_struct_descriptorCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemStructDescriptor', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemStructDescriptor.Factory.newInstance();

					if (!set_integer(list[0], "setCount", "", true, rset.getString("Count"), logw))
						continue;
					if (!set_enum_descriptor(list[0], "setDescriptor", "", true, rset.getString("Descriptor"), logw, errw))
						continue;
					if (!set_integer(list[0], "setSfId", "", true, rset.getString("Sf_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSoftwareId", "setNilSoftwareId", false, rset.getString("Software_ID"), logw))
						continue;
					...
					body.setChemStructDescriptorArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemStructDescriptorType.class.getName());
	...
	private static boolean set_string(ChemStructDescriptor list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemStructDescriptor', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemStructDescriptor', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChemStructDescriptor list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_descriptor(ChemStructDescriptor list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemStructDescriptor.Descriptor.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemStructDescriptor.Descriptor.Enum _enum = ChemStructDescriptor.Descriptor.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemStructDescriptor method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemStructDescriptor.Descriptor.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemStructDescriptor.Descriptor.Enum.forInt(i));
						try {
							errw.write("class_name:ChemStructDescriptor method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemStructDescriptor.Descriptor.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemStructDescriptor.Descriptor.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_type(ChemStructDescriptor list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemStructDescriptor.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemStructDescriptor.Type.Enum _enum = ChemStructDescriptor.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemStructDescriptor method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemStructDescriptor.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemStructDescriptor.Type.Enum.forInt(i));
						try {
							errw.write("class_name:ChemStructDescriptor method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemStructDescriptor.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemStructDescriptor.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
