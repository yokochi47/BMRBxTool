...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SpectralDimTransferType.*;

public class bmr_SpectralDimTransferType {

	private static final String table_name = "Spectral_dim_transfer";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SpectralDimTransferType body = SpectralDimTransferType.Factory.newInstance();
		SpectralDimTransfer[] list = new SpectralDimTransfer[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Spectral_peak_list_ID\")::decimal");
				...
				buffw.write("  <BMRBx:spectral_dim_transferCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SpectralDimTransfer', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SpectralDimTransfer.Factory.newInstance();

					if (!set_enum_indirect(list[0], "setIndirect", "setNilIndirect", false, rset.getString("Indirect"), logw, errw))
						continue;
					...
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSpectralDimId1", "", true, rset.getString("Spectral_dim_ID_1"), logw))
						continue;
					if (!set_integer(list[0], "setSpectralDimId2", "", true, rset.getString("Spectral_dim_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setSpectralPeakListId", "", true, rset.getString("Spectral_peak_list_ID"), logw))
						continue;

					body.setSpectralDimTransferArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SpectralDimTransferType.class.getName());
	...
	private static boolean set_string(SpectralDimTransfer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SpectralDimTransfer', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SpectralDimTransfer', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SpectralDimTransfer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_indirect(SpectralDimTransfer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ SpectralDimTransfer.Indirect.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					SpectralDimTransfer.Indirect.Enum _enum = SpectralDimTransfer.Indirect.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:SpectralDimTransfer method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= SpectralDimTransfer.Indirect.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + SpectralDimTransfer.Indirect.Enum.forInt(i));
						try {
							errw.write("class_name:SpectralDimTransfer method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= SpectralDimTransfer.Indirect.Enum.table.lastInt(); i++)
								errw.write(" enum:" + SpectralDimTransfer.Indirect.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_type(SpectralDimTransfer list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_SpectralDimTransfer.getType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ SpectralDimTransfer.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					SpectralDimTransfer.Type.Enum _enum = SpectralDimTransfer.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:SpectralDimTransfer method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= SpectralDimTransfer.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + SpectralDimTransfer.Type.Enum.forInt(i));
						try {
							errw.write("class_name:SpectralDimTransfer method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= SpectralDimTransfer.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + SpectralDimTransfer.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
