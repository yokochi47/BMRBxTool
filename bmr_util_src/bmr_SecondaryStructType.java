...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SecondaryStructType.*;

public class bmr_SecondaryStructType {

	private static final String table_name = "Secondary_struct";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SecondaryStructType body = SecondaryStructType.Factory.newInstance();
		SecondaryStruct[] list = new SecondaryStruct[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Secondary_struct_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:secondary_structCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SecondaryStruct', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SecondaryStruct.Factory.newInstance();
					...
					if (!set_integer(list[0], "setConfFamilyCoordSetId", "", true, rset.getString("Conf_family_coord_set_ID"), logw))
						continue;
					if (!set_string(list[0], "setConfFamilyCoordSetLabel", "setNilConfFamilyCoordSetLabel", false, rset.getString("Conf_family_coord_set_label"), logw))
						continue;
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setSecondaryStructListId", "", true, rset.getString("Secondary_struct_list_ID"), logw))
						continue;

					body.setSecondaryStructArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SecondaryStructType.class.getName());
	...
	private static boolean set_string(SecondaryStruct list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SecondaryStruct', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SecondaryStruct', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SecondaryStruct list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
