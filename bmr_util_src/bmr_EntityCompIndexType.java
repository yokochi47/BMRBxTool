...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EntityCompIndexType.*;

public class bmr_EntityCompIndexType {

	private static final String table_name = "Entity_comp_index";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EntityCompIndexType body = EntityCompIndexType.Factory.newInstance();
		EntityCompIndex[] list = new EntityCompIndex[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:entity_comp_indexCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EntityCompIndex', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EntityCompIndex.Factory.newInstance();
					...
					if (comp_id == null || comp_id.isEmpty() || comp_id.equals(".") || comp_id.equals("?")) {
						...
						try {
							logw.write("category='EntityCompIndex', content of null comp_id was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					body.setEntityCompIndexArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_EntityCompIndexType.class.getName());
	...
	private static boolean set_string(EntityCompIndex list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EntityCompIndex', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EntityCompIndex', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EntityCompIndex list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
