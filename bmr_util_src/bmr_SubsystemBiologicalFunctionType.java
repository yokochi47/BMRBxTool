...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SubsystemBiologicalFunctionType.*;

public class bmr_SubsystemBiologicalFunctionType {

	private static final String table_name = "Subsystem_biological_function";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SubsystemBiologicalFunctionType body = SubsystemBiologicalFunctionType.Factory.newInstance();
		SubsystemBiologicalFunction[] list = new SubsystemBiologicalFunction[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:subsystem_biological_functionCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SubsystemBiologicalFunction', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SubsystemBiologicalFunction.Factory.newInstance();

					if (!set_integer(list[0], "setAssemblySubsystemId", "", true, rset.getString("Assembly_subsystem_ID"), logw))
						continue;
					if (!set_string(list[0], "setBiologicalFunction", "", true, rset.getString("Biological_function"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;

					body.setSubsystemBiologicalFunctionArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SubsystemBiologicalFunctionType.class.getName());
	...
	private static boolean set_string(SubsystemBiologicalFunction list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SubsystemBiologicalFunction', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SubsystemBiologicalFunction', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SubsystemBiologicalFunction list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
