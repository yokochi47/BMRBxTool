...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.ChemShiftAnisotropyType.*;

public class bmr_ChemShiftAnisotropyType {

	private static final String table_name = "Chem_shift_anisotropy";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		ChemShiftAnisotropyType body = ChemShiftAnisotropyType.Factory.newInstance();
		ChemShiftAnisotropy[] list = new ChemShiftAnisotropy[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:chem_shift_anisotropyCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='ChemShiftAnisotropy', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = ChemShiftAnisotropy.Factory.newInstance();
					...
					if (!set_string(list[0], "setSfFramecode", "", true, rset.getString("Sf_framecode"), logw))
						continue;
					if (!set_decimal(list[0], "setSpectrometerFrequency1H", "setNilSpectrometerFrequency1H", false, rset.getString("Spectrometer_frequency_1H"), logw))
						continue;
					...
					body.setChemShiftAnisotropyArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_ChemShiftAnisotropyType.class.getName());
	...
	private static boolean set_string(ChemShiftAnisotropy list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='ChemShiftAnisotropy', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='ChemShiftAnisotropy', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(ChemShiftAnisotropy list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(ChemShiftAnisotropy list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_val_units(ChemShiftAnisotropy list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ ChemShiftAnisotropy.ValUnits.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					ChemShiftAnisotropy.ValUnits.Enum _enum = ChemShiftAnisotropy.ValUnits.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:ChemShiftAnisotropy method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= ChemShiftAnisotropy.ValUnits.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + ChemShiftAnisotropy.ValUnits.Enum.forInt(i));
						try {
							errw.write("class_name:ChemShiftAnisotropy method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= ChemShiftAnisotropy.ValUnits.Enum.table.lastInt(); i++)
								errw.write(" enum:" + ChemShiftAnisotropy.ValUnits.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
