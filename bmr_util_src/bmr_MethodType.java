...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MethodType;

public class bmr_MethodType {

	private static final String table_name = "Method";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MethodType body = MethodType.Factory.newInstance();
		org.pdbj.bmrbpub.schema.mmcifNmrStar.MethodType.Method[] list = new org.pdbj.bmrbpub.schema.mmcifNmrStar.MethodType.Method[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:methodCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='Method', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = org.pdbj.bmrbpub.schema.mmcifNmrStar.MethodType.Method.Factory.newInstance();

					if (!set_integer(list[0], "setComputerId", "setNilComputerId", false, rset.getString("Computer_ID"), logw))
						continue;
					if (!set_string(list[0], "setComputerLabel", "setNilComputerLabel", false, rset.getString("Computer_label"), logw))
						continue;
					if (!set_string(list[0], "setDerivationType", "", true, rset.getString("Derivation_type"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_MethodType.class.getName());
	...
	private static boolean set_string(org.pdbj.bmrbpub.schema.mmcifNmrStar.MethodType.Method list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='Method', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='Method', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(org.pdbj.bmrbpub.schema.mmcifNmrStar.MethodType.Method list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
