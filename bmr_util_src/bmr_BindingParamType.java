...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.BindingParamType.*;

public class bmr_BindingParamType {

	private static final String table_name = "Binding_param";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		BindingParamType body = BindingParamType.Factory.newInstance();
		BindingParam[] list = new BindingParam[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Binding_param_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:binding_paramCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='BindingParam', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = BindingParam.Factory.newInstance();

					if (!set_integer(list[0], "setBindingResultId", "", true, rset.getString("Binding_result_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setConcentrationVal", "", true, rset.getString("Concentration_val"), logw))
						continue;
					if (!set_decimal(list[0], "setConcentrationValErr", "setNilConcentrationValErr", false, rset.getString("Concentration_val_err"), logw))
						continue;
					if (!set_string(list[0], "setConcentrationValUnits", "setNilConcentrationValUnits", false, rset.getString("Concentration_val_units"), logw))
						continue;
					if (!set_string(list[0], "setObservedParam", "setNilObservedParam", false, rset.getString("Observed_param"), logw))
						continue;
					if (!set_decimal(list[0], "setObservedParamVal", "", true, rset.getString("Observed_param_val"), logw))
						continue;
					if (!set_decimal(list[0], "setObservedParamValErr", "setNilObservedParamValErr", false, rset.getString("Observed_param_val_err"), logw))
						continue;
					if (!set_integer(list[0], "setBindingParamListId", "", true, rset.getString("Binding_param_list_ID"), logw))
						continue;
					...
					body.setBindingParamArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_BindingParamType.class.getName());
	...
	private static boolean set_string(BindingParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='BindingParam', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='BindingParam', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(BindingParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(BindingParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
