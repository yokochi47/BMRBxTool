...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.PbCharType.*;

public class bmr_PbCharType {

	private static final String table_name = "PB_char";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		PbCharType body = PbCharType.Factory.newInstance();
		PbChar[] list = new PbChar[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"PB_list_ID\")::decimal");
				...
				buffw.write("  <BMRBx:pb_charCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='PbChar', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = PbChar.Factory.newInstance();

					if (!set_string(list[0], "setAlign", "setNilAlign", false, rset.getString("Align"), logw))
						continue;
					...
					if (!set_integer(list[0], "setEntityId", "", true, rset.getString("Entity_ID"), logw))
						continue;
					if (!set_enum_pb_code(list[0], "setPbCode", "setNilPbCode", false, rset.getString("PB_code"), logw, errw))
						continue;
					...
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setPbListId", "", true, rset.getString("PB_list_ID"), logw))
						continue;

					body.setPbCharArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_PbCharType.class.getName());
	...
	private static boolean set_string(PbChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='PbChar', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PbChar', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(PbChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_pb_code(PbChar list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ PbChar.PbCode.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					PbChar.PbCode.Enum _enum = PbChar.PbCode.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:PbChar method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= PbChar.PbCode.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + PbChar.PbCode.Enum.forInt(i));
						try {
							errw.write("class_name:PbChar method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= PbChar.PbCode.Enum.table.lastInt(); i++)
								errw.write(" enum:" + PbChar.PbCode.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
