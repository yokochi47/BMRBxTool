...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.LocalStructureQualityType.*;

public class bmr_LocalStructureQualityType {

	private static final String table_name = "Local_structure_quality";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		LocalStructureQualityType body = LocalStructureQualityType.Factory.newInstance();
		LocalStructureQuality[] list = new LocalStructureQuality[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:local_structure_qualityCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='LocalStructureQuality', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = LocalStructureQuality.Factory.newInstance();

					if (!set_string(list[0], "setAsymId", "", true, rset.getString("Asym_ID"), logw))
						continue;
					if (!set_integer(list[0], "setCompIndexIdBegin", "", true, rset.getString("Comp_index_ID_begin"), logw))
						continue;
					...
					body.setLocalStructureQualityArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_LocalStructureQualityType.class.getName());
	...
	private static boolean set_string(LocalStructureQuality list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='LocalStructureQuality', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='LocalStructureQuality', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(LocalStructureQuality list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(LocalStructureQuality list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ LocalStructureQuality.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					LocalStructureQuality.Type.Enum _enum = LocalStructureQuality.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:LocalStructureQuality method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= LocalStructureQuality.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + LocalStructureQuality.Type.Enum.forInt(i));
						try {
							errw.write("class_name:LocalStructureQuality method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= LocalStructureQuality.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + LocalStructureQuality.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
