...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.EntityBondType.*;

public class bmr_EntityBondType {

	private static final String table_name = "Entity_bond";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		EntityBondType body = EntityBondType.Factory.newInstance();
		EntityBond[] list = new EntityBond[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Entity_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:entity_bondCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='EntityBond', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = EntityBond.Factory.newInstance();
			...
			Logger lgr = Logger.getLogger(bmr_EntityBondType.class.getName());
	...
	private static boolean set_string(EntityBond list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='EntityBond', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='EntityBond', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(EntityBond list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(EntityBond list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ EntityBond.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					EntityBond.Type.Enum _enum = EntityBond.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:EntityBond method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= EntityBond.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + EntityBond.Type.Enum.forInt(i));
						try {
							errw.write("class_name:EntityBond method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= EntityBond.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + EntityBond.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_value_order(EntityBond list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_EntityBond.getValueOrder(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ EntityBond.ValueOrder.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					EntityBond.ValueOrder.Enum _enum = EntityBond.ValueOrder.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:EntityBond method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= EntityBond.ValueOrder.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + EntityBond.ValueOrder.Enum.forInt(i));
						try {
							errw.write("class_name:EntityBond method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= EntityBond.ValueOrder.Enum.table.lastInt(); i++)
								errw.write(" enum:" + EntityBond.ValueOrder.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
