...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SaxsExptType.*;

public class bmr_SaxsExptType {

	private static final String table_name = "SAXS_expt";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SaxsExptType body = SaxsExptType.Factory.newInstance();
		SaxsExpt[] list = new SaxsExpt[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:saxs_exptCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SaxsExpt', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SaxsExpt.Factory.newInstance();
					...
					if (!set_string(list[0], "setSoftwareLabel", "setNilSoftwareLabel", false, rset.getString("Software_label"), logw))
						continue;
					if (!set_integer(list[0], "setXrayInstrumentId", "", true, rset.getString("Xray_instrument_ID"), logw))
						continue;
			...
			Logger lgr = Logger.getLogger(bmr_SaxsExptType.class.getName());
	...
	private static boolean set_string(SaxsExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SaxsExpt', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SaxsExpt', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SaxsExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(SaxsExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_sample_volume_units(SaxsExpt list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ SaxsExpt.SampleVolumeUnits.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					SaxsExpt.SampleVolumeUnits.Enum _enum = SaxsExpt.SampleVolumeUnits.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:SaxsExpt method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= SaxsExpt.SampleVolumeUnits.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + SaxsExpt.SampleVolumeUnits.Enum.forInt(i));
						try {
							errw.write("class_name:SaxsExpt method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= SaxsExpt.SampleVolumeUnits.Enum.table.lastInt(); i++)
								errw.write(" enum:" + SaxsExpt.SampleVolumeUnits.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
