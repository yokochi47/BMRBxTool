...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.SoftwareSpecificInfoType.*;

public class bmr_SoftwareSpecificInfoType {

	private static final String table_name = "Software_specific_info";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		SoftwareSpecificInfoType body = SoftwareSpecificInfoType.Factory.newInstance();
		SoftwareSpecificInfo[] list = new SoftwareSpecificInfo[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Software_specific_info_list_ID\")::decimal,(0 || \"Software_saveframe_ID\")::decimal");
				...
				buffw.write("  <BMRBx:software_specific_infoCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='SoftwareSpecificInfo', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = SoftwareSpecificInfo.Factory.newInstance();

					if (!set_string(list[0], "setSoftwareSaveframe", "", true, rset.getString("Software_saveframe"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSoftwareSaveframeId", "", true, rset.getString("Software_saveframe_ID"), logw))
						continue;
					if (!set_integer(list[0], "setSoftwareSpecificInfoListId", "", true, rset.getString("Software_specific_info_list_ID"), logw))
						continue;

					body.setSoftwareSpecificInfoArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_SoftwareSpecificInfoType.class.getName());
	...
	private static boolean set_string(SoftwareSpecificInfo list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='SoftwareSpecificInfo', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='SoftwareSpecificInfo', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(SoftwareSpecificInfo list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
