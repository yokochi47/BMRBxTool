...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.DistanceConstraintListType.*;

public class bmr_DistanceConstraintListType {

	private static final String table_name = "Distance_constraint_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		DistanceConstraintListType body = DistanceConstraintListType.Factory.newInstance();
		DistanceConstraintList[] list = new DistanceConstraintList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:distance_constraint_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='DistanceConstraintList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = DistanceConstraintList.Factory.newInstance();
					...
					body.setDistanceConstraintListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_DistanceConstraintListType.class.getName());
	...
	private static boolean set_string(DistanceConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='DistanceConstraintList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='DistanceConstraintList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(DistanceConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_constraint_type(DistanceConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ DistanceConstraintList.ConstraintType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					DistanceConstraintList.ConstraintType.Enum _enum = DistanceConstraintList.ConstraintType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:DistanceConstraintList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= DistanceConstraintList.ConstraintType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + DistanceConstraintList.ConstraintType.Enum.forInt(i));
						try {
							errw.write("class_name:DistanceConstraintList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= DistanceConstraintList.ConstraintType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + DistanceConstraintList.ConstraintType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
