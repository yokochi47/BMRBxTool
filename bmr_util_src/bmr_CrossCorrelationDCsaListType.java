...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.CrossCorrelationDCsaListType.*;

public class bmr_CrossCorrelationDCsaListType {

	private static final String table_name = "Cross_correlation_D_CSA_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		CrossCorrelationDCsaListType body = CrossCorrelationDCsaListType.Factory.newInstance();
		CrossCorrelationDCsaList[] list = new CrossCorrelationDCsaList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:cross_correlation_d_csa_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='CrossCorrelationDCsaList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = CrossCorrelationDCsaList.Factory.newInstance();
					...
					body.setCrossCorrelationDCsaListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_CrossCorrelationDCsaListType.class.getName());
	...
	private static boolean set_string(CrossCorrelationDCsaList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='CrossCorrelationDCsaList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='CrossCorrelationDCsaList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(CrossCorrelationDCsaList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(CrossCorrelationDCsaList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_val_units(CrossCorrelationDCsaList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ CrossCorrelationDCsaList.ValUnits.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					CrossCorrelationDCsaList.ValUnits.Enum _enum = CrossCorrelationDCsaList.ValUnits.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:CrossCorrelationDCsaList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= CrossCorrelationDCsaList.ValUnits.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + CrossCorrelationDCsaList.ValUnits.Enum.forInt(i));
						try {
							errw.write("class_name:CrossCorrelationDCsaList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= CrossCorrelationDCsaList.ValUnits.Enum.table.lastInt(); i++)
								errw.write(" enum:" + CrossCorrelationDCsaList.ValUnits.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
