...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.GenDistConstraintListType.*;

public class bmr_GenDistConstraintListType {

	private static final String table_name = "Gen_dist_constraint_list";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		GenDistConstraintListType body = GenDistConstraintListType.Factory.newInstance();
		GenDistConstraintList[] list = new GenDistConstraintList[1];
		...
		try {
			...
			if (rset.next()) {
				...
				buffw.write("  <BMRBx:gen_dist_constraint_listCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='GenDistConstraintList', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = GenDistConstraintList.Factory.newInstance();
					...
					if (!set_integer(list[0], "setConstraintFileId", "setNilConstraintFileId", false, rset.getString("Constraint_file_ID"), logw))
						continue;
					if (!set_enum_constraint_type(list[0], "setConstraintType", "", true, rset.getString("Constraint_type"), logw, errw))
						continue;
					...
					body.setGenDistConstraintListArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_GenDistConstraintListType.class.getName());
	...
	private static boolean set_string(GenDistConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='GenDistConstraintList', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='GenDistConstraintList', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(GenDistConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_constraint_type(GenDistConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		if (val_name != null)
			val_name = bmr_Util_GenDistConstraintList.getConstraintType(val_name);
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ GenDistConstraintList.ConstraintType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					GenDistConstraintList.ConstraintType.Enum _enum = GenDistConstraintList.ConstraintType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:GenDistConstraintList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= GenDistConstraintList.ConstraintType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + GenDistConstraintList.ConstraintType.Enum.forInt(i));
						try {
							errw.write("class_name:GenDistConstraintList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= GenDistConstraintList.ConstraintType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + GenDistConstraintList.ConstraintType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_potential_type(GenDistConstraintList list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ GenDistConstraintList.PotentialType.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					GenDistConstraintList.PotentialType.Enum _enum = GenDistConstraintList.PotentialType.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:GenDistConstraintList method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= GenDistConstraintList.PotentialType.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + GenDistConstraintList.PotentialType.Enum.forInt(i));
						try {
							errw.write("class_name:GenDistConstraintList method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= GenDistConstraintList.PotentialType.Enum.table.lastInt(); i++)
								errw.write(" enum:" + GenDistConstraintList.PotentialType.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
