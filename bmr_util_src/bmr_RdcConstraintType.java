...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.RdcConstraintType.*;

public class bmr_RdcConstraintType {

	private static final String table_name = "RDC_constraint";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		RdcConstraintType body = RdcConstraintType.Factory.newInstance();
		RdcConstraint[] list = new RdcConstraint[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"RDC_constraint_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:rdc_constraintCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='RdcConstraint', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = RdcConstraint.Factory.newInstance();
					...
					if (!set_string(list[0], "setPdbStrandId2", "setNilPdbStrandId2", false, rset.getString("PDB_strand_ID_2"), logw))
						continue;
					if (!set_decimal(list[0], "setRdcBondLength", "setNilRdcBondLength", false, rset.getString("RDC_bond_length"), logw))
						continue;
					if (!set_enum_rdc_distant_dependent(list[0], "setRdcDistantDependent", "setNilRdcDistantDependent", false, rset.getString("RDC_distant_dependent"), logw, errw))
						continue;
					if (!set_decimal(list[0], "setRdcLowerBound", "setNilRdcLowerBound", false, rset.getString("RDC_lower_bound"), logw))
						continue;
					if (!set_decimal(list[0], "setRdcLowerLinearLimit", "setNilRdcLowerLinearLimit", false, rset.getString("RDC_lower_linear_limit"), logw))
						continue;
					if (!set_decimal(list[0], "setRdcUpperBound", "setNilRdcUpperBound", false, rset.getString("RDC_upper_bound"), logw))
						continue;
					if (!set_decimal(list[0], "setRdcUpperLinearLimit", "setNilRdcUpperLinearLimit", false, rset.getString("RDC_upper_linear_limit"), logw))
						continue;
					if (!set_decimal(list[0], "setRdcVal", "", true, rset.getString("RDC_val"), logw))
						continue;
					if (!set_decimal(list[0], "setRdcValErr", "setNilRdcValErr", false, rset.getString("RDC_val_err"), logw))
						continue;
					if (!set_decimal(list[0], "setRdcValScaleFactor", "setNilRdcValScaleFactor", false, rset.getString("RDC_val_scale_factor"), logw))
						continue;
					...
					if (!set_integer(list[0], "setSourceExperimentId", "", false, rset.getString("Source_experiment_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setTargetValue", "setNilTargetValue", false, rset.getString("Target_value"), logw))
						continue;
					if (!set_decimal(list[0], "setTargetValueUncertainty", "setNilTargetValueUncertainty", false, rset.getString("Target_value_uncertainty"), logw))
						continue;
					...
					if (!set_integer(list[0], "setIndexId", "", true, rset.getString("ID"), logw))
						continue;
					if (!set_integer(list[0], "setRdcConstraintListId", "", true, rset.getString("RDC_constraint_list_ID"), logw))
						continue;

					body.setRdcConstraintArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_RdcConstraintType.class.getName());
	...
	private static boolean set_string(RdcConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='RdcConstraint', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='RdcConstraint', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(RdcConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(RdcConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?") || (!val_name.matches("^[-+]?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)?$") && !val_name.matches("^[-+]?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)([eE][-+]?[0-9]+)?$"))) && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (method_name.equalsIgnoreCase("setRDCVal"))
				val_name = "0.0";
	...
	private static boolean set_enum_rdc_distant_dependent(RdcConstraint list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ RdcConstraint.RdcDistantDependent.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					RdcConstraint.RdcDistantDependent.Enum _enum = RdcConstraint.RdcDistantDependent.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:RdcConstraint method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= RdcConstraint.RdcDistantDependent.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + RdcConstraint.RdcDistantDependent.Enum.forInt(i));
						try {
							errw.write("class_name:RdcConstraint method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= RdcConstraint.RdcDistantDependent.Enum.table.lastInt(); i++)
								errw.write(" enum:" + RdcConstraint.RdcDistantDependent.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
