...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.PhParamType.*;

public class bmr_PhParamType {

	private static final String table_name = "PH_param";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		PhParamType body = PhParamType.Factory.newInstance();
		PhParam[] list = new PhParam[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"PH_param_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:ph_paramCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='PhParam', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = PhParam.Factory.newInstance();

					if (!set_decimal(list[0], "setObservedNmrParamVal", "", true, rset.getString("Observed_NMR_param_val"), logw))
						continue;
					if (!set_decimal(list[0], "setObservedNmrParamValErr", "setNilObservedNmrParamValErr", false, rset.getString("Observed_NMR_param_val_err"), logw))
						continue;
					if (!set_integer(list[0], "setPhTitrResultId", "", false, rset.getString("PH_titr_result_ID"), logw))
						continue;
					if (!set_decimal(list[0], "setPhVal", "", true, rset.getString("PH_val"), logw))
						continue;
					if (!set_decimal(list[0], "setPhValErr", "setNilPhValErr", false, rset.getString("PH_val_err"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setPhParamListId", "", true, rset.getString("PH_param_list_ID"), logw))
						continue;

					body.setPhParamArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_PhParamType.class.getName());
	...
	private static boolean set_string(PhParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='PhParam', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='PhParam', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(PhParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(PhParam list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml && (val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?") || (!val_name.matches("^[-+]?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)?$") && !val_name.matches("^[-+]?([0-9]+(\\.[0-9]*)?|\\.[0-9]+)([eE][-+]?[0-9]+)?$"))) && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (method_name.equalsIgnoreCase("setPHVal"))
				val_name = "0.0";

			if (method_name.equalsIgnoreCase("setObservedNMRParamVal"))
				val_name = "0.0";
