...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.BondAnnotationType.*;

public class bmr_BondAnnotationType {

	private static final String table_name = "Bond_annotation";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		BondAnnotationType body = BondAnnotationType.Factory.newInstance();
		BondAnnotation[] list = new BondAnnotation[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Bond_annotation_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:bond_annotationCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='BondAnnotation', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = BondAnnotation.Factory.newInstance();
					...
					if (!set_integer(list[0], "setEntityId2", "", true, rset.getString("Entity_ID_2"), logw))
						continue;
					if (!set_integer(list[0], "setSelectionMethodId", "", true, rset.getString("Selection_method_ID"), logw))
						continue;
					...
					if (!set_enum_value_order(list[0], "setValueOrder", "", true, rset.getString("Value_order"), logw, errw))
						continue;
					if (!set_integer(list[0], "setBondAnnotationListId", "", true, rset.getString("Bond_annotation_list_ID"), logw))
						continue;
					...
					body.setBondAnnotationArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_BondAnnotationType.class.getName());
	...
	private static boolean set_string(BondAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='BondAnnotation', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='BondAnnotation', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(BondAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_enum_type(BondAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ BondAnnotation.Type.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					BondAnnotation.Type.Enum _enum = BondAnnotation.Type.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:BondAnnotation method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= BondAnnotation.Type.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + BondAnnotation.Type.Enum.forInt(i));
						try {
							errw.write("class_name:BondAnnotation method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= BondAnnotation.Type.Enum.table.lastInt(); i++)
								errw.write(" enum:" + BondAnnotation.Type.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
	...
	private static boolean set_enum_value_order(BondAnnotation list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw, FileWriter errw) {
		...
		try {
			Method method = _class.getMethod(method_name, new Class[]{ BondAnnotation.ValueOrder.Enum.class });
			;//Method nil_method = null;
			...
			try {
				if (nil)
					...
					BondAnnotation.ValueOrder.Enum _enum = BondAnnotation.ValueOrder.Enum.forString(val_name);
					if (_enum != null)
						method.invoke(list, _enum);
					else {
						System.err.println("class_name:BondAnnotation method_name:" + method_name + " val_name:" + val_name);
						for (int i = 1; i <= BondAnnotation.ValueOrder.Enum.table.lastInt(); i++)
							System.err.println(" enum:" + BondAnnotation.ValueOrder.Enum.forInt(i));
						try {
							errw.write("class_name:BondAnnotation method_name:" + method_name + " val_name:" + val_name + "\n");
							for (int i = 1; i <= BondAnnotation.ValueOrder.Enum.table.lastInt(); i++)
								errw.write(" enum:" + BondAnnotation.ValueOrder.Enum.forInt(i) + "\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
						//if (nil_method != null)
