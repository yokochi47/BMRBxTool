...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.MsChromatogramIonType.*;

public class bmr_MsChromatogramIonType {

	private static final String table_name = "MS_chromatogram_ion";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		MsChromatogramIonType body = MsChromatogramIonType.Factory.newInstance();
		MsChromatogramIon[] list = new MsChromatogramIon[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"MS_chromatogram_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:ms_chromatogram_ionCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='MsChromatogramIon', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = MsChromatogramIon.Factory.newInstance();

					if (!set_decimal(list[0], "setFullWidthHalfMaximum", "setNilFullWidthHalfMaximum", false, rset.getString("Full_width_half_maximum"), logw))
						continue;
					if (!set_decimal(list[0], "setMzIntensity", "", true, rset.getString("MZ_Intensity"), logw))
						continue;
					if (!set_decimal(list[0], "setMzRatio", "", true, rset.getString("MZ_ratio"), logw))
						continue;
					if (!set_decimal(list[0], "setMzRatioErr", "setNilMzRatioErr", false, rset.getString("MZ_ratio_err"), logw))
						continue;
					if (!set_decimal(list[0], "setMzRelativeIntensity", "setNilMzRelativeIntensity", false, rset.getString("MZ_relative_intensity"), logw))
						continue;
					if (!set_decimal(list[0], "setSNRatio", "setNilSNRatio", false, rset.getString("S_N_ratio"), logw))
						continue;
					if (!set_string(list[0], "setEntryId", "", true, rset.getString("Entry_ID"), logw))
						continue;
					if (!set_integer(list[0], "setId", "", true, rid[lines - 1], logw))
						continue;
					if (!set_integer(list[0], "setMsChromatogramListId", "", true, rset.getString("MS_chromatogram_list_ID"), logw))
						continue;

					body.setMsChromatogramIonArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_MsChromatogramIonType.class.getName());
	...
	private static boolean set_string(MsChromatogramIon list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='MsChromatogramIon', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='MsChromatogramIon', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(MsChromatogramIon list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_decimal(MsChromatogramIon list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
