...
import org.pdbj.bmrbpub.schema.mmcifNmrStar.BindingResultType.*;

public class bmr_BindingResultType {

	private static final String table_name = "Binding_result";
	...
	public static int write_xml(Connection conn_bmrb, Connection conn_tax, Connection conn_le, String entry_id, XmlOptions xml_opt, BufferedWriter buffw, FileWriter logw, FileWriter errw) {
		...
		BindingResultType body = BindingResultType.Factory.newInstance();
		BindingResult[] list = new BindingResult[1];
		...
		try {
			...
			if (rset.next()) {
				...
				String query2 = new String("select * from \"" + table_name + "\" where \"Entry_ID\"='" + entry_id + "' order by (0 || \"Binding_value_list_ID\")::decimal,(0 || \"ID\")::decimal");
				...
				buffw.write("  <BMRBx:binding_resultCategory>\n");
				...
				while (rset.next()) {
					...
					if (l < lines++) {
						...
						try {
							logw.write("category='BindingResult', duplicated content of the category was excluded.\n");
						} catch (IOException e) {
							e.printStackTrace();
						}
					...
					list[0] = BindingResult.Factory.newInstance();
					...
					if (!set_integer(list[0], "setAtmObsAtomIsotopeNumber", "setNilAtmObsAtomIsotopeNumber", false, rset.getString("Atm_obs_atom_isotope_number"), logw))
						continue;
					if (!set_string(list[0], "setAtmObsAtomType", "setNilAtmObsAtomType", false, rset.getString("Atm_obs_atom_type"), logw))
						continue;
					...
					if (!set_integer(list[0], "setAtmObsCompIndexId", "", false, rset.getString("Atm_obs_comp_index_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAtmObsEntityAssemblyId", "", true, rset.getString("Atm_obs_entity_assembly_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAtmObsEntityId", "", true, rset.getString("Atm_obs_entity_ID"), logw))
						continue;
					if (!set_integer(list[0], "setAtmObsSeqId", "", false, rset.getString("Atm_obs_seq_ID"), logw))
						continue;
					if (!set_integer_experiment_id(list[0], "setExperimentId", "", false, rset.getString("Experiment_ID"), conn_bmrb, entry_id, logw))
						continue;
					if (!set_string(list[0], "setExptObservedParam", "setNilExptObservedParam", false, rset.getString("Expt_observed_param"), logw))
						continue;
					...
					if (!set_decimal(list[0], "setValErr", "setNilValErr", false, rset.getString("Val_err"), logw))
						continue;
					if (!set_string(list[0], "setValType", "setNilValType", false, rset.getString("Val_type"), logw))
						continue;
					if (!set_string(list[0], "setValUnits", "setNilValUnits", false, rset.getString("Val_units"), logw))
						continue;
					if (!set_integer(list[0], "setBindingValueListId", "", true, rset.getString("Binding_value_list_ID"), logw))
						continue;
					...
					body.setBindingResultArray(list);
			...
			Logger lgr = Logger.getLogger(bmr_BindingResultType.class.getName());
	...
	private static boolean set_string(BindingResult list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
		...
		if (!(val_name == null || val_name.isEmpty() || val_name.equals(".") || val_name.equals("?")) && (_val_name == null || _val_name.isEmpty() || _val_name.equals(".") || _val_name.equals("?") || !val_name.equals(_val_name))) {
			...
			try {
				logw.write("item='" + method_name.substring(3) + "', category='BindingResult', value='" + _val_name + "' -> '" + val_name + "'\n");
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (nil && (nil_method_name == null || nil_method_name.isEmpty())) {
			...
			if (required) {
				...
				try {
					logw.write("item='" + method_name.substring(3) + "', category='BindingResult', value='" + _val_name + "' was empty, but not nillable.\n");
				} catch (IOException e) {
					e.printStackTrace();
				}
	...
	private static boolean set_integer(BindingResult list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
	...
	private static boolean set_integer_experiment_id(BindingResult list, String method_name, String nil_method_name, boolean required, String val_name, Connection conn_bmrb, String entry_id, FileWriter logw) {
		...
		if (bmr_Util_Main.remediate_xml)
			val_name = bmr_Util_BMRB.getExperimentID(val_name, conn_bmrb, entry_id, "");
	...
	private static boolean set_decimal(BindingResult list, String method_name, String nil_method_name, boolean required, String val_name, FileWriter logw) {
